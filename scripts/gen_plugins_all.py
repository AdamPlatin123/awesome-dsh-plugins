...
def main():
    ...
    L.append(f'- 已定位明细 {sum(vc.values())}（本列表展示口径，另 {len(entries) - sum(vc.values())} 条监测/未定位的判定暂不展示）：'
             f'🟩 `已兼容`（{vc.get("✅ 运行级可用", 0)}）/ 🟨 `需适配`（{vc.get("❌ 运行级不兼容", 0)}）/ '
             f'⬜ `待测试`（{vc.get("⚠️ 待定", 0) + vc.get("⏳ 未测", 0)}，其中待定 {vc.get("⚠️ 待定", 0)} · 未测 {vc.get("⏳ 未测", 0)}）')
    L.append('')
    ...
    # 按域拆分：GitHub markdown 渲染上限 1MB，单文件清单超限即按源码显示不渲染——
    # 明细写 catalog/all/<域>.md（每域独立远小于上限），PLUGINS-ALL.md 降为索引页
    ...
    for dom in DOMAIN_ORDER:
        ...
        # 域文件独立成页（SVG 相对路径在 catalog/all/ 深度下需 ../../ 前缀）

...
#
# 状态判定：
#   404/无权限 —— gh api 返回 404/403，仓库不存在或无权访问 → 状态"未知"，退出码 2
...
#   0 = 无新提交（仍为占位，无需重新调研）
#   1 = 有仓库出现了提交（需要重新调研）
#   2 = 依赖缺失（bash/git/gh）或状态未知（404/无权限）
#   3 = 离线 / gh 未认证
#
# 说明：gh / git 网络调用均带 timeout，避免挂死；404 不再当作"空仓"通过。
#
...
# ---- 网络预检（区分"离线"与"未认证"） ----
if ! timeout 30 gh api rate_limit >/dev/null 2>&1; then
...
for repo in "${REPOS[@]}"; do
  ...
  # 状态一：404/无权限（仓库不存在或无权访问）→ 状态"未知"，退出码 2
  if ! timeout 30 gh api "repos/$ORG/$repo" --jq '.size' >/dev/null 2>"$tmp"; then
    if grep -qE 'HTTP 40[34]|Resource not accessible|Not Found' "$tmp"; then
      echo '未知（404/无权限）—— 仓库不存在或无权访问，无法确认状态（退出码 2）'
      exit 2
    else
  ...
  if ! refs=$(timeout 30 git ls-remote "https://github.com/$ORG/$repo.git" 2>"$tmp"); then

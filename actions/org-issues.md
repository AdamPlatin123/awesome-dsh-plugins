...
**现状证据**：索引 pin 的 ref 为 `c02a31be…`，而仓库 HEAD 已推进到 `7acd885…`（一次 docs 提交之后未重跑发布脚本）。按"更新插件 = 换 ref"纪律，索引与 HEAD 不同步时，下游按索引安装将拿不到最新提交。
...
**现状证据**：`catalog.json` pin 在 `083d4c7a…`（updated 时间戳 2026-08-04），而仓库 HEAD 已推进到 `07238b2…`（一次测试补齐提交之后未重跑发布）。下游按 catalog 安装会拿到缺少最新测试与修复的旧皮肤。

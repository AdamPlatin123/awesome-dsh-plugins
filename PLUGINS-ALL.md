...
> 数据源：radar 快照并集（`20260819T021501Z` 2026-08-19 10:15 UTC+8 ⊕ `20260819T014501Z` 2026-08-19 09:45 UTC+8 ⊕ `20260819T013001Z` 2026-08-19 09:30 UTC+8 等 66 轮）⊕ GitHub 定位复核缓存（data/locate-cache.json）。
> 呈现：分组列表（状态 · 名称  · 一句话说明），不使用大表格。

## 统一度量衡

**判定维度**（运行级四档，仅已定位条目 2103 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1107）/ `[不兼容]`（509）/ `[待定]`（221）/ `[未测]`（266）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（97）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- `[未定位]`（153）— 新占位条目，待下一轮定位复核（scripts/resolve_placeholders.py）
- 定位复核累计修复 369 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2457 条（已定位 2103 · 监测/未定位 354）· PR 登记 23 个
...
- `[不兼容]` [d-dev0101-open-sea-skin](https://github.com/d-dev0101/open-sea-skin) 175 — WebGPU ocean skin for DeepSeek Harness — DSH plugin, Harness-only Chrome/Edge extension, static installer, and
...
- `[可用]` [afrel1024-dsh-client-ui-aqua-unofficial](https://github.com/afrel1024/dsh-client-ui-aqua-unofficial) 0 — Unofficial community fork of the Aqua glassmorphism theme for the DeepSeek Harness web UI (upstream: WYH666666
...
- `[不兼容]` [mrlfarano-dsh-tailscale-surface](https://github.com/mrlfarano/dsh-tailscale-surface) 0 — DeepSeek Harness plugin: Tailscale serve surface — canonical external URL, identity-gated privileged RPC relay
...
- `[待定]` [shenyan-008-dsh-visual-aid](https://github.com/shenyan-008/dsh-visual-aid) 0 — dsh visual aid plugin: lets text-only main models handle images via a vision model
...
- `[未定位]` **jungeer-dsh-theme-stardew** — 占位待复核，判定暂不展示
...
- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 88889 — 🎨 Best DeepSeek Harness Design Plugin
...
- `[未定位]` **ishuowang-dsh-sideband** — 占位待复核，判定暂不展示
...
- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 14211 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
...
- `[未定位]` **winada82-dsh-telegram-bridge** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-univer-office](https://github.com/dream-num/dsh-univer-office) — Univer 办公插件：在 DSH 中创建/编辑/检查/交付表格、文档、演示文稿、多维表格和画布，支持 xlsx/docx/pptx 导入导出、实时预览与 worktree 审阅；npm `dsh-univer-office` 0.2.5
...
- `[不兼容]` [tong-io-tongflow](https://github.com/tong-io/tongflow) 858 — TongFlow — multimodal AIGC workflow engine (canvas + Python plugin engine) and dsh-tongflow, the DeepSeek Harn
...
- `[不兼容]` [uson1x-dsh-plugin-llm-verifier](https://github.com/uson1x/dsh-plugin-llm-verifier) 0 — LLM-as-a-Verifier for DeepSeek Harness: continuous reward signals via select / compare / track

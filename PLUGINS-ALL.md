...
> 数据源：radar 快照并集（`20260826T130001Z` 2026-08-26 21:00 UTC+8 ⊕ `20260826T121501Z` 2026-08-26 20:15 UTC+8 ⊕ `20260826T111501Z` 2026-08-26 19:15 UTC+8 等 78 轮）⊕ GitHub 定位复核缓存（data/locate-cache.json）。
> 呈现：分组列表（状态 · 名称  · 一句话说明），不使用大表格。

## 统一度量衡

**判定维度**（运行级四档，仅已定位条目 2232 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1140）/ `[不兼容]`（598）/ `[待定]`（236）/ `[未测]`（258）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（97）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- `[未定位]`（241）— 新占位条目，待下一轮定位复核（scripts/resolve_placeholders.py）
- 定位复核累计修复 363 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2674 条（已定位 2232 · 监测/未定位 442）· PR 登记 23 个
...
- `[待定]` [cocode-agency-cocode](https://github.com/cocode-agency/cocode) 154 — Best ready-to-run DeepSeek Harness distribution: DSH desktop GUI, DSH terminal TUI, and harness integration.
...
- `[可用]` [amlyczz-dsh-agy-link](https://github.com/amlyczz/dsh-agy-link) 18 — Google Antigravity (agy CLI) models for DeepSeek Harness — streaming chat, thinking, tool activity, usage, in-
...
- `[可用]` [whitefirer-dsh-niulai-pet](https://github.com/whitefirer/dsh-niulai-pet) 11 — 牛来桌宠：agent 任务完成它就蹦出来喊「妈～～妈～～」——dsh 纯客户端桌宠插件（6 皮肤/签名动作/合成叫声）
...
- `[不兼容]` [xiaoshihou514-dsh-tui](https://github.com/xiaoshihou514/dsh-tui) 2 — DeepSeek Harness: TUI
...
- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 91443 — 🎨 Best DeepSeek Harness Design Plugin
...
- `[不兼容]` [whitelonng-dshcode](https://github.com/whitelonng/dshcode) 349 — Community desktop companion for DeepSeek Harness — one-click Electron app for macOS and Windows
...
- `[不兼容]` [dundunhan-dsh-video-lens](https://github.com/dundunhan/dsh-video-lens) 40 — —
...
- `[不兼容]` [duyanta123-arch-doc](https://github.com/duyanta123/arch-doc) 0 — Analyze a codebase and generate architecture documentation (module responsibilities, dependencies, entry point
...
- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 16174 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
...
- `[待定]` [dsh-mattpocock-skills](https://github.com/xiaoxiaosrm/dsh-mattpocock-skills) 5 — Unofficial DSH port of mattpocock/skills — Engineering (18) + Productivity (7) skills as a DeepSeek Harness bu
...
- `[不兼容]` [goecho-dsh-generation](https://github.com/goecho/dsh-generation) 1 — Fork agent presets and run tasks on the next generation
...
- `[不兼容]` [young-tim-dsh-profile-lab](https://github.com/young-tim/dsh-profile-lab) 3 — Reproducible DSH profile and patch experiment matrices with reports and policy gates
...
- `[不兼容]` [tong-io-tongflow](https://github.com/tong-io/tongflow) 936 — TongFlow — Multimodal GenAI Studio
...
- `[待定]` [xuedai-mayi-dsh-wenshan](https://github.com/xuedai-mayi/dsh-wenshan) 26 — 问山：DeepSeek Harness 地理野外实习知识图谱插件与非官方二次开发项目

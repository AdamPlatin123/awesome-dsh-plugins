...
> 数据源：radar 快照并集（`20260818T040001Z` 2026-08-18 12:00 UTC+8 ⊕ `20260818T023001Z` 2026-08-18 10:30 UTC+8 ⊕ `20260818T013001Z` 2026-08-18 09:30 UTC+8 等 43 轮）⊕ GitHub 定位复核缓存（data/locate-cache.json）。
> 呈现：分组列表（状态 · 名称  · 一句话说明），不使用大表格。

## 统一度量衡

**判定维度**（运行级四档，仅已定位条目 1844 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1044）/ `[不兼容]`（416）/ `[待定]`（186）/ `[未测]`（198）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（74）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- 定位复核累计修复 391 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2020 条（已定位 1844 · 监测/未定位 176）· PR 登记 22 个

## 🎓 技能包（21）

- `[可用]` [forkprobe](https://github.com/Jayden-X-L/forkprobe) 67 — Compare multiple skills on the same task and pick the winner.
- `[可用]` [superpowers-dsh](https://github.com/LayneChai/superpowers-dsh) 54 — Superpowers skills for DeepSeek Harness: TDD, debugging, planning, and collaboration skills adapted from obra/
...
- `[可用]` [dsh-web-plugin-manager](https://github.com/LX2000WASD/dsh-web-plugin-manager) 58 — 在 Web UI 中一键管理 DeepSeek Harness (DSH) 插件：查看、实时启停、安装/卸载、环境管理、插件市场
- `[未测]` [dsh-suite](https://github.com/whyihaveyou/dsh-suite) 40 — DSH 插件活目录 + 脚手架 + 内置插件商店：785+ 插件每小时刷新、每日兼容 CI 实测、中英双语可搜索目录站；含 create-dsh-plugin 脚手架与 plugin-manager / plugin-notify / plugin-session-export / plugin-team-board 五个 npm 包
...
- `[可用]` [dsh-market](https://github.com/2BingLing/dsh-market) 24 — DeepSeek Harness 插件市场 · 持续收录 1000+ DSH 插件：中文搜索 + 实用五维评分 + 一键安装
- `[可用]` [ericwong5021-deepseek-plugin-store](https://github.com/Ericwong5021/deepseek-plugin-store) 21 — —
- `[可用]` [imsai-sh-awesome-deepseek-harness-plugins](https://github.com/imsai-sh/awesome-deepseek-harness-plugins) 71 — DeepSeek Harness plugin store, marketplace and hub — 3,100+ dsh plugins with search, rankings, install command
- `[可用]` [dsh-plugin-healthcheck](https://github.com/chenw2759-wq/dsh-plugin-healthcheck) 10 — 害怕插件装了就崩溃？用这个插件帮你检测插件是否正常/是否含木马！
...
- `[不兼容]` [dsh-manager](https://github.com/KYZHXL/dsh-manager) 8 — DeepSeek Harness plugin marketplace
...
- `[可用]` [omdsh-dev-dsh-hub-workshop](https://github.com/omdsh-dev/dsh-hub-workshop) 4 — —
...
- `[待定]` [dsh-dynplugin-manager](https://github.com/Thomas-key/dsh-dynplugin-manager) 2 — Manage DeepSeek Harness dynamic plugins: scan local directories, browse, and load with /dyn slash commands
...
- `[待定]` [veloce-ailab-dsh-plugin-market](https://github.com/veloce-ailab/dsh-plugin-market) 1 — —
...
- `[不兼容]` [dsh-dynamic-plugins-manager](https://github.com/wacly/dsh-dynamic-plugins-manager) 2 — deepseek harness 动态插件管理
...
- `[可用]` [modlens](https://github.com/liustack/modlens) 2964 — The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent
- `[可用]` [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) 2069 — 一个侧边栏的完整工作台，支持三方拓展注册新侧边栏页面
- `[可用]` [dsh-TUI](https://github.com/ccch1mneyyy/dsh-TUI) 1911 — 解决DSH 官方尚无终端 TUI 痛点的补位之作，献给偏爱cli的各位极客：Claude Code 风格全屏交互终端插件——像素鲸鱼顶栏、实时工作状态行、思考流式展开、双击 Esc 回滚、上下文进度条 + TPS 仪表
- `[不兼容]` [paean-ai-deeptide](https://github.com/paean-ai/deeptide) 1079 — Built by DeepSeek, for DeepSeek — a Swift-native macOS coding agent
...
- `[可用]` [dsh-ads](https://github.com/Nagi-ovo/dsh-ads) 491 — 把 DSH 变成 2005 年门户网站｜Parody ads, fake games, and popups for the DSH Web UI
- `[可用]` [alaliqing-claude-paper](https://github.com/alaliqing/claude-paper) 317 — 📖 Cross-agent research paper toolkit for Claude Code, Codex, OpenCode, and DeepSeek Harness—quick summaries, d
- `[可用]` [whale-girl](https://github.com/vlln/whale-girl) 230 — DSH Web GUI 桌面宠物插件（QQ 宠物形态）：右下角悬浮、可拖拽/投喂/玩耍的积累型伙伴
...
- `[可用]` [oh-dsh](https://github.com/hust-open-atom-club/oh-dsh) 248 — 一套 DSH runtime，Desktop、Web 与 TUI 三种开发体验
...
- `[可用]` [dsh-visualize](https://github.com/Nagi-ovo/dsh-visualize) 171 — 在 DSH 对话中生成交互式可视化｜Render model-generated interactive cards inside DSH conversations
...
- `[可用]` [libukai-awesome-deepseek-harness](https://github.com/libukai/awesome-deepseek-harness) 128 — DeepSeek Harness 终极指南：快速入门、资源推荐、精选插件与实用工具 ｜The Ultimate Guide to DeepSeek Harness: QuickStart, Resources, Plug
- `[可用]` [dsh-webui-market-plugin](https://github.com/Sanqi-normal/dsh-webui-market-plugin) 83 — dsh Web GUI 社区插件市场：浏览 awesome-dsh-plugin.com 插件目录，一键安装/卸载到 profile
- `[不兼容]` [ningbainb-deepseek-harness-desktop](https://github.com/ningbainb/deepseek-harness-desktop) 97 — Open-source Windows desktop client and GUI for DeepSeek Harness — zero-setup installer with Codex, plugins, sk
...
- `[不兼容]` [zat-dsh-engine](https://github.com/mishibeikejie/zat-dsh-engine) 70 — Visual plugin marketplace for DeepSeek Harness — browse, search and install community plugins
- `[可用]` [bowenliang123-dsh-context](https://github.com/bowenliang123/dsh-context) 241 — Best DeepSeek Harness plugin for context insight and management, with context dashboard / browser and context
- `[不兼容]` [kelai141-dsh-mobile-apk](https://github.com/kelai141/dsh-mobile-apk) 69 — dsh 安卓壳 APK——WebView UI + 内嵌 Termux 运行时快照（解压即跑）、SAF 目录桥、保活服务、看门狗、运行时在线更新
- `[不兼容]` [dsh-open-in-vscode](https://github.com/omdsh-dev/dsh-open-in-vscode) 49 — Open DeepSeek Harness workspace directories in VS Code directly from the web GUI.
- `[不兼容]` [bruc3van-dsh-desktop](https://github.com/bruc3van/dsh-desktop) 56 — 让 Agent 安全常驻桌面的独立 dsh 客户端：官方 Web UI 原封不动，长任务常驻托盘，精选插件先审查、再安装
- `[不兼容]` [dsh-skill-viewer](https://github.com/Fishquito7/dsh-skill-viewer) 61 — DSH Web UI plugin: Skills settings section with hot enable/disable, delete and add（Web界面的skill管理工具）
- `[待定]` [dsh-undo-plugin](https://github.com/lire1131/dsh-undo-plugin) 74 — DSH crash-rescue plugin: undo config & plugin-code changes, secret-safe snapshots, one-click SAFE MODE, plus o
- `[不兼容]` [heigeai-deepseek-harness-skin](https://github.com/HeiGeAi/deepseek-harness-skin) 46 — —
...
- `[不兼容]` [openma-ai-deepseek-harness-tui](https://github.com/openma-ai/deepseek-harness-tui) 38 — Self-Improvement TUI Plugin of DeepSeek Harness
...
- `[不兼容]` [dsh-openbiliclaw](https://github.com/whiteguo233/dsh-openbiliclaw) 42 — OpenBiliClaw 是本地运行的跨平台个性化内容推荐 Agent，持续理解你的兴趣并主动找内容
...
- `[不兼容]` [dsh-plugin-session-delete](https://github.com/lsz-asd/dsh-plugin-session-delete) 23 — Delete DeepSeek Harness sessions from the UI: header danger button + sidebar session-row menu item (no convers
...
- `[不兼容]` [kejixiaoliang-awesome-dsh-plugins](https://github.com/kejixiaoliang/awesome-dsh-plugins) 20 — DeepSeek Harness (DSH) 插件精选目录 — 14 类 280+ 个社区插件，覆盖 MCP / Skill / TUI / 多 Agent / 上下文记忆 / UI 皮肤，点链接直达仓库
...
- `[不兼容]` [dsh-tui-dsh-tui](https://github.com/dsh-tui/dsh-tui) 18 — Claude Code-style terminal UI for DeepSeek Harness agents, as an out-of-tree dsh plugin bundle
...
- `[不兼容]` [dsh-diff-viewer](https://github.com/lehhair/dsh-diff-viewer) 18 — DSH Web GUI PiUI-style diff viewer plugin: replaces the stock DiffBlock for write/edit tool calls via ui-tool
...
- `[可用]` [dsh-drag-and-drop](https://github.com/bill9109/dsh-drag-and-drop) 16 — 为 DSH Web UI 增加跨平台文件拖拽与原始路径插入能力，无需复制文件
- `[可用]` [liguobao-dsh-desktop](https://github.com/liguobao/dsh-desktop) 17 — An independent, open-source desktop wrapper for DeepSeek Harness
- `[可用]` [runzhliu-deepseek-harness-docker](https://github.com/runzhliu/deepseek-harness-docker) 20 — Community Docker and Kubernetes packaging for DeepSeek Harness (@deepseek-ai/dsh), with a hardened image, Comp
...
- `[待定]` [techysy-deepseek-harness-fnos](https://github.com/techysy/deepseek-harness-fnos) 13 — DeepSeek Harness (DeepSeek 官方 agent 浏览器 UI) fnOS 应用 — 本地常驻服务
...
- `[可用]` [railgun0325-dsh-phone](https://github.com/railgun0325/dsh-phone) 10 — 让 DeepSeek Harness 的 agent 跑在手机里，通过 Magisk root 原生操作安卓系统（截图/点击/滑动/开应用）+ 移动端布局 + WebView APK
...
- `[可用]` [dsh-plugin-better-sidebar-plugin-office](https://github.com/HuanLinOTO/dsh-plugin-better-sidebar-plugin-office) 20 — 为 better-sidebar 提供 Office 三件套预览（.docx/.xlsx/.pptx），独立 bundle 瘦身主体 \| Provides Office-suite preview (.docx/.xls
...
- `[可用]` [dsh-context-doctor](https://github.com/Zhenyu98/dsh-context-doctor) 13 — DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具
...
- `[可用]` [deepseek-harness-tui](https://github.com/boxeryao/deepseek-harness-tui) 10 — DSH-TUI: a lightweight and fast terminal plugin connected directly to the DSH runtime.
...
- `[可用]` [dsh-better-archive](https://github.com/huahai0202/dsh-better-archive) 6 — DeepSeek Harness (DSH) web-GUI plugin: archived-session panel with unarchive & delete
...
- `[可用]` [dsh-portable-tavern](https://github.com/XCNXNXNX/dsh-portable-tavern) 16 — DeepSeek Harness 的「便携酒馆」插件：RPG 式 SillyTavern V2/V3 角色卡生成器 + 酒馆角色扮演聊天
...
- `[可用]` [dsh-mobile-gui-agent](https://github.com/kunjinkao-os/dsh-mobile-gui-agent) 5 — Android Mobile GUI Agent plugin for DeepSeek Harness with ADB control, iterative verification, approvals, and
...
- `[可用]` [dsh-douyin](https://github.com/AnacondaKC/dsh-douyin) 4 — DSH WebUI 侧栏短视频插件：原生播放器、系列导航、直链解析与精确历史回放
...
- `[可用]` [cccakeee-awesome-dsh-plugins](https://github.com/cccakeee/awesome-dsh-plugins) 4 — A curated, evidence-led directory of DeepSeek Harness (DSH) plugins: verified loadable extensions, skills, and
...
- `[可用]` [omdsh-dev-dsh-input-history](https://github.com/omdsh-dev/dsh-input-history) 3 — —
...
- `[可用]` [Better_Deepseek_Harkness](https://github.com/silencieuxzero/Better_Deepseek_Harkness) 4 — Better Deepseek Harness, with some functional extensions to webui and Deepseek Harness·更好的deepseek harness，对we
...
- `[可用]` [dsh-claude-tui](https://github.com/cogine-ai/dsh-claude-tui) 7 — Claude-Code-Styled TUI for DeepSeek Harness
...
- `[可用]` [dsh-skin-claude-code](https://github.com/LucasN0820/dsh-skin-claude-code) 3 — Claude Code-inspired skin for the DeepSeek Harness web GUI
...
- `[可用]` [omdsh-dev-dsh-ui-progress](https://github.com/omdsh-dev/dsh-ui-progress) 2 — —
...
- `[可用]` [renat3u-tonghuashun-webui](https://github.com/renat3u/tonghuashun-webui) 6 — —
...
- `[待定]` [dsh-plugin-terminal](https://github.com/siberiah2o/dsh-plugin-terminal) 4 — Bottom terminal panel plugin for DeepSeek Harness (DSH Web GUI)
...
- `[可用]` [dsh-skin-amis](https://github.com/wanzhiwei5/dsh-skin-amis) 2 — 鸣潮爱弥斯主题皮肤: 粉白配色+赛博霓虹装饰的 DeepSeek Harness Web GUI 皮肤 / Amis-inspired pink-white s
...
- `[不兼容]` [dsh-token-viewer](https://github.com/qwert702/dsh-token-viewer) 6 — DSH web GUI plugin: live token consumption surfaces (composer dock strip + sideb
...
- `[待定]` [dsh-Solarized](https://github.com/zhijun-dai/dsh-Solarized) 3 — Solarized + Selenized themes for DeepSeek Harness (dsh): four faithful palettes
...
- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 88599 — 🎨 Best DeepSeek Harness Design Plugin
- `[不兼容]` [titanwings-colleague-skill](https://github.com/titanwings/colleague-skill) 23264 — 将冰冷的离别化为温暖的 Skill，欢迎加入数字生命1.0！Transforming cold farewells into warm skills? It's giving rebirth era
- `[未测]` [TokenTracker](https://github.com/xiufengsun/TokenTracker) 1353 — Local-first AI token usage & cost tracker for 31 coding tools incl
- `[可用]` [mnemon-dev-mnemon](https://github.com/mnemon-dev/mnemon) 474 — LLM-supervised persistent memory for AI agents — graph-based recall, cross-session knowledge, single binary
- `[可用]` [dsh-market-dsh-market](https://github.com/dsh-market/dsh-market) 931 — The plugin market inside DeepSeek Harness — browse, search, one-click install · DSH 可视化插件市场
- `[待定]` [dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams) 524 — AgentTeams plugin for DeepSeek Harness
- `[不兼容]` [dsh-at-file](https://github.com/omdsh-dev/dsh-at-file) 358 — Codex-style @file mentions for DeepSeek Harness: search workspace files in the composer and attach their path
...
- `[可用]` [drewnekota-cetus](https://github.com/drewnekota/cetus) 116 — One macOS app for Claude Code, Codex, and every agent runtime you use — scheduled runs, global hotkey launcher
- `[可用]` [modsearch](https://github.com/liustack/modsearch) 139 — The web search plugin for DeepSeek Harness, and the search bridge for every model without native web access
- `[可用]` [dsh-memory-evolve](https://github.com/csyangwen/dsh-memory-evolve) 166 — 为 DeepSeek Harness 带来「跨会话长期记忆 + 后台自我进化」能力的纯插件实现：五轨记忆 · git 分支感知 · 回合内自我审查 · 技能自我进化与技能管理器 · 四轨待办 · COI 调度 · 会话广 〔PR〕
- `[不兼容]` [taxueseek-argo](https://github.com/taxueseek/argo) 100 — 专门为 agent 打造的 agent 搜索工具，具备多语言搜索能力，覆盖中文/英文/学术/代码/购物/金融/新闻/百科
- `[不兼容]` [pulseaiclub-phi](https://github.com/pulseaiclub/phi) 105 — a coding Agent from pi
...
- `[可用]` [anysearch-dsh](https://github.com/anysearch-team/anysearch-dsh) 91 — AnySearch web search provider and advanced search tools for DeepSeek Harness (DSH)
- `[待定]` [dsh_workflow](https://github.com/icetomoyo/dsh_workflow) 80 — 把Claude Code的UltraCode模式带给DSH，把 DSH 的一次性多 Agent 调度，升级为可生成、可保存、可治理、可观察、可恢复的 Workflow 层
...
- `[待定]` [dsh-find-plugin](https://github.com/awesome-dsh-plugin/dsh-find-plugin) 52 — Find DSH plugins inside the agent — live GitHub dsh-plugin topic search, star-ranked / 会话内搜索发现 DSH 插件
...
- `[可用]` [cpj-dev-dsh-plugin-cc](https://github.com/cpj-dev/dsh-plugin-cc) 3 — Bridge Deepseek-harness into Claude Code for review, critique, delegation, and session import.
...
- `[可用]` [dsh-commandcode-provider](https://github.com/Mars-Sea/dsh-commandcode-provider) 58 — Unofficial DeepSeek Harness LLM provider plugin for Command Code: live model catalog, reasoning-effort support
...
- `[不兼容]` [dsh-toolkit](https://github.com/omdsh-dev/dsh-toolkit) 21 — DSH 零依赖工具包 collection —— time / encoding / json / calculator / csv / regex / markdown / diff / stat / schema 十
...
- `[可用]` [dsh-file-review](https://github.com/left0ver/dsh-file-review) 18 — a dsh plugin - review  files that an agent just changed,you can see the diff
...
- `[不兼容]` [pivotstackintelligence-dsh-github](https://github.com/PivotStackIntelligence/dsh-github) 4 — —
...
- `[可用]` [dsh-web-search-pro](https://github.com/anweat/dsh-web-search-pro) 24 — Enhanced, persistent web search plugin for DeepSeek Harness (multi-engine search, SQLite+LRU cache, platform b
...
- `[可用]` [dsh-plugin-product-subagents](https://github.com/shaokeyibb/dsh-plugin-product-subagents) 17 — Role-based Codex / Claude Code / ACP subagent providers for the DeepSeek Harness — continuable children, durab
- `[待定]` [deepseek-harness-for-vscode](https://github.com/skymecode/deepseek-harness-for-vscode) 14 — deepseek-harness for vscode .This is a community project, and we welcome your valuable feedback!
- `[可用]` [dsh-llm-codebuddy](https://github.com/Axiaohungry/dsh-llm-codebuddy) 15 — 在deepseek harness中使用workbuddy api，因为公司只提供workbuddy积分
...
- `[可用]` [dsh-vision-opencode](https://github.com/poiuyjie/dsh-vision-opencode) 9 — DSH plugin: Auto-convert images to text for pure-text LLMs (DeepSeek etc.) via any vision model
...
- `[可用]` [deepseek-harness-vsc-extension](https://github.com/weinibuliu/deepseek-harness-vsc-extension) 19 — DeepSeek Harness for VS Code as extension
...
- `[可用]` [dsh-claude-compat](https://github.com/biedongbin/dsh-claude-compat) 6 — DSH plugin: bridge Claude Code's .claude/ directory (skills, commands, rules) into DeepSeek Harness natively
...
- `[可用]` [dsh-continual-evolve](https://github.com/ZK-Andy/dsh-continual-evolve) 12 — Continual self-evolution plugin for DeepSeek Harness: versioned, auditable, rollback-safe harness state refine
...
- `[可用]` [dsh-hud](https://github.com/a903067276-rgb/dsh-hud) 7 — HUD status panel plugin for DeepSeek Harness (dsh) web: git status, MCP servers, skills, model & token usage i
...
- `[可用]` [codex-plugin-dsh](https://github.com/wingoo/codex-plugin-dsh) 6 — Use local Codex App Server as a model provider in DeepSeek Harness
- `[可用]` [dsh-token-monitor](https://github.com/zhangzheng25/dsh-token-monitor) 5 — DeepSeek Harness plugin: token usage & conversation stats as a native settings page - today / 7d / 30d totals,
...
- `[不兼容]` [dsh-win-terminal-inspector](https://github.com/clearkurt/dsh-win-terminal-inspector) 6 — Windows (win32) terminal inspection for DSH persistent/PTY shells
...
- `[可用]` [inmny-dsh-git-bash](https://github.com/inmny/dsh-git-bash) 3 — —
...
- `[待定]` [v587d-dsh-opencode-go-usage](https://github.com/v587d/dsh-opencode-go-usage) 3 — —
...
- `[可用]` [dsh-file-mentions](https://github.com/a903067276-rgb/dsh-file-mentions) 9 — Clickable file paths in DSH replies: Codex-style inline open, 📂 reveal in file manager, mentioned-files chip l
...
- `[可用]` [dsh-failure-capsule](https://github.com/YiHarvest/dsh-failure-capsule) 3 — Local-first failure evidence capsules for DeepSeek Harness sessions
...
- `[未测]` [dsh-openai-codex-auth](https://github.com/yoke233/dsh-openai-codex-auth) 6 — OpenAI Codex OAuth login and usage card plugin for DeepSeek Harness
...
- `[待定]` [knowlp-rag](https://github.com/wly8691-jpg/knowlp-rag) 3 — KnowLP-RAG: dual knowledge-graph RAG for Markdown notes — dsh plugin add @eqman00003/knowlp-rag · MCP + native
...
- `[可用]` [dsh-commandcode-provider](https://github.com/Kristin130/dsh-commandcode-provider) 2 — dsh LLM provider plugin for Command Code — every plan incl
...
- `[可用]` [dsh-plugin-commandcode-provider](https://github.com/mitian233/dsh-plugin-commandcode-provider) 1 — —
...
- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 14047 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
- `[可用]` [foryourhealth111-pixel-vibe-skills](https://github.com/foryourhealth111-pixel/Vibe-Skills) 2882 — —
- `[不兼容]` [anionex-agent-vision-toolkit](https://github.com/Anionex/agent-vision-toolkit) 1020 — —
- `[可用]` [hellowind777-helloagents](https://github.com/hellowind777/helloagents) 688 — —
...
- `[不兼容]` [dsh-vision-toolkit](https://github.com/Anionex/dsh-vision-toolkit) 676 — 让纯文本模型更好地做视觉任务的DeepSeek Harness插件：带意图的图片问答、长截图 OCR、UI 还原等｜DeepSeek Harness-native integration for agent-vision
...
- `[可用]` [morluto-rea](https://github.com/morluto/rea) 347 — Reverse engineer anything with agents, from app behavior down to native binaries.
- `[不兼容]` [Abu-Cowork](https://github.com/PM-Shawn/Abu-Cowork) 334 — Open-source alternative to Claude Cowork — a local-first AI agent desktop app · multi-model · self-evolving sk
- `[可用]` [linhay-harmony-next-skills](https://github.com/linhay/harmony-next.skills) 329 — —
- `[不兼容]` [openma-ai-open-managed-agents](https://github.com/openma-ai/open-managed-agents) 237 — Open-source Claude Managed Agents API implementation and self-hosted Claude Tag-style agent runtime
- `[可用]` [pingfanfan-hello-dsh](https://github.com/pingfanfan/hello-dsh) 71 — 从零开始，看懂 DeepSeek Harness 的「万物皆可插件」— 零基础插件开发教程（含 22 个中文技能实例）\| Zero-to-plugin tutorial for DeepSeek Harness
- `[可用]` [morluto-jacobian](https://github.com/morluto/jacobian) 52 — Pure mathematics for agents: search for examples and counterexamples, compute exactly, and independently check
- `[不兼容]` [mstar-harness](https://github.com/btspoony/mstar-harness) 49 — An omni-plugin for harness engineering workflows with multi-agents, programmatic gates and skills.
- `[不兼容]` [dsh-automation](https://github.com/titanwings/dsh-automation) 53 — DSH 自动化插件：让 Coding 任务按计划在全新 Agent Session 中运行，并由用户或 Agent 创建和管理定时任务
- `[可用]` [hanelalo-browser-bridge](https://github.com/hanelalo/browser-bridge) 37 — 让你的agent真的像你一样操控你的浏览器窗口
- `[待定]` [dsh-data-agent](https://github.com/omdsh-dev/dsh-data-agent) 52 — Connect DSH to your database for conversational data analysis and actionable business insights.
- `[可用]` [openguardrails-openguardrails](https://github.com/openguardrails/openguardrails) 28 — The vendor-neutral protocol for AI agent safety & security — and the neutral benchmark that ranks the vendors.
...
- `[待定]` [dsh-super-injector](https://github.com/yjh051108/dsh-super-injector) 101 — —
- `[不兼容]` [atlascloudai-atlas-cloud-skills](https://github.com/AtlasCloudAI/atlas-cloud-skills) 23 — —
- `[可用]` [dsh-reverse-skill](https://github.com/dhicoc/dsh-reverse-skill) 30 — Complete reverse-skill (85 SKILL.md) as a DeepSeek Harness (dsh) Cordis plugin — reverse engineering, authoriz
- `[可用]` [dsh-auto-memory](https://github.com/Aik358/dsh-auto-memory) 15 — DSH 自动记忆插件:三层记忆(用户级/项目笔记/每日日志)自动注入与检索、每日反思、可视化面板与设置页,支持继承其他 AI 工具的历史记忆
...
- `[不兼容]` [dsh-record-replay](https://github.com/humblebanana/dsh-record-replay) 9 — DeepSeek Harness record macOS desktop workflows by demonstration and turn them into agent skills (open-record-
...
- `[可用]` [dsh-plugin-langfuse](https://github.com/linyp/dsh-plugin-langfuse) 9 — Langfuse observability for DeepSeek Harness (dsh): exports agent sessions as OpenTelemetry trace trees (GenAI
...
- `[待定]` [dsh-harness-mcp-server](https://github.com/chushixixin/dsh-harness-mcp-server) 7 — Expose DeepSeek Harness agent capabilities as an MCP server (brain=Hermes, arms=Harness)
...
- `[可用]` [context-vista](https://github.com/GooodWei/context-vista) 8 — 为 DeepSeek Harness 提供右侧悬浮栏以及 /context 命令，用环形图实时展示当前上下文 token 用量与分配，compact指令效果，同时支持估算费用消耗，对标 Claude Code 的 /co
...
- `[可用]` [dsh-evolve](https://github.com/william-jin-cmu/dsh-evolve) 10 — 自进化插件：agent 在 session 内随对话给自己长出/剪掉能力 —— evolve_add 热挂载持久化 cordis 插件（下一 step 工具即可见），evolve_remove 可逆卸载，重启自动恢复
...
- `[可用]` [dsh-media-skills](https://github.com/akqwpeter-prog/dsh-media-skills) 8 — —
- `[未测]` [dsh-prompt-persona](https://github.com/Xilin3/dsh-prompt-persona) 8 — DSH plugin: edit the system prompt (deployment persona) from the Settings page, with live preview.
...
- `[待定]` [dsh-noema](https://github.com/ZSeven-W/dsh-noema) 106 — Noema long-term memory plugin for DSH: durable, inspectable agent memory with recall tools and a settings page
...
- `[可用]` [dsh-acp-plugin](https://github.com/agentic-control-plane/dsh-acp-plugin) 4 — Agentic Control Plane for DeepSeek Harness — policy-check every tool call before it runs
...
- `[未测]` [dsh-smarthome](https://github.com/YLifeOnlyOnce/dsh-smarthome) 4 — Home Assistant control for DeepSeek Harness agents — approval-gated lights, switches, climate
...
- `[待定]` [dsh-im-gateway](https://github.com/jelech/dsh-im-gateway) 2 — An IM gateway for the DeepSeek Harness: bridge messengers into harness agent ses
...
- `[可用]` [dsh-navbar](https://github.com/vlln/dsh-navbar) 38 — DSH 插件：对话节点导航条（右缘节点串快速跳转 user 消息）
- `[待定]` [dsh-interconnect](https://github.com/Chinesezjc/dsh-interconnect) 32 — Cross-instance message/event handoff plugins for DSH (interconnect service + tools)
...
- `[可用]` [dsh-message-edit](https://github.com/Moeblack/dsh-message-edit) 30 — DSH plugin: branch-based message editing, reroll, retry, version timeline
- `[可用]` [dsh-lark](https://github.com/omdsh-dev/dsh-lark) 32 — Lark/Feishu IM bot channel for DeepSeek Harness \| 飞书 DeepSeek Harness （DSH）插件
...
- `[可用]` [ChatCCC](https://github.com/wzj998/ChatCCC) 20 — 飞书（Lark）或微信（WeChat）聊天控制 DeepSeek Harness / Claude Code / Cursor / Codex / CCC Agent
- `[不兼容]` [dingkaihu63-dsh-robotic-harness](https://github.com/dingkaihu63/dsh-robotic-harness) 14 — Robotic Harness: embodied-intelligence research tools for DeepSeek Harness - robot asset inspection, MuJoCo pi
...
- `[可用]` [deepseek-manners](https://github.com/Moeblack/deepseek-manners) 13 — DSH 插件：给每次消息后注入感谢语（deepseek-manners）
- `[不兼容]` [franksong2702-dsh-codex-connect](https://github.com/franksong2702/dsh-codex-connect) 25 — ChatGPT OAuth and Codex models for DeepSeek Harness.
- `[待定]` [dsh-qq2006](https://github.com/LaplaceYoung/dsh-qq2006) 17 — DSH (DeepSeek Harness) 的 QQ2006 皮肤插件：注册 qq2006 主题、镜像 body[data-ds-skin]、全局皮肤表与完整素材
...
- `[可用]` [dsh-session-notification](https://github.com/dingyi222666/dsh-session-notification) 13 — 提供会话完成等四种状态的通知响应，支持浏览器提示和提示词
...
- `[待定]` [dsh-codex-auth](https://github.com/suntianc/dsh-codex-auth) 9 — DeepSeek Harness plugin that reuses the local Codex CLI ChatGPT login and adds a native GPT Auth settings card
...
- `[待定]` [dsh-telegram-channel](https://github.com/hi-wenw/dsh-telegram-channel) 7 — DeepSeek Harness Telegram mobile remote: bind live Web sessions (Codex-style)
...
- `[未测]` [dsh-codex](https://github.com/Yan-Zero/dsh-codex) 25 — Use your ChatGPT subscription in DeepSeek Harness through OpenAI's Codex sign-in
- `[不兼容]` [justgenius-s-dsh-plugs](https://github.com/JustGenius-s/DSH-Plugs) 6 — —
- `[待定]` [dsh-bottom-bar](https://github.com/kc0ed/dsh-bottom-bar) 7 — 用于提供更丰富的DeepSeek Harness底栏信息显示插件
...
- `[未测]` [dsh-expression](https://github.com/yyh-001/dsh-expression) 5 — 找得到、发得出 —— DSH 表情包插件：语义搜图，只发真实文件，走 companion QQ 通道
...
- `[可用]` [dsh-voice](https://github.com/3274375092/dsh-voice) 4 — DeepSeek Harness 的语音输入插件:网页里点 🎤(或按Ctrl+空格)说话,识别文本作为普通消息提交进会话
...
- `[可用]` [dsh-task-notify](https://github.com/kaotusi/dsh-task-notify) 3 — DeepSeek Harness (DSH) system-level task notifications: approval required / awai
...
- `[可用]` [dsh-codex-subscription](https://github.com/WSL043/dsh-codex-subscription) 11 — 在 DeepSeek Harness 中使用 ChatGPT/Codex 订阅，支持 OAuth 登录、额度与重置时间 \| ChatGPT/Codex subs
...
- `[不兼容]` [imsai-sh-zhuzhiliao](https://github.com/imsai-sh/zhuzhiliao) 2833 — 竹知了 —— 一转就哇哇叫的传统玩具，Web 模拟版
- `[可用]` [bruc3van-awesome-dsh-plugin](https://github.com/bruc3van/awesome-dsh-plugin) 216 — 用 30 秒为你的 DeepSeek Harness找到合适的插件
- `[可用]` [dsh-openpencil](https://github.com/ZSeven-W/dsh-openpencil) 116 — OpenPencil design preview and editing plugin for DSH
- `[待定]` [dsh-plugin-mineru](https://github.com/HuanLinOTO/dsh-plugin-mineru) 35 — 向模型暴露 MinerU 文档解析工具，将 PDF/图片/DOCX/PPTX/XLSX 转为结构化 Markdown/JSON \| Exposes MinerU document-parsing tools to the
- `[可用]` [modusensus-dsh-mneme](https://github.com/modusensus/dsh-mneme) 25 — Structured memory engine for DeepSeek Harness
- `[不兼容]` [dsh-vision-fix](https://github.com/zhouwumu2-lab/dsh-vision-fix) 15 — Temporary fork for PR: fix dsh.plugin.json in files
- `[可用]` [dsh-openmaic](https://github.com/THU-MAIC/dsh-openmaic) 14 — OpenMAIC for DeepSeek Harness: classrooms, slides, interactive widgets, and Socratic teaching
...
- `[可用]` [surfing-plugin](https://github.com/cyijun/surfing-plugin) 12 — SearXNG search and Crawl4AI fetch providers for DeepSeek Harness
...
- `[可用]` [dsh-files](https://github.com/taxueseek/dsh-files) 13 — DeepSeek Harness dual-face plugin: session-isolated file upload with colorful composer cards + read_document t
- `[可用]` [dsh-archived-sessions](https://github.com/Zephyr-vibe/dsh-archived-sessions) 13 — DSH Session Manager: manage conversations, archive/restore, delete safely, open record folders.
...
- `[不兼容]` [dsh-file-mount](https://github.com/acefun29/dsh-file-mount) 5 — —
...
- `[不兼容]` [deepseek-harness-file-upload-ocr-plugin](https://github.com/BYYY-eng/deepseek-harness-file-upload-ocr-plugin) 3 — DeepSeek Harness 文件上传与本地 OCR 插件 \| File upload and local OCR plugin for PDF, Word
...
- `[可用]` [omdsh-dev-dsh-drag-and-drop](https://github.com/omdsh-dev/dsh-drag-and-drop) 3 — DeepSeek Harness plugin: dsh-drag-and-drop (org copy)
...
- `[可用]` [schhaohao-dsh-file-explorer](https://github.com/schhaohao/dsh-file-explorer) 2 — —
...
- `[可用]` [crafter-station-petdex](https://github.com/crafter-station/petdex) 3884 — A public gallery of animated pets for Codex, Claude Code, DeepSeek Harness, Hermes, OpenCode, Gemini CLI, and
- `[待定]` [small-tailqwq-dsh-deep-whale](https://github.com/Small-tailqwq/dsh-deep-whale) 1308 — —
...
- `[不兼容]` [pc2005-cloud-dsh-pet](https://github.com/PC2005-cloud/dsh-pet) 119 — —
- `[可用]` [liyupi-dsh-kun-like-pet](https://github.com/liyupi/dsh-kun-like-pet) 58 — Kun Like 桌宠 —— DeepSeek Harness 桌面宠物插件：右下角小坤宠随 Agent 工作状态切换 9 种动作，任务完成播放「你干嘛~哎哟」
- `[可用]` [awu12277-dsh-stock-watch](https://github.com/Awu12277/dsh-stock-watch) 47 — —
- `[可用]` [deepseek-pet](https://github.com/keleus/deepseek-pet) 31 — 在你的deepseek-harness上养一只吃白饭的大蓝鲸
- `[待定]` [dsh-emoji](https://github.com/hellodigua/dsh-emoji) 24 — 让 AI 回复加入自定义表情，支持Bilibili、小红书、贴吧、知乎等多平台表情包，或自定义表情
- `[不兼容]` [huanlinoto-dsh-plugin-pet-rs](https://github.com/HuanLinOTO/dsh-plugin-pet-rs) 19 — —
- `[可用]` [dsh-gomoku](https://github.com/omdsh-dev/dsh-gomoku) 13 — 在DSH中与AI下五子棋，也可以让AI对局，看哪个AI棋力更强
- `[可用]` [dsh-stickers](https://github.com/william-jin-cmu/dsh-stickers) 20 — DSH WebUI sticker plugin for bidirectional user and agent reactions
...
- `[可用]` [dsh-godot-skill](https://github.com/akira399/dsh-godot-skill) 8 — Godot Engine 4.x 全栈游戏开发技能插件 for DeepSeek Harness (DSH) — registers the godot-4-development skill at runtime
- `[可用]` [dsh-answer-pet](https://github.com/Nanki-nn/dsh-answer-pet) 9 — —
- `[可用]` [dsh-live2d-pets](https://github.com/cyanfish-x/dsh-live2d-pets) 7 — Live2D 桌宠插件 for DeepSeek Harness：Agent 状态镜像 + 互动陪伴，内置宽松许可预设模型 / Live2D pet plugin: agent state mirror + intera
...
- `[可用]` [dsh-pets](https://github.com/hellosz/dsh-pets) 4 — —
...
- `[不兼容]` [dsh-desktop-pet](https://github.com/2002yxy/dsh-desktop-pet) 2 — Unofficial customizable desktop-pet plugin for DeepSeek Harness
...
- `[不兼容]` [anywhere-labs-deepseek-harness-desktop](https://github.com/anywhere-labs/deepseek-harness-desktop) 12756 — 为 DeepSeek Harness (DSH) 插件生态打造的现代化桌面端解决方案
- `[不兼容]` [dataelement-dsh-desktop](https://github.com/dataelement/dsh-desktop) 875 — DeepSeek Harness Desktop
- `[可用]` [hairyf-deepseek-harness-desktop](https://github.com/hairyf/deepseek-harness-desktop) 457 — DeepSeek Harness Tauri 桌面版 \| Only 5mb installer, zero environment setup
...
- `[不兼容]` [lum1104-dsh-browser](https://github.com/Lum1104/dsh-browser) 278 — —
- `[不兼容]` [steven-kid-deepseek-harness-desktop](https://github.com/steven-kid/deepseek-harness-desktop) 152 — Minimal desktop wrapper for DeepSeek Harness 极简 Deespeek Harness 桌面端，跨平台，免配置，开箱即用
- `[待定]` [Deepseek-Harness-Desktop](https://github.com/ChisaAlter/Deepseek-Harness-Desktop) 116 — DSH桌面端，支持主题和背景图等多种个性化配置
- `[可用]` [dsh-notification](https://github.com/omdsh-dev/dsh-notification) 59 — Desktop notifications for DeepSeek Harness turn completions, with per-outcome controls and include/exclude key
...
- `[待定]` [deepseek-harness-wallet](https://github.com/feibi-mochi/deepseek-harness-wallet) 30 — Balance monitoring, per-session spend & token tracking, low-balance alerts, and an official recharge shortcut
- `[不兼容]` [ego-browser](https://github.com/Fisfzy/ego-browser) 24 — DSH（DeepSeek Harness）插件：把 ego-lite 浏览器（给 AI Agent 用的 Chromium）接入 HARNESS——13 个结构化 ego_* 工具（文本语义快照、语义定位点击、表单填充、
...
- `[可用]` [flymysql-dsh-remote](https://github.com/flymysql/dsh-remote) 21 — Remote-work assistant for DeepSeek Harness (DSH): connect via SSH (key or password), pick a remote workspace,
- `[不兼容]` [dsh-mobile](https://github.com/lehhair/dsh-mobile) 18 — —
...
- `[可用]` [lbwnb666-ai-deepseekharnessremotegateway](https://github.com/lbwnb666-ai/DeepSeekHarnessRemoteGateway) 17 — —
- `[可用]` [omdsh-dev-deepseek-harness-desktop](https://github.com/omdsh-dev/deepseek-harness-desktop) 8 — DSH 桌面应用打包器
- `[不兼容]` [dsh-melody-launcher](https://github.com/rirko/dsh-melody-launcher) 11 — dsh-旋律启动器：DeepSeek Harness 桌面启动器与插件管理器
...
- `[待定]` [sorsama-deepseek-harness-mobile](https://github.com/sorsama/deepseek-harness-mobile) 16 — Android companion for DeepSeek Harness \| chat, goals, approvals & notifications from your phone, over your LAN
...
- `[可用]` [ankocd-dsh-server-deployment](https://github.com/AnkoCD/dsh-server-deployment) 7 — —
...
- `[不兼容]` [dsh-loop](https://github.com/vlln/dsh-loop) 4 — DSH 插件：定时循环（/loop 命令 + loop 工具 + 活动状态条）
...
- `[不兼容]` [microtree9-deepseek-harness-desktop](https://github.com/microtree9/deepseek-harness-desktop) 4 — dsh桌面客户端
...
- `[不兼容]` [diamondfsd-deepseek-harness-desktop](https://github.com/diamondfsd/deepseek-harness-desktop) 2 — —
...
- `[可用]` [ethanweave-glm4v-vision-mcp](https://github.com/ethanweave/glm4v-vision-mcp) 2 — —
...
- `[不兼容]` [huangj17-deepseek-harness-desktop](https://github.com/huangj17/deepseek-harness-desktop) 6 — DeepSeek Harness Desktop — an unofficial, ready-to-use macOS, Windows, and Linux wrapper for DeepSeek Harness.
...
- `[不兼容]` [fellow99-deepseek-harness-desktop](https://github.com/fellow99/deepseek-harness-desktop) 1 — 基于Electron，针对deepseek-harness封装的桌面版
- `[不兼容]` [hbhszy-deepseek-harness-desktop](https://github.com/hbhszy/deepseek-harness-desktop) 1 — —
...
- `[不兼容]` [hikariming-dshfind](https://github.com/hikariming/dshfind) 148 — DSH (DeepSeek Harness) 原理学习、插件市场与最佳实践 · Learn DSH principles, plugin marketplace & best practices
- `[不兼容]` [dsh-deep-research](https://github.com/omdsh-dev/dsh-deep-research) 14 — Adaptive deep-research orchestrator plugin for DeepSeek Harness (official workflow engine, cybernetics/informa
- `[待定]` [dsh-deepresearch](https://github.com/havingautism/dsh-deepresearch) 8 — —
...
- `[可用]` [deepseek-ai-deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) 155663 — DeepSeek Harness: Everything is a Plugin.
...
- `[可用]` [devin-axis-ipollowork](https://github.com/Devin-AXIS/iPolloWork) 4137 — —
- `[不兼容]` [awesome-dsh-plugin-awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin) 8099 — A curated list of plugins for DeepSeek Harness (dsh) · DeepSeek Harness 插件精选列表
- `[可用]` [adamplatin123-awesome-dsh-plugins](https://github.com/AdamPlatin123/awesome-dsh-plugins) 1177 — —
- `[可用]` [PicGo-Core](https://github.com/PicGo/PicGo-Core) 978 — :zap:The ultimate image uploading engine
- `[不兼容]` [0xsline-awesome-deepseek-harness](https://github.com/0xsline/awesome-deepseek-harness) 714 — DeepSeek Harness (DSH) ecosystem: curated plugins, tools, and infrastructure from dsh-external/hub and the pub
- `[不兼容]` [electricitysheep-dsh-handbook](https://github.com/Electricitysheep/dsh-handbook) 477 — —
...
- `[不兼容]` [ruler4396-dsh-launcher](https://github.com/Ruler4396/dsh-launcher) 140 — —
- `[可用]` [nagi-ovo-dsh-find-plugins](https://github.com/Nagi-ovo/dsh-find-plugins) 141 — —
- `[可用]` [sikao-engine-kimix](https://github.com/Sikao-Engine/KimiX) 106 — —
- `[不兼容]` [dominic789654-awesome-deepseek-harness](https://github.com/Dominic789654/awesome-deepseek-harness) 139 — —
- `[未测]` [deep-flow](https://github.com/zimenglan-sysu-512/deep-flow) 71 — —
- `[不兼容]` [alex-yanggg-awesome-dsh-plugin](https://github.com/Alex-Yanggg/awesome-DSH-plugin) 68 — —
- `[可用]` [dsh-turn-rewind](https://github.com/Anionex/dsh-turn-rewind) 80 — deepseek harness对话和代码状态回退插件 \| DSH — rewind conversation and workspace state, powered by a persistent Change Le
- `[可用]` [ayase34-gal-view](https://github.com/Ayase34/gal-view) 88 — —
- `[可用]` [lyn-77-promentor](https://github.com/Lyn-77/ProMentor) 61 — —
- `[可用]` [dsh-toy](https://github.com/c3ll256/dsh-toy) 55 — Toy Control Protocol for DSH
- `[不兼容]` [oil-oil-dsh-vision](https://github.com/oil-oil/dsh-vision) 67 — Near-native image understanding for DeepSeek Harness
- `[不兼容]` [laplaceyoung-oh-my-dsh](https://github.com/LaplaceYoung/oh-my-dsh) 50 — —
- `[不兼容]` [beancookie-awesome-dsh-plugin](https://github.com/beancookie/awesome-dsh-plugin) 85 — Awesome DeepSeek Harness (DSH) Plugin
- `[可用]` [dsh-cost-meter](https://github.com/Han-1413141/dsh-cost-meter) 88 — DeepSeek Harness 会话费用统计插件:本会话费用、当日费用、历史记录与官方价格同步
- `[不兼容]` [like-study1-oh-my-dsh](https://github.com/like-study1/Oh-My-DSH) 58 — —
- `[待定]` [dsh-multica-runtime](https://github.com/multica-ai/dsh-multica-runtime) 46 — Support dsh runtime on Multica.
- `[可用]` [noob-stupid-dsh-plugin-hub](https://github.com/Noob-stupid/dsh-plugin-hub) 49 — —
- `[不兼容]` [kuangre123-iosdev](https://github.com/kuangre123/iosdev) 24 — —
- `[不兼容]` [n0zom1z0-th08](https://github.com/N0zoM1z0/th08) 30 — —
- `[可用]` [dsh-plugin-template](https://github.com/bugmaker2/dsh-plugin-template) 4 — Template for deepseek-harness plugin development.
...
- `[可用]` [dsh-share](https://github.com/hellodigua/dsh-share) 23 — DSH 对话分享插件，分享单轮或多轮对话，可导出为图片或 Markdown
- `[可用]` [dsh-balance](https://github.com/crazywoola/dsh-balance) 20 — DeepSeek Harness balance plugin for the Settings page
...
- `[可用]` [blender](https://github.com/CheshireJCat/blender) 16 — DeepSeek Harness plugin for complete Blender 3D modeling, reconstruction, rendering, validation, and export wo
- `[可用]` [dqsjqian-aria](https://github.com/dqsjqian/Aria) 21 — —
...
- `[待定]` [fabric](https://github.com/omdsh-dev/fabric) 14 — 一种类似MC Fabric的hook处理器
- `[可用]` [biociao-dsh-science](https://github.com/biociao/dsh-science) 16 — —
- `[可用]` [dsh-status-rotator](https://github.com/01Virex/dsh-status-rotator) 21 — A DeepSeek Harness (dsh) web plugin that replaces the "Deep diving…" turn-status label with phase-aware, typew
...
- `[不兼容]` [billliao-awesome-dsh-plugin](https://github.com/billLiao/awesome-dsh-plugin) 11 — —
...
- `[可用]` [blockrunai-dsh-clawrouter](https://github.com/BlockRunAI/dsh-clawrouter) 13 — —
- `[待定]` [dsh-acp-for-bitfun](https://github.com/bobleer/dsh-acp-for-bitfun) 9 — BitFun 与 DSH ACP 交互对接 插件
- `[可用]` [ch4acko3-dsh-harmony](https://github.com/CH4ACKO3/dsh-harmony) 10 — —
...
- `[可用]` [dsh-agy](https://github.com/chaos-03x/dsh-agy) 15 — Google Antigravity (agy) OAuth auth + model access plugin for DeepSeek Harness: multi-account pool, 429 rotati
...
- `[可用]` [pi2dsh](https://github.com/weijiafu14/pi2dsh) 21 — Bridge the Pi and DeepSeek Harness ecosystems: one Pi Host ABI runs unmodified Pi extensions as native DSH plu
...
- `[可用]` [deepseek-harness-acp](https://github.com/openma-ai/deepseek-harness-acp) 9 — ACP server implementation for DeepSeek harness
- `[不兼容]` [thetianzz-dsh-billing](https://github.com/TheTianzz/dsh-billing) 7 — —
- `[未测]` [dsh-ramify](https://github.com/yanglongyun/dsh-ramify) 9 — Ramify 是 DeepSeek Harness 的创意分支画布插件，用树状工作区生成、对比和迭代多个可交互方案
...
- `[不兼容]` [dsh-plugin-aigc-canvas](https://github.com/HuanLinOTO/dsh-plugin-aigc-canvas) 11 — provider-agnostic AIGC HTTP 桥 + 无限画布 + ffmpeg 后处理，13 个工具含画布连边/reroll/媒体编辑 \| Provider-agnostic AIGC HTTP bridge
...
- `[待定]` [stevenx65-dsh-balance-plugin](https://github.com/stevenx65/dsh-balance-plugin) 7 — —
...
- `[可用]` [dsh-spotlight](https://github.com/0xsline/dsh-spotlight) 8 — Keyboard-first command palette for DeepSeek Harness Web
- `[不兼容]` [zotero-harvest](https://github.com/Fisfzy/zotero-harvest) 6 — Zotero 文献采集入库插件（DSH external plugin）：多源检索（OpenAlex/arXiv/Crossref/Europe PMC/Semantic Scholar）+ OA 下载链接解析（Unpa
...
- `[不兼容]` [Deepseek-Harness-](https://github.com/Links2008/Deepseek-Harness-) 22 — —
- `[待定]` [dsh-llm-fallbacks](https://github.com/omdsh-dev/dsh-llm-fallbacks) 8 — An dsh plugin for role-based LLM retry&fallback strategy
...
- `[不兼容]` [1514100951-dsh-usage-footer](https://github.com/1514100951/dsh-usage-footer) 4 — —
...
- `[不兼容]` [ceelog-dsh-plugins](https://github.com/Ceelog/dsh-plugins) 6 — —
...
- `[不兼容]` [dsh-mc-launcher](https://github.com/hellosky983/dsh-mc-launcher) 6 — Minecraft launcher built on DeepSeek Harness: full-screen launcher UI (root slot) with version download, Micro
...
- `[可用]` [dsh-restart](https://github.com/anweat/dsh-restart) 4 — —
...
- `[可用]` [jesse-njx-dsh-cowork](https://github.com/Jesse-njx/dsh-cowork) 4 — —
...
- `[待定]` [TokenLedger](https://github.com/zh667/TokenLedger) 92 — Token usage accounting for DeepSeek Harness, reconciled against New API and Sub2API relay-site billing
...
- `[可用]` [dsh-windows-readiness-proof](https://github.com/dongsheng123132/dsh-windows-readiness-proof) 2 — Content-addressed readiness proof for sanitized DeepSeek Harness observations on
...
- `[可用]` [adamplatin123-dsh-tonghuashun](https://github.com/AdamPlatin123/dsh-tonghuashun) 2 — —
...
- `[不兼容]` [dsh-vision-bridge](https://github.com/GXX182/dsh-vision-bridge) 3 — DeepSeek Harness plugin that bridges session images to pluggable vision APIs whi
...
- `[未测]` [dsh-tool-chaos](https://github.com/cyanseek/dsh-tool-chaos) 3 — —
...
- `[待定]` [dsh-experience-plugin](https://github.com/SouleyMoni1/dsh-experience-plugin) 0 — —
...
- `[不兼容]` [dsh-project-wiki](https://github.com/yu-xin-c/dsh-project-wiki) 2 — —
...
- `[未测]` [dsh-vision-no-vision](https://github.com/wdwind/dsh-vision-no-vision) 1 — DeepSeek Harness native vision plugin!

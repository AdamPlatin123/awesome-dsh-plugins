...
> 数据源：radar 快照并集（`20260826T231501Z` 2026-08-27 07:15 UTC+8 ⊕ `20260826T193001Z` 2026-08-27 03:30 UTC+8 ⊕ `20260826T184501Z` 2026-08-27 02:45 UTC+8 等 85 轮）⊕ GitHub 定位复核缓存（data/locate-cache.json）。
> 呈现：分组列表（状态 · 名称  · 一句话说明），不使用大表格。

## 统一度量衡

**判定维度**（运行级四档，仅已定位条目 2261 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1148）/ `[不兼容]`（637）/ `[待定]`（248）/ `[未测]`（228）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（97）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- `[未定位]`（267）— 新占位条目，待下一轮定位复核（scripts/resolve_placeholders.py）
- 定位复核累计修复 363 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2729 条（已定位 2261 · 监测/未定位 468）· PR 登记 23 个
...
- `[未测]` [dsh-desk-pet](https://github.com/anneheartrecord/dsh-desk-pet) — macOS 桌宠：真的 AppKit 置顶窗口而不是页面里的挂件，全屏 Space 也盖得住；六种状态跟着本地 DSH 走（空闲／干活／等你确认／报错／刚跑完／打盹），原生右键菜单含免打扰与会话清单；自带的 skill 用你自己的画图工具和额度把一张照片扩成整套十八个姿势的皮肤，装在包外，升级不会删。跑系统自带的 Python 与 ctypes，零运行时依赖，不用 Electron。**仅 macOS**
- `[未测]` [Aegis](https://github.com/ganyuanran/aegis) — 软件工程方法论技能包：baseline-first 规划、系统性重构等技能（对方清单 1089★，rc.8 实测 ✅）
- `[未测]` [superdesign-skill](https://github.com/superdesigndev/superdesign-skill) — UI 与营销图形设计技能（Superdesign 画布，读仓取上下文，436★，rc.8 实测 ✅）
...
- `[未测]` [dsh-simple-wiki-memory](https://github.com/rainow/dsh-simple-wiki-memory) — 简易 Wiki 持久记忆（DSWM）：一个索引文档自动注入 + 每主题一个 md 文件按需读取，省 token 轻量无压力；pending 准入→确认晋升→archive 归档三区 + memory-log 审计 + git 自动备份；纯 Markdown 所见即所得，跨 harness 共享
...
- `[未测]` [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard) — 两阶段 DSH 预设：极简模式对齐启动 → 全量装载（Top50 Booster 成员，registry 插队实测）
...
- `[未测]` [dsh-capcheck](https://github.com/heming-gmh/dsh-capcheck) — 装插件之前先看看它实际能碰到什么：静态分析 inject 声明、ctx 成员访问和依赖声明，对照能力分级表出报告，扫过的插件里 dsh-ssh 的远程命令执行绕过了官方审批沙箱、dshmarket 拿到了能启停其它插件的 Loader 控制权；优先扫真实发布的 npm tarball，因为很多插件的构建产物没提交到 GitHub 仓库
...
- `[不兼容]` [d-dev0101-open-sea-skin](https://github.com/d-dev0101/open-sea-skin) 194 — WebGPU ocean skin for DeepSeek Harness — DSH plugin, Harness-only Chrome/Edge extension, static installer, and
...
- `[不兼容]` [jsdvjx-dsh-strata](https://github.com/jsdvjx/dsh-strata) 30 — Session strata for the DeepSeek Harness Web GUI: the transcript's scrollbar becomes a to-scale, colour-coded m
...
- `[可用]` [amlyczz-dsh-agy-link](https://github.com/amlyczz/dsh-agy-link) 23 — Google Antigravity (agy CLI) models for DeepSeek Harness — streaming chat, thinking, tool activity, usage, in-
...
- `[可用]` [naodeng-dsh-qa](https://github.com/naodeng/dsh-qa) 5 — dsh-qa · QA Workbench — A local software testing workbench plugin for DeepSeek Harness
...
- `[可用]` [zoumutou-dsh-web-preview](https://github.com/zoumutou/dsh-web-preview) 3 — DeepSeek Harness 侧边网页预览面板：本地静态托管、项目运行、元素标记批注、链接点击接管（Cordis 插件包）
...
- `[未定位]` **aihesa-ui-conversation-indexed** — 占位待复核，判定暂不展示
- `[未定位]` **alexyin-tongji-dsh-ui-enhancer** — 占位待复核，判定暂不展示
- `[可用]` [dsh-plugin-ui-turnav](https://github.com/AuraxM/dsh-plugin-ui-turnav) 0 — —
- `[未定位]` **benhuhuan-dhs-tuicode** — 占位待复核，判定暂不展示
...
- `[未定位]` **changguo1998-dsh-turtle-ui-prompt-extra** — 占位待复核，判定暂不展示
...
- `[未定位]` **dongdong1997-dsh-mcskin** — 占位待复核，判定暂不展示
...
- `[未定位]` **easytz-dsh-tui** — 占位待复核，判定暂不展示
...
- `[未定位]` **louishaol-dsh-timer-agent** — 占位待复核，判定暂不展示
...
- `[未定位]` **max-null-dsh-sidebar-preview-select** — 占位待复核，判定暂不展示
...
- `[未定位]` **mrelysium-dsh-md-quiz** — 占位待复核，判定暂不展示
...
- `[未定位]` **odefined-dsh-mobile-webui** — 占位待复核，判定暂不展示
- `[可用]` [penguinpanda-sebastian-kitchen-board](https://github.com/penguinpanda/sebastian-kitchen-board) 0 — —
- `[未定位]` **perederey-dsh-mobile-ui** — 占位待复核，判定暂不展示
- `[未定位]` **pixelgeek325-dsh-high-contrast-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **rainpomelo-deepseek-harness-liquid-glass-them** — 占位待复核，判定暂不展示
...
- `[未定位]` **ryzezhou-dsh-ui-mobile-layout** — 占位待复核，判定暂不展示
...
- `[未定位]` **shadowbrucemeaninglau-dsh-rss-for-shadow-suit** — 占位待复核，判定暂不展示
...
- `[未定位]` **sixtysevenlf-dsh-routing-suite-opencode-go** — 占位待复核，判定暂不展示
...
- `[未定位]` **starryhui-dsh-custom-background** — 占位待复核，判定暂不展示
...
- `[未定位]` **wuwl-98-dsh-theme-paper** — 占位待复核，判定暂不展示
- `[未定位]` **xiaoloveying-deepseek-harness-theme** — 占位待复核，判定暂不展示
...
- `[未定位]` **yuanqiyibiansheng-deepseek-harness-gui** — 占位待复核，判定暂不展示
...
- `[未定位]` **yugitan-dsh-skin** — 占位待复核，判定暂不展示
...
- `[未定位]` **ngk3pori-dsh-zh-cn-ui** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-session-tabs](https://github.com/licat2023/dsh-session-tabs) — 浏览器式会话标签页：顶部标签条从侧边栏右缘开始（不挤占侧边栏），点击切换/关闭/新建、中键关标签与侧边栏后台打开、鼠标侧键会话历史导航、运行状态点；纯客户端零网络零持久化
- `[未测]` [dsh-tokstat](https://github.com/kongjianguan/dsh-tokstat) — DSH 使用量与性能统计：设置页「统计」面板（概览/模型/会话/请求，TTFT/TPS/Tokens/成本）+ Python TUI；Node half + client web bundle
...
- `[未测]` [dsh-mcp-list](https://github.com/dmz2922990/dsh-mcp-list) — 设置面板新增 MCP 页：按服务器分组列出全部已挂载 MCP 服务器与工具，扫描最近会话统计各工具调用次数，一键在系统编辑器打开 cordis.patch.yml 的 MCP 配置
...
- `[未测]` [dsh-easyrewrite](https://github.com/renzic-stone/dsh-easyrewrite) — DSH Web 用户消息气泡内联编辑与撤回插件：惰性提交、无痕替换、版本翻页器、草稿自动备份、三语 i18n（中/英/日）；npm `dsh-easyrewrite`（dsh.bundle.patch → cordis.patch.yml，`dsh plugin add` 一键安装）
...
- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 91757 — 🎨 Best DeepSeek Harness Design Plugin
...
- `[不兼容]` [whitelonng-dshcode](https://github.com/whitelonng/dshcode) 393 — Community desktop companion for DeepSeek Harness — one-click Electron app for macOS and Windows
...
- `[可用]` [modsearch](https://github.com/liustack/modsearch) 107 — 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without nat
...
- `[不兼容]` [dundunhan-dsh-video-lens](https://github.com/dundunhan/dsh-video-lens) 49 — —
...
- `[待定]` [ChatCCC](https://github.com/wzj998/ChatCCC) 17 — 飞书（Lark）或微信（WeChat）聊天控制 DeepSeek Harness / Claude Code / Cursor / Codex / CCC Agent
...
- `[不兼容]` [dsh-openai-codex-auth](https://github.com/yoke233/dsh-openai-codex-auth) 10 — OpenAI Codex OAuth login and usage card plugin for DeepSeek Harness
...
- `[待定]` [dsh-codex](https://github.com/Yan-Zero/dsh-codex) 6 — Use your ChatGPT subscription in DeepSeek Harness through OpenAI's Codex sign-in
...
- `[待定]` [dsh-aux-vision](https://github.com/XyTT2N2bTc/dsh-aux-vision) 4 — DeepSeek Harness 辅助视觉插件：任意纯文本主模型 + 任意视觉模型（默认 opencode-go/mimo-v2.5）
...
- `[不兼容]` [dsh-zen-proxy](https://github.com/Yee-h/dsh-zen-proxy) 2 — dsh plugin: in-process proxy that injects official OpenCode Zen client headers,
...
- `[未定位]` **angel-gp-dsh-gitpeek** — 占位待复核，判定暂不展示
...
- `[未定位]` **chuankris-dsh-video-factory** — 占位待复核，判定暂不展示
...
- `[未定位]` **debug-lzy-restart-service-button** — 占位待复核，判定暂不展示
...
- `[未定位]` **eve-146t-dsh-codex-subscription-pool** — 占位待复核，判定暂不展示
...
- `[未定位]` **g1en-114-dsh-codex-import** — 占位待复核，判定暂不展示
...
- `[未定位]` **gutianshuo-powershell-fix** — 占位待复核，判定暂不展示
...
- `[未定位]` **kyincode-dsh-hooks-plugin** — 占位待复核，判定暂不展示
...
- `[未定位]` **lzh3070-dsh-file-review-tab** — 占位待复核，判定暂不展示
...
- `[未定位]` **sjkhx-yukinodigital** — 占位待复核，判定暂不展示
...
- `[未定位]` **wanbinyu-dsh-provider-probe** — 占位待复核，判定暂不展示
...
- `[未定位]` **xincodes-deepseek-billing-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **xyz1024-alt-dsh-side-panel** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-test-normalizer](https://github.com/mkaliezz/dsh-test-normalizer) — pytest / Vitest / Jest / Cargo 测试结果归一化为稳定结构；npm 包 @mkaliezz/dsh-test-runner
...
- `[未测]` [dsh-agent-team-gui](https://github.com/toolclub/dsh-agent-team-gui) — 持久化多模型 Agent 小队：主 Agent 按成员特性动态编排有界 DAG，成员独立配置模型与工具策略；在 Settings/Composer 管理和选择小队，Run Center 支持追踪、重试与取消，并提供逐 Agent Token 成本洞察；v0.5.0 通过 117 Host + 62 Client 测试及 DSH rc.6 全新 Web profile 安装/浏览器冒烟
...
- `[未测]` [dsh-plugins](https://github.com/mkaliezz/dsh-plugins) — DSH 插件家族索引：16 个插件（fail-closed 授权安全线 / 策略测试与诊断 / 工作区生产力 / 运行时基础设施），统一 dsh-plugin topic
...
- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 17592 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
...
- `[不兼容]` [dsh-prompt-persona](https://github.com/Xilin3/dsh-prompt-persona) 4 — DSH plugin: edit the system prompt (deployment persona) from the Settings page, with live preview.
...
- `[待定]` [dsh-session-timeline](https://github.com/XiLuovo/dsh-session-timeline) 3 — DeepSeek Harness 会话时间轴插件：横短横线波浪、当前消息定位、点击跳转、圆角预览 tooltip、可收起/展开
...
- `[不兼容]` [dsh-dynamic-island](https://github.com/YLifeOnlyOnce/dsh-dynamic-island) 3 — A tiny glass companion for DeepSeek Harness — it breathes while the agent thinks, pulses while it works, and p
- `[不兼容]` [dsh-smarthome](https://github.com/YLifeOnlyOnce/dsh-smarthome) 3 — Home Assistant control for DeepSeek Harness agents — approval-gated lights, switches, climate
...
- `[未定位]` **dm-odyssey-dsh-session-manager** — 占位待复核，判定暂不展示
...
- `[未定位]` **jasperguwp-dsh-project-memory** — 占位待复核，判定暂不展示
...
- `[未定位]` **leshm0321-dsh-plugin-local-agent-bridge** — 占位待复核，判定暂不展示
...
- `[未定位]` **memoryit-dsh-trail-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **mryuanliu-dsh-prompt-presets** — 占位待复核，判定暂不展示
...
- `[未定位]` **mvyvn-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **nb4747-dsh-session-sync** — 占位待复核，判定暂不展示
...
- `[未定位]` **nexusclawhq-nexusclaw-agent-governance** — 占位待复核，判定暂不展示
...
- `[未定位]` **oolleyw-dsh-skill-manager** — 占位待复核，判定暂不展示
- `[未定位]` **player-yn-dsh-agent-driver-writehere** — 占位待复核，判定暂不展示
- `[未定位]` **qianling-0831-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **rainpomelo-deepseek-harness-mimo-memory** — 占位待复核，判定暂不展示
- `[未定位]` **relethe-dsh-brief-session-title** — 占位待复核，判定暂不展示
...
- `[未定位]` **sivan757-dsh-agent-plugin-market** — 占位待复核，判定暂不展示
...
- `[未定位]` **snow-the-dsh-ark-plan** — 占位待复核，判定暂不展示
- `[未定位]` **snow-the-dsh-session-handoff** — 占位待复核，判定暂不展示
...
- `[未定位]` **wavespeedai-wavespeed-dsh-skill** — 占位待复核，判定暂不展示
- `[未定位]` **wccong1213-dsh-auto-memory** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-novera-ai-agent** — 占位待复核，判定暂不展示
- `[未定位]` **whaojie797-design-novera-ai-skills** — 占位待复核，判定暂不展示
...
- `[未定位]` **y1x1n-dsh-prompt-optimizer** — 占位待复核，判定暂不展示
- `[未定位]` **yogioo-dsh-agentmemory** — 占位待复核，判定暂不展示
- `[未定位]` **yuntaihua-illusion-agent** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-ssh](https://github.com/dmz2922990/dsh-ssh) — SSH 主机管理 + 远程执行：ssh_host_list/add/update/remove/ssh_bash 五工具，主机台账持久化到 ~/.dsh/ssh-hosts.json，支持 agent/key/password 三种认证（password 走 SSH_ASKPASS，沙箱可用）；提供 Agent 预设与 Cordis 插件两种形态
- `[未测]` [dsh-daily-kit](https://github.com/zhouwei713/dsh-daily-kit) — 日常插件集合 monorepo：16 个插件（审批门/桌面与Webhook通知/成本计量/会话导出/Ollama 本地模型/上下文水位/办公文档解析/本地文件夹索引/cron 全局调度/RSS与网页内容监控/日历/Gmail/待办/天气地点/票据识别/音视频转录）+ 4 个一键 bundle（dev-buddy/evidence-wall/inbox-zero/daily-briefing）；权限透明、读取优先、零 native 依赖，596 单测，dsh rc.6/rc.7 真实加载与 npm 安装实测通过
- `[未测]` [dsh-change-budget](https://github.com/raphaelutumn/dsh-change-budget) — 为每个 DSH Agent 回合设置文件数、修改调用数与 UTF-8 载荷额度；在受支持的文件修改工具执行前阻止超额修改，并对并行调用同步预留额度
...
- `[可用]` [DSH-telegram](https://github.com/yuko0331/DSH-telegram) 5 — 通过 Telegram 私聊远程使用和查看 DeepSeek Harness
...
- `[未定位]` **clarklevis1995-dsh-plugin-mobile-gateway** — 占位待复核，判定暂不展示
- `[未定位]` **clarknu-dsh-tray** — 占位待复核，判定暂不展示
...
- `[未定位]` **jackcontrols-dsh-telegram-control** — 占位待复核，判定暂不展示
...
- `[未定位]` **max-null-dsh-chat-rail** — 占位待复核，判定暂不展示
...
- `[未定位]` **nkhszkqaq-dsh-notify** — 占位待复核，判定暂不展示
...
- `[未定位]` **potterlee369-prog-dsh-feishu** — 占位待复核，判定暂不展示
- `[未定位]` **ricketts-guo-dsh-off-peak-message** — 占位待复核，判定暂不展示
...
- `[未定位]` **wyxbupt-22-dsh-plugin-edit-message** — 占位待复核，判定暂不展示
- `[不兼容]` [dsh-plugin-qqbot](https://github.com/Yunqingqingxi/dsh-plugin-qqbot) 0 — QQ Bot transport adapter for DeepSeek Harness: drive agents from QQ chat, stream replies back.
...
- `[未定位]` **lessxi-paper-workbench** — 占位待复核，判定暂不展示
...
- `[未定位]` **zenjibad-dsh-drop-any-file** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-open-file](https://github.com/hyp6666/dsh-open-file) — DSH Web 工作区文件附件插件：多文件选择与全页拖放，读取文本、PDF、DOCX、PPTX、XLSX、ZIP 和图片，提供英中 OCR、页面渲染及 `file_inspect` / `file_read` / `file_ocr` / `file_render` 四个可追溯工具；npm `dsh-open-file` 0.1.1
- `[未测]` [dsh-mmx-bridge](https://github.com/welsione/dsh-mmx-bridge) — MiniMax 多模态桥接插件：一个 mmx_bridge 工具覆盖图像理解（VLM）/文生图/文图生视频/语音合成/音乐生成/音频翻唱/联网搜索/用量查询，生成产物经 /mmx-files/ 同源内嵌图片预览与音视频播放器，附 Web 设置页管理卡片；零 npm 运行时依赖
- `[未测]` [dsh-univer-plugin](https://github.com/dream-num/dsh-univer-plugin) — Univer 办公插件：在 DSH 中创建/编辑/检查/交付表格、文档、演示文稿、多维表格和画布，支持 xlsx/docx/pptx 导入导出、实时预览与 worktree 审阅；npm `dsh-univer-office` 0.2.5
- `[未测]` [dsh-socrates](https://github.com/cruciforms/dsh-socrates) — 苏格拉底式澄清优先的深度研究插件：自适应多轮研究、交叉验证（声明级三分裁决+单跳回补）、程序化引用审计、学术三源（arXiv/PubMed/S2）、模型分层降本（Pro/Flash）
...
- `[未测]` [dsh-fund-research](https://github.com/perrylink/dsh-fund-research) — 中国公募基金确定性研究：天天基金/东方财富公开数据采集（pingzhongdata JS 块、F10 持仓与经理页、个股估值 + push2delay 兜底主机），业绩拆解/持仓穿透/风格归因/经理画像纯函数计算，版本化 Markdown 报告附每个关键数字可回溯到 sha256 源快照的附录；npm dsh-fund-research 已发布；仅供研究，不构成投资建议
...
- `[不兼容]` [xiaoshihou514-dsh-desktop-pet](https://github.com/xiaoshihou514/dsh-desktop-pet) 27 — DeepSeek Harness：鲸鱼娘桌宠！
...
- `[可用]` [dsh-blue-whale-maid](https://github.com/yuxino/dsh-blue-whale-maid) 5 — DeepSeek Harness 桌宠插件
...
- `[未定位]` **ho11ow8-deepseek-harness-balance-pet** — 占位待复核，判定暂不展示
...
- `[未定位]` **xiaoloveying-deepseek-harness-cat-pet** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-trading-toolkit](https://github.com/kentleenot/dsh-trading-toolkit) — A股+美股行情工具箱：实时行情/OHLCV K线/ADX 三状态信号/回测预览，东方财富免 Key 直连，只读永不下单
...
- `[不兼容]` [dshplugin-dsh-plugin-hub](https://github.com/dshplugin/dsh-plugin-hub) 52 — DeepSeek Harness 社区内置插件市场（dsh-plugin）— 搜索插件、下载并安装 4000+ 人工精选社区插件，每日更新、完全免费
...
- `[可用]` [173787247-dsh-wsl-net](https://github.com/173787247/dsh-wsl-net) 0 — DeepSeek Harness Tool: Diagnose WSL/Windows Proxy, Node 24 fetch, and DeepSeek/npm reachability.
...
- `[未定位]` **hgxy2004-dsh-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **jiaoxicai2013-deepseek-harness-desktop-electr** — 占位待复核，判定暂不展示
...
- `[未定位]` **neevcloudai-dsh-neev-sandbox** — 占位待复核，判定暂不展示
- `[未定位]` **nobullymethanks-dsh-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **normanfxxkingrockwell-harmonyos-dev-mcp-for-d** — 占位待复核，判定暂不展示
- `[空仓监测]` **qinlinglong-dsh-desktop** — GitHub 无此仓库，判定暂不展示
- `[未定位]` **scottliu2512-dsh-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **shilunka-deepseek-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **star-dreamy-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[空仓监测]` **sunhong5542-deepseek-harness-desktop** — GitHub 无此仓库，判定暂不展示
- `[未定位]` **swordywq-deepseek-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **wzl0813-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **xccelephant-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **xcyh26-deepseek-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **xpdd-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **xytom-coding-tools-mcp** — 占位待复核，判定暂不展示
- `[不兼容]` [yangloop-deepseek-harness-desktop](https://github.com/yangloop/deepseek-harness-desktop) 0 — —
- `[未定位]` **yhdyyds-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **yliuling-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[不兼容]` [dsh-ssh-plugin](https://github.com/YOLO-LZL/dsh-ssh-plugin) 0 — —
- `[未定位]` **yu-bufan-k-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **yuanimal-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[可用]` [yuanzhoucanxiang-dsh-desktop](https://github.com/yuanzhoucanxiang/dsh-desktop) 0 — DeepSeek Harness Desktop shell
- `[未定位]` **yuel25-dsh-desktop** — 占位待复核，判定暂不展示
- `[不兼容]` [yxx-jf-deepseek-harness-desktop](https://github.com/yxx-jf/deepseek-harness-desktop) 0 — —
- `[未定位]` **zenjibad-dsh-mcp-toggle** — 占位待复核，判定暂不展示
...
- `[未测]` [api-relay-audit](https://github.com/toby-bridges/api-relay-audit) — 从 DSH 发起 AI API 中继/LLM 代理的本地安全审计，产出 Markdown 报告（797★，rc.8 实测 ✅）
...
- `[未定位]` **adydc-99-dsh-wallpaper** — 占位待复核，判定暂不展示
...
- `[未定位]` **yangl326-dylan-learning-dsh** — 占位待复核，判定暂不展示
- `[未定位]` **yourtsao-offical-paper-wrighting-chinese-skil** — 占位待复核，判定暂不展示
- `[歧义监测]` **dsh-101** — 同名多仓，判定暂不展示
- `[歧义监测]` **dsh-wallpaper** — 同名多仓，判定暂不展示
- `[未测]` [dsh-research-report](https://github.com/perrylink/dsh-research-report) — 证据账本与可验证报告引擎：内容寻址的声明-快照绑定与逐字节核查，产出带逐条判定与 SHA-256 清单封存的版本化报告；npm 0.1.0 已发布
- `[未测]` [dsh-industry-research](https://github.com/perrylink/dsh-industry-research) — 行业与公司研究领域包：产业链结构模型（industry_map）、公开来源政策/新闻追踪（industry_track）、公司扫描卡片（company_scan）与可审计研究报告（industry_report，可选 ctx.researchReport 密封桥 + 内置 Markdown 兜底）；全部检索走官方 ctx.web，指标带来源引用，仅供研究不构成投资建议
...
- `[不兼容]` [TokenTracker](https://github.com/xiufengsun/TokenTracker) 1312 — Local-first AI token usage & cost tracker for 31 coding tools incl
...
- `[不兼容]` [tong-io-tongflow](https://github.com/tong-io/tongflow) 938 — TongFlow — Multimodal GenAI Studio
...
- `[不兼容]` [dsheval-dsh-top100](https://github.com/dsheval/dsh-top100) 68 — —
...
- `[待定]` [xuedai-mayi-dsh-wenshan](https://github.com/xuedai-mayi/dsh-wenshan) 33 — 问山：DeepSeek Harness 地理野外实习知识图谱插件与非官方二次开发项目
...
- `[待定]` [dsh-portable-tavern](https://github.com/XCNXNXNX/dsh-portable-tavern) 5 — DeepSeek Harness 的「便携酒馆」插件：RPG 式 SillyTavern V2/V3 角色卡生成器 + 酒馆角色扮演聊天
...
- `[不兼容]` [kaixinbaba-dsh-model-preset](https://github.com/kaixinbaba/dsh-model-preset) 1 — DeepSeek Harness model preset shortcuts with Settings customization
...
- `[可用]` [dsh-plugin-quote-reply](https://github.com/yangYzc/dsh-plugin-quote-reply) 1 — DSH plugin: select text in a conversation, then quote it into the composer or reply in a new window
...
- `[未定位]` **arcticus17-dsh-toolkit** — 占位待复核，判定暂不展示
- `[未定位]` **armywas-dsh-failure-lens** — 占位待复核，判定暂不展示
...
- `[未定位]` **bil812-dsh-balance-tasks** — 占位待复核，判定暂不展示
- `[未定位]` **billychen123-qdd** — 占位待复核，判定暂不展示
...
- `[未定位]` **boltblue121-kimi-for-dsh** — 占位待复核，判定暂不展示
...
- `[未定位]` **cuteg41cute-dsh-api** — 占位待复核，判定暂不展示
- `[未定位]` **daseanle-dsh-obsidian-bridge** — 占位待复核，判定暂不展示
- `[未定位]` **daseanle-dsh-teacher-preset** — 占位待复核，判定暂不展示
...
- `[未定位]` **dinosaur-mc-dsh-win-multi-bash** — 占位待复核，判定暂不展示
...
- `[未定位]` **duyanta123-arch-doc** — 占位待复核，判定暂不展示
...
- `[未定位]` **g1en-114-dsh-failbook** — 占位待复核，判定暂不展示
...
- `[未定位]` **gyanano-dsh-plugin-wayfinder-dashboard** — 占位待复核，判定暂不展示
- `[未定位]` **gyanano-wayfinder-dashboard** — 占位待复核，判定暂不展示
- `[未定位]` **hann428-dsh-usage-dashboard** — 占位待复核，判定暂不展示
- `[未定位]` **haodongzhang022-hue-openliulan** — 占位待复核，判定暂不展示
- `[未定位]` **harzva-dsh-maclens** — 占位待复核，判定暂不展示
...
- `[未定位]` **horusjiang-dsh-map-tools** — 占位待复核，判定暂不展示
- `[未定位]` **hudson-junior-wang-dsh-search-switcher** — 占位待复核，判定暂不展示
- `[不兼容]` [deep-flow](https://github.com/hunterxxn/deep-flow) 0 — deepseek-harness tui
- `[未定位]` **icebear662-dsh-vision-edge-doubao** — 占位待复核，判定暂不展示
- `[不兼容]` [dsh-zenmux-oauth](https://github.com/ilimei/dsh-zenmux-oauth) 0 — ZenMux OAuth 2.0 PKCE plugin for DeepSeek Harness
- `[未定位]` **imchenmin-dsh-auth-lock** — 占位待复核，判定暂不展示
...
- `[未定位]` **jasperguwp-dsh-plugin-schemes** — 占位待复核，判定暂不展示
- `[不兼容]` [jbwashington-dsh-voice](https://github.com/jbwashington/dsh-voice) 0 — DSH plugin: voice notes in, spoken answers out
- `[未定位]` **johnoooooo-dsh-view-image** — 占位待复核，判定暂不展示
- `[未定位]` **justinhuangai-one-harness** — 占位待复核，判定暂不展示
- `[未定位]` **kaieye-dsh-air** — 占位待复核，判定暂不展示
- `[可用]` [dsh-plugin-medhub](https://github.com/KevinShao1991/dsh-plugin-medhub) 0 — —
- `[未定位]` **kongchengavg-dsh-statuslight** — 占位待复核，判定暂不展示
- `[未定位]` **kotdath-dsh-voice** — 占位待复核，判定暂不展示
- `[待定]` [l541402398-dsh-plugin](https://github.com/l541402398/dsh-plugin) 0 — —
- `[未定位]` **lee-si-yoon-dsh-llm-friendli** — 占位待复核，判定暂不展示
...
- `[未定位]` **leonx01-peak-valley-alarm** — 占位待复核，判定暂不展示
- `[未定位]` **leonx01-peak-valley-ticker** — 占位待复核，判定暂不展示
- `[未定位]` **leyan0365-dsh-retro-mac** — 占位待复核，判定暂不展示
- `[可用]` [lidongjmx-dsh-plugins](https://github.com/lidongJMX/DSH-plugins) 0 — —
- `[未定位]` **likenttt-garmin-connect-plugin-for-dsh** — 占位待复核，判定暂不展示
...
- `[未定位]` **lin-a1-dsh-hub** — 占位待复核，判定暂不展示
- `[未定位]` **littleblacktong-dsh-plugin-heartbeat** — 占位待复核，判定暂不展示
- `[未定位]` **littledogwang-dsh-agora** — 占位待复核，判定暂不展示
- `[未定位]` **lizhennet-dsh-antigravity** — 占位待复核，判定暂不展示
- `[未定位]` **lm20230311-awesome-dsh-plugin-cn** — 占位待复核，判定暂不展示
- `[未定位]` **lm20230311-dsh-billing-cn** — 占位待复核，判定暂不展示
- `[未定位]` **loping151-dsh-vibeos** — 占位待复核，判定暂不展示
- `[可用]` [dsh-plugin-warroom-garak](https://github.com/lukethecat/dsh-plugin-warroom-garak) 0 — —
- `[未定位]` **makoveli89-dsh-swarm** — 占位待复核，判定暂不展示
- `[未定位]` **max-null-dsh-node-appearance** — 占位待复核，判定暂不展示
...
- `[未定位]` **mcxcc303-dsh-dslow** — 占位待复核，判定暂不展示
- `[未定位]` **mdr-ex1000-dsh-rw** — 占位待复核，判定暂不展示
- `[不兼容]` [menantonio83-hue-tnt-house](https://github.com/menantonio83-hue/tnt-house) 0 — TNT House - Safe New Tokens Platform
- `[未定位]` **mfwtw-auto-load-our-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **mindscale-noah-mindmemos** — 占位待复核，判定暂不展示
- `[未定位]` **minokun-dsh-quota** — 占位待复核，判定暂不展示
...
- `[未定位]` **myg133-anspire-ai-search** — 占位待复核，判定暂不展示
- `[未定位]` **n5tar-dsh-workspace-view** — 占位待复核，判定暂不展示
...
- `[未定位]` **neomei-dsh-roundtable** — 占位待复核，判定暂不展示
...
- `[未定位]` **palaiologos1453-dsh-worktree-studio** — 占位待复核，判定暂不展示
...
- `[未定位]` **potterlee369-prog-dsh-sound-notifier** — 占位待复核，判定暂不展示
- `[未定位]` **potterlee369-prog-dsh-workspace-pin** — 占位待复核，判定暂不展示
...
- `[未定位]` **ratevoid-dsh-aseprite** — 占位待复核，判定暂不展示
- `[未定位]` **raybbish-dsh-poker-plugin** — 占位待复核，判定暂不展示
- `[空仓监测]` **dsh-vision-tool** — GitHub 无此仓库，判定暂不展示
- `[未定位]` **renji004-dsh-omni-vision** — 占位待复核，判定暂不展示
- `[未定位]` **renzic-stone-dsh-easyrewrite** — 占位待复核，判定暂不展示
- `[未定位]` **roooooyczx-dsh-peakseek** — 占位待复核，判定暂不展示
...
- `[未定位]` **sd3247930-skydome** — 占位待复核，判定暂不展示
- `[未定位]` **sensenmeng-dsh-plugin-8r** — 占位待复核，判定暂不展示
- `[未定位]` **shenpingdd-wooden-fish-dsh** — 占位待复核，判定暂不展示
- `[可用]` [DSHelperPlugin](https://github.com/snak10/DSHelperPlugin) 0 — —
- `[未定位]` **sparkelf-dsh-plugins-plus** — 占位待复核，判定暂不展示
- `[空仓监测]` **spyqwer1-dsh-imagecraft** — GitHub 无此仓库，判定暂不展示
- `[未定位]` **sunqingyuan0-kabutack** — 占位待复核，判定暂不展示
...
- `[不兼容]` [dsh-vision-no-vision](https://github.com/wdwind/dsh-vision-no-vision) 0 — DeepSeek Harness native vision plugin!
- `[未定位]` **whaojie797-design-novera-ai-pipeline** — 占位待复核，判定暂不展示
...
- `[未定位]` **windsland52-dsh-auto-retry** — 占位待复核，判定暂不展示
- `[未定位]` **wiziscool-dsh-ears** — 占位待复核，判定暂不展示
- `[未定位]` **woyeshishen-dsh-restart-backend** — 占位待复核，判定暂不展示
- `[未定位]` **wudifive-dsh-plugin-vision** — 占位待复核，判定暂不展示
- `[未定位]` **xavier711-dsh-deepseek-usage** — 占位待复核，判定暂不展示
- `[未定位]` **xdylanlong-dsh-northstar** — 占位待复核，判定暂不展示
- `[未定位]` **xdzzzzzzyq-dsh-survival** — 占位待复核，判定暂不展示
- `[未定位]` **xgrin-dsh-search-router** — 占位待复核，判定暂不展示
- `[未定位]` **xiangliuy-dsh-web-search-bing-rss** — 占位待复核，判定暂不展示
- `[未定位]` **xiaohai-78-top** — 占位待复核，判定暂不展示
- `[未定位]` **xiaoheizi1212-dsh-cookie-bridge** — 占位待复核，判定暂不展示
- `[未定位]` **xiayuhkust-dsh-pace-popups** — 占位待复核，判定暂不展示
- `[未定位]` **xieweikang123-dsh-vision-bridge** — 占位待复核，判定暂不展示
- `[未定位]` **xin-zhang-iceman-dsh-vision-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **xjwwjx-dsh-conversation-quote** — 占位待复核，判定暂不展示
...
- `[未定位]` **xsuas-dsh-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **xuanyvne-dshlauncher** — 占位待复核，判定暂不展示
- `[待定]` [xyxi-u0-dsh-plugin](https://github.com/xyxi-u0/dsh-plugin) 0 — —
- `[未定位]` **y2zyyr-dsh-restart-button** — 占位待复核，判定暂不展示
- `[待定]` [yacuo-dsh-plugin](https://github.com/yacuo/dsh-plugin) 0 — 独立的 DeepSeek Harness 社区插件目录
- `[未定位]` **yancey2023-dsh-fold** — 占位待复核，判定暂不展示
- `[未定位]` **yaoay-dsh-conversation-accents** — 占位待复核，判定暂不展示
- `[未定位]` **yaoay-dsh-gpt-compat** — 占位待复核，判定暂不展示
- `[未定位]` **ychris12138-dsh-usage-stats** — 占位待复核，判定暂不展示
- `[未定位]` **yejiming-museai** — 占位待复核，判定暂不展示
- `[未定位]` **yihong89-dsh-usage-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **yinfengwindy-dsh-plugin-shiori-role** — 占位待复核，判定暂不展示
- `[未定位]` **yjsoooooo-dsh-chrome** — 占位待复核，判定暂不展示
- `[不兼容]` [token-statistics](https://github.com/YlLVTECH/token-statistics) 0 — a token statistics plugin for dsh (deepseek-harness)
...
- `[未定位]` **yukunr-dsh-ezprot-plugin** — 占位待复核，判定暂不展示
- `[未定位]` **yuukilike-zeromd** — 占位待复核，判定暂不展示
- `[未定位]` **ywleeo-dsh-balance** — 占位待复核，判定暂不展示
- `[未定位]` **yytbit-awesome-dsh-bridges** — 占位待复核，判定暂不展示
- `[未定位]` **zanderkong-look-here** — 占位待复核，判定暂不展示
- `[未定位]` **zelinw1-dsh-cosplay** — 占位待复核，判定暂不展示
- `[未定位]` **zenjibad-dsh-plugin-toggle** — 占位待复核，判定暂不展示
...
- `[未定位]` **hongsantu-dsh-personalization** — 占位待复核，判定暂不展示
...
- `[未定位]` **zhangjianyu1006156-dsh-balance-display** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-evidence-task-board](https://github.com/mkaliezz/dsh-evidence-task-board) — 持久化确定性任务状态原语（创建/状态/证据转移）；npm 包 @mkaliezz/dsh-task-board
...
- `[未测]` [dsh-thinkmeter](https://github.com/dmz2922990/dsh-thinkmeter) — Think 思考行 Token 计量表：把聊天视图流式 Think 预览替换为实时 token 数显示（Thinking ≈ N tokens），落定时精确取 usage.reasoningTokens、缺失时启发式估算，点击可展开/收起完整思考文本（web client 插件，dsh.bundle 一键安装）
- `[未测]` [openpencil](https://github.com/zseven-w/openpencil) — OpenPencil 设计工具本体（DSH 适配器 dsh-openpencil 已另有条目；本体插队实测）
- `[未测]` [treg](https://github.com/superdesigndev/treg) — 工具目录：可检索约 2600 个外部端点（SEO/SERP/外链/社媒/人脉挖掘，536★，rc.8 实测 ✅）
- `[未测]` [better-deepseek](https://github.com/edgetype/better-deepseek) — Better DeepSeek Chrome 扩展桥接插件（398★，rc.8 实测 ✅）

...
> 数据源：radar 快照并集（`20260826T161502Z` 2026-08-27 00:15 UTC+8 ⊕ `20260826T150002Z` 2026-08-26 23:00 UTC+8 ⊕ `20260826T134501Z` 2026-08-26 21:45 UTC+8 等 81 轮）⊕ GitHub 定位复核缓存（data/locate-cache.json）。
> 呈现：分组列表（状态 · 名称  · 一句话说明），不使用大表格。

## 统一度量衡

**判定维度**（运行级四档，仅已定位条目 2242 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1141）/ `[不兼容]`（610）/ `[待定]`（244）/ `[未测]`（247）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（97）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- `[未定位]`（247）— 新占位条目，待下一轮定位复核（scripts/resolve_placeholders.py）
- 定位复核累计修复 363 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2690 条（已定位 2242 · 监测/未定位 448）· PR 登记 23 个
...
- `[待定]` [cocode-agency-cocode](https://github.com/cocode-agency/cocode) 156 — Best ready-to-run DeepSeek Harness distribution: DSH desktop GUI, DSH terminal TUI, and harness integration.
...
- `[不兼容]` [dsh-openbiliclaw](https://github.com/whiteguo233/dsh-openbiliclaw) 54 — OpenBiliClaw 是本地运行的跨平台个性化内容推荐 Agent，持续理解你的兴趣并主动找内容
...
- `[不兼容]` [jsdvjx-dsh-strata](https://github.com/jsdvjx/dsh-strata) 26 — Session strata for the DeepSeek Harness Web GUI: the transcript's scrollbar becomes a to-scale, colour-coded m
...
- `[可用]` [amlyczz-dsh-agy-link](https://github.com/amlyczz/dsh-agy-link) 20 — Google Antigravity (agy CLI) models for DeepSeek Harness — streaming chat, thinking, tool activity, usage, in-
...
- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 91647 — 🎨 Best DeepSeek Harness Design Plugin
...
- `[不兼容]` [whitelonng-dshcode](https://github.com/whitelonng/dshcode) 383 — Community desktop companion for DeepSeek Harness — one-click Electron app for macOS and Windows
...
- `[不兼容]` [dundunhan-dsh-video-lens](https://github.com/dundunhan/dsh-video-lens) 47 — —
...
- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 16816 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
...
- `[待定]` [bigstyle777-dsh-tool-arxiv](https://github.com/bigstyle777/dsh-tool-arxiv) 0 — arXiv preprint search tool for DeepSeek Harness agents — abstracts only, token-bounded
...
- `[不兼容]` [tong-io-tongflow](https://github.com/tong-io/tongflow) 937 — TongFlow — Multimodal GenAI Studio
...
- `[可用]` [pi2dsh](https://github.com/weijiafu14/pi2dsh) 166 — Bridge the Pi and DeepSeek Harness ecosystems: one Pi Host ABI runs unmodified Pi extensions as native DSH plu
...
- `[待定]` [xuedai-mayi-dsh-wenshan](https://github.com/xuedai-mayi/dsh-wenshan) 31 — 问山：DeepSeek Harness 地理野外实习知识图谱插件与非官方二次开发项目

...
**判定维度**（运行级四档，仅已定位条目 1817 个进入统计；测试：dsh 容器 agent + Qwen3.6-35B · k8s 5 分片 · run_id 锚定轮次）：

- `[可用]`（1039）/ `[不兼容]`（403）/ `[待定]`（176）/ `[未测]`（199）

**定位维度**（与判定正交；监测类不显示对错判定，原始结果保留于快照层）：

- `[空仓监测]`（71）— GitHub 复核无此仓库；待重现后恢复判定显示
- `[歧义监测]`（102）— 同名多仓无法锁定本体；锁定前不展示
- `[未定位]`（31）— 新占位条目，待下一轮定位复核（scripts/resolve_placeholders.py）
- 定位复核累计修复 363 个占位 URL

> 〔PR〕= 经已合并 PR 正式登记；收录 ≠ 兼容 ≠ 运行可用 ≠ 安全审计。

## 汇总：2021 条（已定位 1817 · 监测/未定位 204）· PR 登记 22 个
...
- `[未测]` [claude2dsh](https://github.com/kirkchinese/claude2dsh) 0 — 将 Claude Code 会话、技能、记忆与插件资产导入为 DSH 原生可续聊会话，并支持导出/同步回 Claude Code JSONL；双向冲突检测 + 显式三路合并；Settings 配置页
...
- `[未测]` [dsh-mcp-skill-panel](https://github.com/lilyblessing/dsh-mcp-skill-panel) 0 — MCP 与技能管理面板：MCP 服务器/Skill 实时启停释放上下文（停用态回填目录工具数）；可选 AI 中间层（mcp_search/mcp_call）按 server 状态过滤可见性、保活启用 + 空闲 30s 回收
...
- `[未测]` [dsh-chinese-traditional-wisdom-skill](https://github.com/dhicoc/dsh-chinese-traditional-wisdom-skill) 0 — 中华传统智慧（玄枢）AI Agent 技能包：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard；dsh.bundle manifest 可安装
- `[未测]` [dsh-skill-pack-security](https://github.com/perrylink/dsh-skill-pack-security) 0 — 安全审计方法论技能包：八个 agent 技能（密钥扫描、依赖审计、供应链评审、提示注入审查、审计总编排、威胁建模、漏洞情报、事件响应），中英双版本；`dsh plugin add @perrylink/dsh-skill-pack-security-provider` 一键挂载
- `[未测]` [sandbase-skills](https://github.com/sandbaseai/sandbase-skills) 0 — Research and growth skill collection with an npm CLI that installs complete bundles into DSH native .dsh/skills discovery root; verified against DSH 47f9438
- `[未测]` [dsh-plugin-installer](https://github.com/zhang66633/dsh-plugin-installer) 0 — 插件商店 + 安装助手：Web GUI「插件商店」页签 + 内置安装技能（npm 直装 / GitHub clone 注册，18 个已知坑配方）
- `[未测]` [dsh-remotion](https://github.com/stardustlc666/dsh-remotion) 0 — 视频创作技能插件：注册 Remotion 官方移植技能（React 编程式视频，动画/音频/字幕/3D/图表/字体 + 38 个规则文件），安装即用
- `[未测]` [dsh-hyperframes](https://github.com/stardustlc666/dsh-hyperframes) 0 — 视频创作技能插件：注册 HyperFrames by HeyGen 官方移植技能五件套（HTML 写视频 / hyperframes CLI / 注册表 / 网址转视频 / GSAP 参考），安装即用
- `[未测]` [dsh-codex-port](https://github.com/stardustlc666/dsh-codex-port) 0 — Codex 技能移植插件：扫描 ~/.codex 把官方 Codex 插件（本机实测 186 个插件/583 技能，一次移植 577 成功）批量转为 DSH 技能（codex_list/port/status），frontmatter 自动转换、幂等跳过
...
- `[未测]` [dsh-openviking](https://github.com/rxiain/dsh-openviking) 0 — OpenViking 检索、资源管理、自动召回与会话记忆：memsearch/memfind 等 10 个工具、已索引仓库上下文注入、自动召回注入、会话同步+自动提交
...
- `[未测]` [dsh-pixel-ui](https://github.com/zhang66633/dsh-pixel-ui) 0 — 像素风皮肤（Agent Xi 风格）：四主题一键切换 + fusion-pixel/Press Start 2P 像素字体 + CRT 质感，随时切回现代默认 UI
- `[未测]` [dsh-whale-animation](https://github.com/leemancheung/dsh-whale-animation) 0 — DSH Web 回合状态旁的 60 帧随主题适配的单色鲸鱼深潜动画：传播式水面、无缝闭环、资源内嵌、`prefers-reduced-motion` 静态 PNG 回退，停止时完整清理
- `[未测]` [dsh-qq2007-skin](https://github.com/leemancheung/dsh-qq2007-skin) 0 — DSH Web GUI 的 QQ 2007 风格皮肤：72 个原生主题 token、作用域三栏窗框、原创离线素材与像素伙伴、可选合成发送提示音、响应式/无障碍回退和可恢复设置开关
- `[未测]` [ncm-player](https://github.com/wolfgenerals/ncm-player) 0 — 网易云音乐浮窗播放器：歌单/歌词/歌词翻译/播放队列/账号登录，适配皮肤主题
- `[未测]` [dsh-agfs](https://github.com/openagfs/dsh-agfs) 0 — 文件浏览器 Web 插件（npm `@open-agfs/dsh-agfs`，`dsh plugin add` 一行安装）：宿主 webserver 同进程托管 React 前端 + REST API + `/dsh-agfs` 命令 + `browse_files`/`read_file` 模型工具；一键AI分析（右键建工作区并唤醒新会话，与手动创建一致的 agent preset 工具集）、本地打开（系统文件管理器）、三种显示尺寸、Win10 文件管理器主题（Segoe MDL2 Assets 官方图标）；109 单测 + 真实 Loader/HTTP 组合测试全绿，rc.6 web profile 实测通过（一键分析真实运行 55 次工具调用 0 错误）
...
- `[未测]` [dsh-plugin-guard](https://github.com/lxzy-7/dsh-plugin-guard) 0 — 插件安装安全网：装/卸/开关插件前自动快照备份，启动健康检查失败自动回退最后良好快照并重试一次，设置>备份管理面板 + 双击一键回退脚本，事故报告自动触发 Agent 分析；零运行时依赖，引擎冒烟测试 + 三种安装布局实测通过
- `[未测]` [dsh-suite](https://github.com/whyihaveyou/dsh-suite) 0 — DSH 插件活目录 + 脚手架 + 内置插件商店：785+ 插件每小时刷新、每日兼容 CI 实测、中英双语可搜索目录站；含 create-dsh-plugin 脚手架与 plugin-manager / plugin-notify / plugin-session-export / plugin-team-board 五个 npm 包
- `[未测]` [dsh-open-eyes](https://github.com/hyp6666/dsh-open-eyes) 0 — 让用户选择的多模态模型成为 DeepSeek 的眼睛：显式纯文本路由下桥接 WebUI 粘贴/拖入图片，并提供 vision_analyze；原生支持 OpenAI Responses、Chat Completions 与 Anthropic Messages，Credential Reference + workspace 边界；125 单测、Node 22/24 CI 与真实 tarball 临时 profile 安装/卸载通过
...
- `[可用]` [whale-girl](https://github.com/vlln/whale-girl) 222 — DSH Web GUI 桌面宠物插件（QQ 宠物形态）：右下角悬浮、可拖拽/投喂/玩耍的积累型伙伴
- `[可用]` [oh-dsh](https://github.com/hust-open-atom-club/oh-dsh) 210 — 一套 DSH runtime，Desktop、Web 与 TUI 三种开发体验
- `[待定]` [dsh-tianshu-tui](https://github.com/huiliyi37/dsh-tianshu-tui) 184 — 官方 DeepSeek Harness 的交互式终端 UI 插件：自研 ANSI 极简渲染核心（由作者自己的开源项目天枢 Tui 演进）、流式 Markdown/工具卡、多会话 tab、16+ 主题、slash 命令与选
...
- `[不兼容]` [vlln-plugin-registry](https://github.com/vlln/plugin-registry) 54 — DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
...
- `[可用]` [dsh-mnemon](https://github.com/omdsh-dev/dsh-mnemon) 45 — Three-tier memory control plane for DeepSeek Harness: persistent runtime context, searchable project documents
...
- `[可用]` [dsh-hdc-bridge](https://github.com/1na-ko/dsh-hdc-bridge) 7 — DSH 原生鸿蒙开发助手：hdc 设备闭环调试 + 设备面板（官方 client 插件形态）+ 离线官方知识层（Tier-1 随包）+ DevEco CLI 构建/签名/模拟器控制 / DSH-native Harmon
...
- `[待定]` [vvlife-awesome-deepseek-harness-plugins](https://github.com/vvlife/awesome-deepseek-harness-plugins) 7 — A curated list of plugins, tools, skins, and extensions for DeepSeek Harness (DSH).
...
- `[未定位]` **tqsy114514-dsh-ui-appearance** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-office](https://github.com/fayelin12/dsh-office) 0 — 办公室工作区/会话仪表盘：悬浮 6 列精灵面板，可视化工作区、会话、token 用量与子代理（web bundle）
- `[未测]` [dsh-split-panes](https://github.com/dsh-external/dsh-split-panes) 0 — 分屏面板
- `[未测]` [dsh-docker](https://github.com/stardustlc666/dsh-docker) 0 — 容器管理插件：docker_ps/logs/inspect/exec/manage 五工具，官方 subprocess 服务、argv 无 shell 注入、exec 审批门、零运行时依赖
- `[未测]` [dsh-sidebar-mode](https://github.com/meredith2328/dsh-sidebar-mode) 0 — 侧边栏「新会话」按钮内嵌 Agent 预设快速切换：点击弹出预设菜单即点即用，与设置里的「Agent 预设」双向同步
...
- `[未测]` [dsh-side-chat](https://github.com/heartmove/dsh-side-chat) 0 — DSH Web 侧边聊天：选中对话片段在右侧面板的侧边聊天提问（按会话隔离，继承主会话模型/思考难度/权限），AI 回复可原文或摘要带回主会话，问题弹框选项可一键带入
- `[未测]` [dsh-subagent-max](https://github.com/aaravarr/dsh-subagent-max) 0 — 子代理委派按次指定模型/提供商（host 侧 `subagent_with_model` 工具）+ 多面板实时流式子代理查看器（client 侧浮动面板 token 级流式输出、卡片网格、拖拽弹出、中英 i18n）；rc.6 headless 实测通过
- `[未测]` [dsh-update-notifier](https://github.com/arvin-yd/dsh-update-notifier) 0 — DSH 本体版本徽标：常驻侧边栏 Settings 行右侧，三色状态点（红=有更新/绿=最新/黄=检查中或失败），点开五态弹窗（复制更新命令/忽略/稍后/立即检查），启动 10s 首查+6h 复查；零构建、29 单测、mock-llm headless L4 实测（mainline 47f9438，证据见 [VERIFICATION.md](https://github.com/arvin-yd/dsh-update-notifier/blob/main/VERIFICATION.md)）
- `[未测]` [dsh-task-dag](https://github.com/leemancheung/dsh-task-dag) 0 — 由投影驱动的会话子代理与持久工作流实时 DAG：状态与节点导航、深层链路确定性布局、适应/平移画布，以及当前会话内节点拖动重排；无并行数据库或 Host 轮询
- `[未测]` [dsh-feishucard](https://github.com/cmfok/dsh-feishucard) 0 — DSH ↔ 飞书桥（自研非 fork）：官方 SDK 长连接（无需公网）+ 流式回复卡片（过程话语内联/工具折叠面板/状态符号/限流退避熔断兜底），每聊天独立会话 + live 复用保上下文，配置独立 ~/.dsh-feishucard；npm dsh-feishucard v0.1.0，冒烟 18 项 + 实机链路实测通过
- `[未测]` [dsh-opencodego-usage](https://github.com/beizi6/dsh-opencodego-usage) 0 — OpenCodeGo 剩余额度监视器：输入框右下角呼吸灯（按剩余额度绿/黄/红）+ 液态玻璃面板（滚动/周/月三窗口用量与重置时间），每 30 秒自动刷新，Key 自动读取 DSH 凭据（opencode-go 提供商）也可手动覆盖
...
- `[未测]` [dsh-session-hotkeys](https://github.com/yeyeyeyeshifu/dsh-session-hotkeys) 0 — Web UI 会话快捷键：Alt+1-9 顺序切换、固定槽位三态、Alt+↑↓ 循环切换、高亮导航模式、归档/重命名/新建会话、可改键面板（键盘导航 + 内置诊断），Windows/macOS 双预设无冲突，npm 已发布
- `[未测]` [dsh-feishu](https://github.com/pgzxb/dsh-feishu) 0 — DeepSeek Harness 的飞书 UI：面板驱动控制台，卡内审批与提问，流式卡片，扫码一键配置

## 💻 编码开发（316）

- `[不兼容]` [nexu-io-open-design](https://github.com/nexu-io/open-design) 88295 — 🎨 Best DeepSeek Harness Design Plugin
...
- `[不兼容]` [dsh-plugin-subscriptions](https://github.com/V1ki/dsh-plugin-subscriptions) 3 — Use ChatGPT (Codex), Claude, and Grok (X Premium) subscriptions as DeepSeek Harness LLM providers — OAuth logi
- `[不兼容]` [Deepseek-Harness-for-VS-Code](https://github.com/Vithrive/Deepseek-Harness-for-VS-Code) 3 — Deepseek Harness for VS Code
...
- `[未定位]` **unlinearity-dsh-code** — 占位待复核，判定暂不展示
- `[未定位]` **v587d-dsh-opencode-go-usage** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-win32](https://github.com/sjh9714/dsh-win32) 0 — 在 Windows 上把 DSH 用起来：一行装好极简模式的持久 shell（Git Bash/ConPTY，沙箱内可用），npm 可装
- `[未测]` [dsh-ffmpeg](https://github.com/stardustlc666/dsh-ffmpeg) 0 — 视频处理插件：ffmpeg_probe/cut/concat/encode/subtitle/extract/gif 七工具（探测/剪辑/拼接/转码/字幕烧录/抽帧/GIF），走官方 subprocess 服务、argv 数组无 shell 注入、零运行时依赖
- `[未测]` [vpshub](https://github.com/sdongmaker/vpshub) 0 — DSH 的 VPS Hub:本地 SSH 台账(Orca 风格 ssh-config/manual + tombstone),8 个 vps_* 工具让 AI 发现/测试/执行/传输,密钥仅路径引用;ProxyJump/ProxyCommand、Windows 密钥认证、i18n 设置页 UI;npm 包 dsh-vps-hub(v0.1.8)
...
- `[未测]` [dsh-agent-preset-recommender](https://github.com/leemancheung/dsh-agent-preset-recommender) 0 — 有界、隐私安全的本地扫描器：汇总 Codex、Claude Code、WorkBuddy、CodeBuddy 元数据，原子保存密钥化聚合证据，并确定性推荐 DSH 内置 preset 与可选能力；不保留正文、不联网、不修改 preset
- `[未测]` [dsh-approve-for-me](https://github.com/timeance/dsh-approve-for-me) 0 — DeepSeek Harness 沙箱扩权审批：Shell/PowerShell 字面命令前缀规则、固定高风险检查、可选无工具 LLM reviewer；成功仅授予一次 `allowed-once`，高危或不确定请求回原生人工审批，支持 Web/headless Profile
- `[未测]` [dsh-compressor](https://github.com/lifeodyssey/dsh-compressor) 0 — [Headroom](https://github.com/headroomlabs-ai/headroom) 的精简移植，在不影响模型上下文缓存以及 Agent 性能的情况下，压缩工具的输出，至多减少 20% 的上下文。
...
- `[未测]` [dsh-image-gen](https://github.com/leemancheung/dsh-image-gen) 0 — GPT Image 2 `image_gen`：默认复用 Codex 订阅 OAuth，也可显式使用 API Key；显影卡片、最多 3 张 API 实时局部图、持久附件回放/灯箱/下载、纯文本模型输出和受限的凭据安全请求
- `[未测]` [dsh-session-sync](https://github.com/perrylink/dsh-session-sync) 0 — 跨设备会话同步：专用 git 镜像 + 仅追加的双方保留冲突裁决（本地保留、远端留存 fork 文件，绝不静默覆盖），/sync 命令与 sync_status/sync_pull/sync_push 工具，自动推拉可配；49 测试 + 真实 git e2e 全绿，rc.6 --patch 加载实测通过

## 🤖 Agent 能力（266）

- `[待定]` [tt-a1i-archify](https://github.com/tt-a1i/archify) 13842 — Agent skill for beautiful, verifiable architecture, workflow, sequence, data-flow, and lifecycle diagrams—self
...
- `[可用]` [morluto-jacobian](https://github.com/morluto/jacobian) 48 — Pure mathematics for agents: search for examples and counterexamples, compute exactly, and independently check
...
- `[不兼容]` [dsh-work](https://github.com/vibeinging/dsh-work) 26 — Local-first AI workbench for DSH Plugins, combining Agent sessions, project files, data analysis, web research
...
- `[不兼容]` [w2112515-dsh-plugin-development](https://github.com/w2112515/dsh-plugin-development) 10 — Portable Agent Skill for developing and auditing DeepSeek Harness plugins, with an optional profile-installabl
...
- `[待定]` [dsh-ssh](https://github.com/UynajGI/dsh-ssh) 4 — SSH remote-execution plugin for DeepSeek Harness: ProxyJump chain, SFTP filesystem, subprocess and PTY over ss
...
- `[未定位]` **towzai-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **truehooha-dsh-plugin-dev-skill** — 占位待复核，判定暂不展示
- `[未定位]` **u-illll-dsh-memory** — 占位待复核，判定暂不展示
- `[未定位]` **v587d-dsh-multimodal-skill** — 占位待复核，判定暂不展示
...
- `[空仓监测]` **dsh-plugin-ptc-context** — GitHub 无此仓库，判定暂不展示
- `[未测]` [deepseek-heartflow](https://github.com/yun520-1/deepseek-heartflow) 0 — 心虫（AGI 第1层辨别门禁）：47 维纯规则文本判别 heartflow_check 工具 + tools/post-execute 自动输出监督（block 拦截 / rewrite 提醒），引擎缺失 fail-closed；dsh.bundle manifest 可安装
...
- `[未测]` [dsh-approval-gate](https://github.com/moon09300731/dsh-approval-gate) 0 — 自动审批门控：Flash 预判写入/命令是否不可回补，安全操作自动批准、危险操作转人工（fail-safe）
- `[未测]` [dsh-telegram](https://github.com/ben7am1n/dsh-telegram) 0 — Telegram runtime adapter — chat with dsh agents from Telegram; per-chat sessions, followup bridging, committed-text streaming, allowlist auth, zero runtime deps
- `[未测]` [dsh-sql](https://github.com/stardustlc666/dsh-sql) 0 — 工程师级数据库插件：sql_list/query/exec/schema 四工具，SQLite/MySQL/PostgreSQL 三引擎、多连接、只读白名单、写操作审批门、行数钳制
- `[未测]` [dsh-trajectory-reader](https://github.com/flyingtimes/dsh-trajectory-reader) 0 — 轨迹解读标签页：按用户轮次解读助手行为（需求/思路/执行/结果），规则引擎 + 可选 LLM 叙述，文件/命令/错误一目了然，用户消息原样保留；dsh.bundle.patch 一键安装
- `[未测]` [dsh-headroom](https://github.com/wanyantiande/dsh-headroom) 0 — Headroom 上下文压缩：历史区间压缩（替代 LLM 总结）+ 大工具输出压缩（超阈值自动瘦身）+ CCR 原文取回（headroom_retrieve 工具）；本地代理自动安装/生命周期管理 + 设置卡片；npm 已发布（@wanyantiande/dsh-headroom）
...
- `[可用]` [dsh-navbar](https://github.com/vlln/dsh-navbar) 36 — DSH 插件：对话节点导航条（右缘节点串快速跳转 user 消息）
...
- `[未测]` [dsh-daily-brief](https://github.com/equinox7379/dsh-daily-brief) 0 — 回合日报：跨 live 会话统计回合/用户消息/助手回复/工具调用（daily_brief 工具，只读零依赖）
...
- `[未测]` [logicprobe](https://github.com/amethystluna/logicprobe) 0 — 设计文档与重构计划声明核查：claim 枚举 + 代码库事实核对 + 逻辑原语验证（7 结构 + 7 对抗探针），dsh 原生 bundle 注入核查纪律门
...
- `[未定位]` **vulcan626-dsh-pet** — 占位待复核，判定暂不展示
...
- `[可用]` [hairyf-deepseek-harness-desktop](https://github.com/hairyf/deepseek-harness-desktop) 261 — DeepSeek Harness Tauri 桌面版 \| Only 5mb installer, zero environment setup
...
- `[不兼容]` [Bigfish](https://github.com/turtle2209/Bigfish) 184 — Bigfish —— DeepSeek Harness 的第三方桌面端，内置 Node 运行时，双击即用，附带桌面萌宠
...
- `[待定]` [deepseek-harness-wallet](https://github.com/feibi-mochi/deepseek-harness-wallet) 19 — Balance monitoring, per-session spend & token tracking, low-balance alerts, and an official recharge shortcut
...
- `[未定位]` **988hj7tczd-oss-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **rzdfzcl-dsh-desktop** — 占位待复核，判定暂不展示
...
- `[未定位]` **vickylastshao-deepseek-harness-desktop** — 占位待复核，判定暂不展示
- `[未定位]` **wangshu-lunadriver-deepseek-harness-desktop** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-full-remote](https://github.com/juanwang-buaa/dsh-full-remote) 0 — 令牌反向代理：改写 Host/Origin，远程恢复 `settings.*`/`credentials.*`/`host.listDirectory`（通用隧道会 403）；一次性扫码邀请、按设备会话；npm `dsh-full-remote`
- `[未测]` [dsh-malong-bridge](https://github.com/wulun811/liuhe) 0 — 六合工具集 DSH bundle：44 个 MCP 代码操作工具 + 动态 workspace 注入；npm 安装 @jieai/dsh-malong-bridge
- `[未测]` [dsh-voice](https://github.com/stardustlc666/dsh-voice) 0 — 语音双件套：voice_tts（edge-tts 协议免费微软神经语音，Sec-MS-GEC 本地 DRM 生成）/ voice_stt（OpenAI 兼容 ASR）/ voice_list，WS 压缩 + 可选代理隧道
...
- `[待定]` [quantum-practices](https://github.com/unitarylab/quantum-practices) 17 — 量子计算最佳实践 Quantum Algorithms Best Practices
...
- `[未定位]` **2633352305-deepseek-harness-edge-app** — 占位待复核，判定暂不展示
...
- `[未定位]` **junwan666-dsh-plugins** — 占位待复核，判定暂不展示
...
- `[未定位]` **liujunheng-deepseekharnessgreen** — 占位待复核，判定暂不展示
...
- `[未定位]` **thjyy-deepseek-harness-vision-assistant** — 占位待复核，判定暂不展示
- `[不兼容]` [timekeeperxy-harnessdesk](https://github.com/TimekeeperXY/HarnessDesk) 0 — —
- `[未定位]` **tom6814-dsh-web** — 占位待复核，判定暂不展示
- `[未定位]` **tomsawyerhu-deepseek-harness-cli** — 占位待复核，判定暂不展示
- `[未定位]` **totoro-qaq-cobsidian** — 占位待复核，判定暂不展示
- `[未定位]` **tttwh-dsh-plugin-audit** — 占位待复核，判定暂不展示
- `[未定位]` **twotwopiggy-dsh-balance** — 占位待复核，判定暂不展示
- `[未定位]` **tzhr-invest-dsh-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **uddoo-dsh-dashboard** — 占位待复核，判定暂不展示
- `[未定位]` **unknowbug-re-framework** — 占位待复核，判定暂不展示
- `[未定位]` **very12345-sai-dsh-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **vithrive-deepseek-harness-for-chrome** — 占位待复核，判定暂不展示
- `[未定位]` **wanbinyu-dsh-billing** — 占位待复核，判定暂不展示
- `[未定位]` **wanbinyu-dsh-plugins** — 占位待复核，判定暂不展示
- `[未定位]` **wangricardo-dsh-plugin-vision-tool** — 占位待复核，判定暂不展示
...
- `[未测]` [dsh-artifact](https://github.com/dsh-external/dsh-artifact) 0 — 制品管理 〔PR〕
- `[未测]` [dsh-balance](https://github.com/twotwopiggy/dsh-balance) 0 — 在 DSH Web 聊天框底部实时估算对话 Token 消耗并显示您的 DeepSeek 账户余额
- `[未测]` [dsh-latexcp](https://github.com/chi-hong22/dsh-latexcp) 0 — DSH Web 界面 LaTeX 公式复制插件：悬停 KaTeX 公式复制按钮，一键复制 TeX 源码（$…$ / \(…\) 两种格式
...
- `[未测]` [dsh-smooth-stream](https://github.com/laplace-bit/dsh-smooth-stream) 0 — DSH Web 界面丝滑流式渲染：打字机跟随 token 到达、Markdown 边流边渲染、换行滑入、不闪烁，滚动归用户，尊重 prefers-reduced-motion
...
- `[未测]` [dsh-vision-tools](https://github.com/moon09300731/dsh-vision-tools) 0 — 视觉能力全家桶：vision_understand 工具（OpenAI 兼容视觉 API，默认免费智谱 GLM-4V-Flash）+ 粘贴/拖拽/按钮三入口识图
- `[未测]` [dsh-routing-suite](https://github.com/yjh051108/dsh-routing-suite) 0 — 注入器 × 思维模式路由套装（3300+★ 正主仓，勿与同名仿品混）：免重启运行时注入器 + 任务感知推理模式路由预设（P1-P23 实测）；injector 子目录 `dsh plugin add` 安装
- `[未测]` [dsh-result-only-view](https://github.com/yeyeyeyeshifu/dsh-result-only-view) 0 — Web 对话「只看结果」开关：隐藏思考与工具调用过程，运行中仅保留一条实时状态行，回合结束显示「已处理 N 步」痕迹行可点击展开回看；中英双语，常规设置可配置痕迹行与 reduced-motion 光影策略；npm 已发布
- `[未测]` [dsh-self-evolving](https://github.com/timwhitez/dsh-self-evolving) 0 — 证据优先的 DSH 自进化引擎（标准 Cordis controller/service）：有界生成 Cordis 候选插件，一次性真实 Loader 隔离准入，Harbor 评估，可崩溃恢复的 journal 谱系；291 单测 + 36 Loader E2E

...
[![运行级可用](https://img.shields.io/badge/运行级可用_8804-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1731-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2534-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260904T061501Z`（2026-09-04 14:15:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 18010 · 龄 46m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 580"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 0）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 12960"}
    D1 -->|"8804 / 2534"| E1["聚合 + README 分类统计"]
    D1 -->|"1622 环境类重试"| C1
    E1 --> E2["cadence 交付<br/>本周期增量 —/100<br/>双仓 bot PR（幂等 supersede）"]
    M["radar-probe 每 15 分钟 自愈<br/>7 指标流 × 60s · 完成累计 43"]
    M -.-> A1
    M -.-> C1
```
<!-- AUTO:pipeline:END -->
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260904T061501Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

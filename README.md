...
[![confirmed](https://img.shields.io/badge/confirmed-1258-blue)](#精选插件榜) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#当前生态快照) [![tested](https://img.shields.io/badge/tested-2354-orange)](#本仓库如何判定) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=zh)](https://dshfind.com/zh/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![运行级可用](https://img.shields.io/badge/运行级可用-1607-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-516-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-231-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 收录 1258 个 DSH 插件仓库（索引到15443个repos，正由专用K8s集群，动态在DSH最新版本下验证可用性，目前高速迭代中）。

## 工作原理

> 数据截至快照 `20260827T200001Z`（2026-08-28 04:00:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 15443 · 龄 15m"]
        A2["本地库补全 · 去重 repo id"]
...
- **🛒 市场与管理**（67）— 可用 30 · 不兼容 13 · 待定 4 · 未测 11 · 监测 9 — [明细](PLUGINS-ALL.md#-市场与管理67)
- **🔌 Web UI 增强**（530）— 可用 307 · 不兼容 107 · 待定 56 · 未测 30 · 监测 30 — [明细](PLUGINS-ALL.md#-web-ui-增强530)
- **💻 编码开发**（438）— 可用 231 · 不兼容 81 · 待定 56 · 未测 32 · 监测 38 — [明细](PLUGINS-ALL.md#-编码开发438)
- **🤖 Agent 能力**（364）— 可用 177 · 不兼容 88 · 待定 50 · 未测 15 · 监测 34 — [明细](PLUGINS-ALL.md#-agent-能力364)
- **📡 消息通讯**（138）— 可用 68 · 不兼容 39 · 待定 18 · 未测 3 · 监测 10 — [明细](PLUGINS-ALL.md#-消息通讯138)
- **🗂 文件数据**（118）— 可用 53 · 不兼容 23 · 待定 21 · 未测 12 · 监测 9 — [明细](PLUGINS-ALL.md#-文件数据118)
- **🎮 娱乐生活**（64）— 可用 39 · 不兼容 11 · 待定 6 · 未测 1 · 监测 7 — [明细](PLUGINS-ALL.md#-娱乐生活64)
- **🛠 基建部署**（288）— 可用 140 · 不兼容 63 · 待定 48 · 未测 4 · 监测 33 — [明细](PLUGINS-ALL.md#-基建部署288)
- **📚 学习研究**（21）— 可用 11 · 不兼容 4 · 待定 0 · 未测 2 · 监测 4 — [明细](PLUGINS-ALL.md#-学习研究21)
- **❓ 其他**（780）— 可用 375 · 不兼容 152 · 待定 69 · 未测 31 · 监测 153 — [明细](PLUGINS-ALL.md#-其他780)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260827T200001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

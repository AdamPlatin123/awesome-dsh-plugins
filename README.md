...
[![运行级可用](https://img.shields.io/badge/运行级可用_7440-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1774-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2052-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260829T220001Z`（2026-08-30 06:00:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 16357 · 龄 51m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 7804"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 11160"}
    D1 -->|"7440 / 2052"| E1["聚合 + README 分类统计"]
    D1 -->|"1668 环境类重试"| C1
...
- **🔌 Web UI 增强**（2107）— 可用 1158 · 不兼容 302 · 待定 263 · 未测 16 · 监测 368 — [明细](PLUGINS-ALL.md#-web-ui-增强2107)
- **💻 编码开发**（1584）— 可用 788 · 不兼容 263 · 待定 191 · 未测 18 · 监测 324 — [明细](PLUGINS-ALL.md#-编码开发1584)
- **🤖 Agent 能力**（1559）— 可用 708 · 不兼容 234 · 待定 184 · 未测 10 · 监测 423 — [明细](PLUGINS-ALL.md#-agent-能力1559)
- **📡 消息通讯**（535）— 可用 224 · 不兼容 100 · 待定 56 · 未测 3 · 监测 152 — [明细](PLUGINS-ALL.md#-消息通讯535)
- **🗂 文件数据**（474）— 可用 224 · 不兼容 66 · 待定 56 · 未测 9 · 监测 119 — [明细](PLUGINS-ALL.md#-文件数据474)
...
- **❓ 其他**（4035）— 可用 1439 · 不兼容 249 · 待定 245 · 未测 20 · 监测 2082 — [明细](PLUGINS-ALL.md#-其他4035)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260829T220001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

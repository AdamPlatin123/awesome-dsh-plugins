...
[![运行级可用](https://img.shields.io/badge/运行级可用_8534-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1539-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2642-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260903T173001Z`（2026-09-04 01:30:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 17870 · 龄 1m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 441"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 0）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 12605"}
    D1 -->|"8534 / 2642"| E1["聚合 + README 分类统计"]
    D1 -->|"1429 环境类重试"| C1
...
- **🔌 Web UI 增强**（2652）— 可用 1296 · 不兼容 391 · 待定 225 · 未测 16 · 监测 724 — [明细](PLUGINS-ALL.md#-web-ui-增强2652)
- **💻 编码开发**（2061）— 可用 902 · 不兼容 342 · 待定 162 · 未测 19 · 监测 636 — [明细](PLUGINS-ALL.md#-编码开发2061)
...
- **🗂 文件数据**（648）— 可用 265 · 不兼容 89 · 待定 48 · 未测 9 · 监测 237 — [明细](PLUGINS-ALL.md#-文件数据648)
- **🎮 娱乐生活**（439）— 可用 173 · 不兼容 29 · 待定 29 · 未测 0 · 监测 208 — [明细](PLUGINS-ALL.md#-娱乐生活439)
- **🛠 基建部署**（1320）— 可用 435 · 不兼容 129 · 待定 118 · 未测 3 · 监测 635 — [明细](PLUGINS-ALL.md#-基建部署1320)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260903T173001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

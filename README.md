...
[![运行级可用](https://img.shields.io/badge/运行级可用_8160-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1480-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2614-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260901T220001Z`（2026-09-02 06:00:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 17411 · 龄 44m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 8770"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 12146"}
    D1 -->|"8160 / 2614"| E1["聚合 + README 分类统计"]
    D1 -->|"1372 环境类重试"| C1
...
- **💻 编码开发**（1729）— 可用 853 · 不兼容 324 · 待定 168 · 未测 19 · 监测 365 — [明细](PLUGINS-ALL.md#-编码开发1729)
- **🤖 Agent 能力**（1684）— 可用 757 · 不兼容 296 · 待定 156 · 未测 10 · 监测 465 — [明细](PLUGINS-ALL.md#-agent-能力1684)
- **📡 消息通讯**（561）— 可用 231 · 不兼容 123 · 待定 47 · 未测 3 · 监测 157 — [明细](PLUGINS-ALL.md#-消息通讯561)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260901T220001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

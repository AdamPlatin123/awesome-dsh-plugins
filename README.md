...
[![运行级可用](https://img.shields.io/badge/运行级可用_8125-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1438-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2621-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260901T001501Z`（2026-09-01 08:17:05 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 17120 · 龄 182m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 8691"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 12076"}
    D1 -->|"8125 / 2621"| E1["聚合 + README 分类统计"]
    D1 -->|"1330 环境类重试"| C1
...
- **🔌 Web UI 增强**（2242）— 可用 1236 · 不兼容 382 · 待定 213 · 未测 16 · 监测 395 — [明细](PLUGINS-ALL.md#-web-ui-增强2242)
- **💻 编码开发**（1717）— 可用 854 · 不兼容 321 · 待定 162 · 未测 19 · 监测 361 — [明细](PLUGINS-ALL.md#-编码开发1717)
- **🤖 Agent 能力**（1668）— 可用 751 · 不兼容 294 · 待定 153 · 未测 10 · 监测 460 — [明细](PLUGINS-ALL.md#-agent-能力1668)
...
- **❓ 其他**（4580）— 可用 1665 · 不兼容 290 · 待定 231 · 未测 21 · 监测 2373 — [明细](PLUGINS-ALL.md#-其他4580)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260901T001501Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

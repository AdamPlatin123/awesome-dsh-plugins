...
[![confirmed](https://img.shields.io/badge/confirmed-3004-blue)](#精选插件榜) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#当前生态快照) [![tested](https://img.shields.io/badge/tested-2409-orange)](#本仓库如何判定) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=zh)](https://dshfind.com/zh/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![运行级可用](https://img.shields.io/badge/运行级可用-1567-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-556-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-286-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260827T230001Z`（2026-08-28 07:00:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 15459 · 龄 112m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 3004"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 2409"}
    D1 -->|"1567 / 556"| E1["聚合 + README 分类统计"]
    D1 -->|"286 环境类重试"| C1
...
- **🛒 市场与管理**（83）— 可用 36 · 不兼容 14 · 待定 7 · 未测 10 · 监测 16 — [明细](PLUGINS-ALL.md#-市场与管理83)
- **🔌 Web UI 增强**（607）— 可用 335 · 不兼容 122 · 待定 72 · 未测 29 · 监测 49 — [明细](PLUGINS-ALL.md#-web-ui-增强607)
- **💻 编码开发**（493）— 可用 249 · 不兼容 89 · 待定 75 · 未测 30 · 监测 50 — [明细](PLUGINS-ALL.md#-编码开发493)
- **🤖 Agent 能力**（400）— 可用 184 · 不兼容 92 · 待定 63 · 未测 15 · 监测 46 — [明细](PLUGINS-ALL.md#-agent-能力400)
- **📡 消息通讯**（151）— 可用 71 · 不兼容 41 · 待定 21 · 未测 3 · 监测 15 — [明细](PLUGINS-ALL.md#-消息通讯151)
- **🗂 文件数据**（134）— 可用 58 · 不兼容 24 · 待定 26 · 未测 12 · 监测 14 — [明细](PLUGINS-ALL.md#-文件数据134)
- **🎮 娱乐生活**（74）— 可用 45 · 不兼容 8 · 待定 9 · 未测 1 · 监测 11 — [明细](PLUGINS-ALL.md#-娱乐生活74)
- **🛠 基建部署**（339）— 可用 147 · 不兼容 68 · 待定 61 · 未测 4 · 监测 59 — [明细](PLUGINS-ALL.md#-基建部署339)
- **📚 学习研究**（26）— 可用 10 · 不兼容 4 · 待定 1 · 未测 2 · 监测 9 — [明细](PLUGINS-ALL.md#-学习研究26)
- **❓ 其他**（916）— 可用 393 · 不兼容 156 · 待定 104 · 未测 26 · 监测 237 — [明细](PLUGINS-ALL.md#-其他916)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260827T230001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

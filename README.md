...
[![运行级可用](https://img.shields.io/badge/运行级可用_7442-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1812-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2012-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260829T210001Z`（2026-08-30 05:00:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 16321 · 龄 209m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 7783"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 11160"}
    D1 -->|"7442 / 2012"| E1["聚合 + README 分类统计"]
    D1 -->|"1706 环境类重试"| C1
...
- **🔌 Web UI 增强**（2107）— 可用 1156 · 不兼容 297 · 待定 270 · 未测 16 · 监测 368 — [明细](PLUGINS-ALL.md#-web-ui-增强2107)
- **💻 编码开发**（1584）— 可用 788 · 不兼容 260 · 待定 195 · 未测 18 · 监测 323 — [明细](PLUGINS-ALL.md#-编码开发1584)
- **🤖 Agent 能力**（1559）— 可用 708 · 不兼容 229 · 待定 190 · 未测 10 · 监测 422 — [明细](PLUGINS-ALL.md#-agent-能力1559)
...
- **❓ 其他**（4025）— 可用 1435 · 不兼容 248 · 待定 250 · 未测 20 · 监测 2072 — [明细](PLUGINS-ALL.md#-其他4025)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260829T210001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

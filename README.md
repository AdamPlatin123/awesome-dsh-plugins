...
[![运行级可用](https://img.shields.io/badge/运行级可用-1179-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-874-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-222-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260827T073001Z`（2026-08-27 15:30:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 15158 · 龄 11m"]
        A2["本地库补全 · 去重 repo id"]
...
- **🧠 记忆增强**（19）— 可用 7 · 不兼容 5 · 待定 2 · 未测 5 · 监测 0 — [明细](PLUGINS-ALL.md#-记忆增强19)
...
- **🔌 Web UI 增强**（504）— 可用 272 · 不兼容 124 · 待定 51 · 未测 32 · 监测 25 — [明细](PLUGINS-ALL.md#-web-ui-增强504)
- **💻 编码开发**（420）— 可用 192 · 不兼容 99 · 待定 52 · 未测 43 · 监测 34 — [明细](PLUGINS-ALL.md#-编码开发420)
- **🤖 Agent 能力**（355）— 可用 147 · 不兼容 108 · 待定 48 · 未测 23 · 监测 29 — [明细](PLUGINS-ALL.md#-agent-能力355)
- **📡 消息通讯**（130）— 可用 61 · 不兼容 44 · 待定 15 · 未测 4 · 监测 6 — [明细](PLUGINS-ALL.md#-消息通讯130)
- **🗂 文件数据**（110）— 可用 51 · 不兼容 23 · 待定 17 · 未测 13 · 监测 6 — [明细](PLUGINS-ALL.md#-文件数据110)
- **🎮 娱乐生活**（61）— 可用 33 · 不兼容 14 · 待定 7 · 未测 1 · 监测 6 — [明细](PLUGINS-ALL.md#-娱乐生活61)
- **🛠 基建部署**（277）— 可用 97 · 不兼容 112 · 待定 35 · 未测 6 · 监测 27 — [明细](PLUGINS-ALL.md#-基建部署277)
- **📚 学习研究**（19）— 可用 7 · 不兼容 7 · 待定 0 · 未测 2 · 监测 3 — [明细](PLUGINS-ALL.md#-学习研究19)
- **❓ 其他**（737）— 可用 324 · 不兼容 192 · 待定 60 · 未测 51 · 监测 110 — [明细](PLUGINS-ALL.md#-其他737)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260827T073001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

...
[![运行级可用](https://img.shields.io/badge/运行级可用_8110-0.1.1--rc.2-brightgreen)](#2-看懂状态统一四档口径) [![待测](https://img.shields.io/badge/待测_1398-0.1.1--rc.2-lightgrey)](#2-看懂状态统一四档口径) [![需适配](https://img.shields.io/badge/需适配_2676-0.1.1--rc.2-yellow)](#2-看懂状态统一四档口径)
...
> 数据截至快照 `20260831T213002Z`（2026-09-01 05:30:03 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 17120 · 龄 15m"]
        A2["本地库补全 · 去重 repo id"]
    ...
    B1 -->|"插件 8553"| C1["k8s 运行级测试<br/>一插件一 pod · 并发 10<br/>dsh agent + Qwen（de-stream）"]
    B1 -->|"非插件（累计删 1064）"| B3["即删省空间"]
    C1 --> D1{"判定 · 总 12076"}
    D1 -->|"8110 / 2676"| E1["聚合 + README 分类统计"]
    D1 -->|"1290 环境类重试"| C1
...
- **🔌 Web UI 增强**（2242）— 可用 1234 · 不兼容 395 · 待定 202 · 未测 16 · 监测 395 — [明细](PLUGINS-ALL.md#-web-ui-增强2242)
- **💻 编码开发**（1717）— 可用 853 · 不兼容 328 · 待定 156 · 未测 19 · 监测 361 — [明细](PLUGINS-ALL.md#-编码开发1717)
- **🤖 Agent 能力**（1668）— 可用 748 · 不兼容 302 · 待定 148 · 未测 10 · 监测 460 — [明细](PLUGINS-ALL.md#-agent-能力1668)
- **📡 消息通讯**（561）— 可用 230 · 不兼容 126 · 待定 46 · 未测 3 · 监测 156 — [明细](PLUGINS-ALL.md#-消息通讯561)
- **🗂 文件数据**（509）— 可用 242 · 不兼容 84 · 待定 47 · 未测 9 · 监测 127 — [明细](PLUGINS-ALL.md#-文件数据509)
...
- **❓ 其他**（4580）— 可用 1665 · 不兼容 293 · 待定 228 · 未测 21 · 监测 2373 — [明细](PLUGINS-ALL.md#-其他4580)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260831T213002Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

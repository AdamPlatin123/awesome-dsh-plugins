...
**自动发现、证据验证的 DeepSeek Harness 插件生态雷达。自动发现 15300+ 候选、逐个 k8s 实测**


安装前就知道哪个能用，不用自己踩坑。

[![confirmed](https://img.shields.io/badge/confirmed-1258-blue)](#精选插件榜) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#当前生态快照) [![tested](https://img.shields.io/badge/tested-2177-orange)](#本仓库如何判定) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=zh)](https://dshfind.com/zh/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![运行级可用](https://img.shields.io/badge/运行级可用-1446-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-440-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-291-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 收录 1258 个 DSH 插件仓库（索引到15312个repos，正由专用K8s集群，动态在DSH最新版本下验证可用性，目前高速迭代中）。

## 工作原理

> 数据截至快照 `20260827T170001Z`（2026-08-28 01:00:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 15312 · 龄 212m"]
        A2["本地库补全 · 去重 repo id"]
...
- **🧠 记忆增强**（20）— 可用 10 · 不兼容 2 · 待定 3 · 未测 5 · 监测 0 — [明细](PLUGINS-ALL.md#-记忆增强20)
- **🎨 主题皮肤**（14）— 可用 3 · 不兼容 0 · 待定 2 · 未测 8 · 监测 1 — [明细](PLUGINS-ALL.md#-主题皮肤14)
- **🛒 市场与管理**（66）— 可用 30 · 不兼容 13 · 待定 4 · 未测 11 · 监测 8 — [明细](PLUGINS-ALL.md#-市场与管理66)
- **🔌 Web UI 增强**（511）— 可用 298 · 不兼容 102 · 待定 53 · 未测 32 · 监测 26 — [明细](PLUGINS-ALL.md#-web-ui-增强511)
- **💻 编码开发**（426）— 可用 215 · 不兼容 73 · 待定 64 · 未测 39 · 监测 35 — [明细](PLUGINS-ALL.md#-编码开发426)
- **🤖 Agent 能力**（355）— 可用 169 · 不兼容 86 · 待定 52 · 未测 19 · 监测 29 — [明细](PLUGINS-ALL.md#-agent-能力355)
- **📡 消息通讯**（134）— 可用 72 · 不兼容 35 · 待定 15 · 未测 4 · 监测 8 — [明细](PLUGINS-ALL.md#-消息通讯134)
- **🗂 文件数据**（110）— 可用 51 · 不兼容 21 · 待定 20 · 未测 12 · 监测 6 — [明细](PLUGINS-ALL.md#-文件数据110)
- **🎮 娱乐生活**（63）— 可用 38 · 不兼容 11 · 待定 6 · 未测 1 · 监测 7 — [明细](PLUGINS-ALL.md#-娱乐生活63)
- **🛠 基建部署**（278）— 可用 136 · 不兼容 60 · 待定 48 · 未测 6 · 监测 28 — [明细](PLUGINS-ALL.md#-基建部署278)
- **📚 学习研究**（19）— 可用 10 · 不兼容 4 · 待定 0 · 未测 2 · 监测 3 — [明细](PLUGINS-ALL.md#-学习研究19)
- **❓ 其他**（739）— 可用 362 · 不兼容 151 · 待定 64 · 未测 45 · 监测 117 — [明细](PLUGINS-ALL.md#-其他739)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260827T170001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

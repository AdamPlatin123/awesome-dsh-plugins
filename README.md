...
**自动发现、证据验证的 DeepSeek Harness 插件生态雷达。自动发现 9500+ 候选、逐个 k8s 实测**


安装前就知道哪个能用，不用自己踩坑。

[![confirmed](https://img.shields.io/badge/confirmed-1258-blue)](#精选插件榜) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#当前生态快照) [![tested](https://img.shields.io/badge/tested-2031-orange)](#本仓库如何判定) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=zh)](https://dshfind.com/zh/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![运行级可用](https://img.shields.io/badge/运行级可用-1115-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-739-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-177-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 收录 1258 个 DSH 插件仓库（索引到9584个repos，正由专用K8s集群，动态在DSH最新版本下验证可用性，目前高速迭代中）。

## 工作原理

> 数据截至快照 `20260825T123001Z`（2026-08-25 20:30:01 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 9584 · 龄 9028m"]
        A2["本地库补全 · 去重 repo id"]
...
- **🎓 技能包**（27）— 可用 6 · 不兼容 1 · 待定 1 · 未测 18 · 监测 1 — [明细](PLUGINS-ALL.md#-技能包27)
- **🧠 记忆增强**（20）— 可用 7 · 不兼容 4 · 待定 2 · 未测 6 · 监测 1 — [明细](PLUGINS-ALL.md#-记忆增强20)
- **🎨 主题皮肤**（13）— 可用 2 · 不兼容 1 · 待定 2 · 未测 8 · 监测 0 — [明细](PLUGINS-ALL.md#-主题皮肤13)
- **🛒 市场与管理**（60）— 可用 23 · 不兼容 13 · 待定 2 · 未测 11 · 监测 11 — [明细](PLUGINS-ALL.md#-市场与管理60)
- **🔌 Web UI 增强**（484）— 可用 249 · 不兼容 97 · 待定 59 · 未测 38 · 监测 41 — [明细](PLUGINS-ALL.md#-web-ui-增强484)
- **💻 编码开发**（407）— 可用 183 · 不兼容 81 · 待定 43 · 未测 55 · 监测 45 — [明细](PLUGINS-ALL.md#-编码开发407)
- **🤖 Agent 能力**（340）— 可用 141 · 不兼容 80 · 待定 34 · 未测 31 · 监测 54 — [明细](PLUGINS-ALL.md#-agent-能力340)
- **📡 消息通讯**（131）— 可用 63 · 不兼容 34 · 待定 14 · 未测 8 · 监测 12 — [明细](PLUGINS-ALL.md#-消息通讯131)
- **🗂 文件数据**（110）— 可用 49 · 不兼容 21 · 待定 17 · 未测 15 · 监测 8 — [明细](PLUGINS-ALL.md#-文件数据110)
- **🎮 娱乐生活**（57）— 可用 31 · 不兼容 10 · 待定 8 · 未测 2 · 监测 6 — [明细](PLUGINS-ALL.md#-娱乐生活57)
- **🛠 基建部署**（246）— 可用 92 · 不兼容 82 · 待定 23 · 未测 8 · 监测 41 — [明细](PLUGINS-ALL.md#-基建部署246)
- **📚 学习研究**（20）— 可用 5 · 不兼容 4 · 待定 1 · 未测 3 · 监测 7 — [明细](PLUGINS-ALL.md#-学习研究20)
- **❓ 其他**（701）— 可用 283 · 不兼容 131 · 待定 46 · 未测 63 · 监测 178 — [明细](PLUGINS-ALL.md#-其他701)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260825T123001Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

...
**自动发现、证据验证的 DeepSeek Harness 插件生态雷达。自动发现 14500+ 候选、逐个 k8s 实测**


安装前就知道哪个能用，不用自己踩坑。

[![confirmed](https://img.shields.io/badge/confirmed-1258-blue)](#精选插件榜) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#当前生态快照) [![tested](https://img.shields.io/badge/tested-2064-orange)](#本仓库如何判定) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=zh)](https://dshfind.com/zh/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![运行级可用](https://img.shields.io/badge/运行级可用-1139-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-786-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-139-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 收录 1258 个 DSH 插件仓库（索引到14541个repos，正由专用K8s集群，动态在DSH最新版本下验证可用性，目前高速迭代中）。

## 工作原理

> 数据截至快照 `20260826T111501Z`（2026-08-26 19:15:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 14541 · 龄 859m"]
        A2["本地库补全 · 去重 repo id"]
...
- **🎓 技能包**（29）— 可用 6 · 不兼容 1 · 待定 1 · 未测 18 · 监测 3 — [明细](PLUGINS-ALL.md#-技能包29)
- **🧠 记忆增强**（20）— 可用 7 · 不兼容 4 · 待定 2 · 未测 6 · 监测 1 — [明细](PLUGINS-ALL.md#-记忆增强20)
- **🎨 主题皮肤**（12）— 可用 1 · 不兼容 1 · 待定 2 · 未测 8 · 监测 0 — [明细](PLUGINS-ALL.md#-主题皮肤12)
- **🛒 市场与管理**（64）— 可用 24 · 不兼容 13 · 待定 2 · 未测 11 · 监测 14 — [明细](PLUGINS-ALL.md#-市场与管理64)
- **🔌 Web UI 增强**（483）— 可用 255 · 不兼容 105 · 待定 43 · 未测 38 · 监测 42 — [明细](PLUGINS-ALL.md#-web-ui-增强483)
- **💻 编码开发**（409）— 可用 184 · 不兼容 84 · 待定 40 · 未测 55 · 监测 46 — [明细](PLUGINS-ALL.md#-编码开发409)
- **🤖 Agent 能力**（345）— 可用 142 · 不兼容 81 · 待定 37 · 未测 31 · 监测 54 — [明细](PLUGINS-ALL.md#-agent-能力345)
- **📡 消息通讯**（133）— 可用 62 · 不兼容 35 · 待定 14 · 未测 8 · 监测 14 — [明细](PLUGINS-ALL.md#-消息通讯133)
- **🗂 文件数据**（108）— 可用 48 · 不兼容 23 · 待定 15 · 未测 15 · 监测 7 — [明细](PLUGINS-ALL.md#-文件数据108)
- **🎮 娱乐生活**（59）— 可用 31 · 不兼容 12 · 待定 7 · 未测 2 · 监测 7 — [明细](PLUGINS-ALL.md#-娱乐生活59)
- **🛠 基建部署**（259）— 可用 95 · 不兼容 88 · 待定 23 · 未测 8 · 监测 45 — [明细](PLUGINS-ALL.md#-基建部署259)
- **📚 学习研究**（19）— 可用 5 · 不兼容 4 · 待定 0 · 未测 3 · 监测 7 — [明细](PLUGINS-ALL.md#-学习研究19)
- **❓ 其他**（718）— 可用 282 · 不兼容 135 · 待定 45 · 未测 61 · 监测 195 — [明细](PLUGINS-ALL.md#-其他718)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260826T111501Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

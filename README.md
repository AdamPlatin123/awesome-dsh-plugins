...
[![运行级可用](https://img.shields.io/badge/运行级可用-1405-brightgreen)](#2-看懂状态统一四档口径) [![运行级不兼容](https://img.shields.io/badge/运行级不兼容-457-red)](#2-看懂状态统一四档口径) [![待定](https://img.shields.io/badge/待定-188-yellow)](#2-看懂状态统一四档口径) [![未测](https://img.shields.io/badge/·_未测-0-lightgrey)](#2-看懂状态统一四档口径)
...
> 收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个 DSH 插件仓库（索引到15443个repos，正由专用K8s集群，动态在DSH最新版本下验证可用性，目前高速迭代中）。

## 工作原理

> 数据截至快照 `20260827T204501Z`（2026-08-28 04:45:02 UTC+8 · 分类器 unified-v2-bridge）

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["发现（每 6 小时 · probe 每 15 分钟 巡检触发）"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>候选 15443 · 龄 59m"]
        A2["本地库补全 · 去重 repo id"]
...
- **💻 编码开发**（448）— 可用 234 · 不兼容 84 · 待定 57 · 未测 31 · 监测 42 — [明细](PLUGINS-ALL.md#-编码开发448)
- **🤖 Agent 能力**（371）— 可用 181 · 不兼容 87 · 待定 52 · 未测 15 · 监测 36 — [明细](PLUGINS-ALL.md#-agent-能力371)
...
- **🛠 基建部署**（296）— 可用 136 · 不兼容 64 · 待定 51 · 未测 4 · 监测 41 — [明细](PLUGINS-ALL.md#-基建部署296)
- **📚 学习研究**（22）— 可用 11 · 不兼容 4 · 待定 0 · 未测 2 · 监测 5 — [明细](PLUGINS-ALL.md#-学习研究22)
- **❓ 其他**（802）— 可用 373 · 不兼容 155 · 待定 76 · 未测 30 · 监测 168 — [明细](PLUGINS-ALL.md#-其他802)
...
> [!NOTE]
> **测试版本**：dsh（容器内 agent）+ Qwen3.6-35B 驱动（经 de-stream 代理）· k8s 5 分片 · 以快照 `run_id` 锚定具体轮次（当前 `20260827T204501Z`）。DSH 的 npm 版本号未随快照记录，以 run_id 与 `reports/agent-test/` 日期交叉核对。

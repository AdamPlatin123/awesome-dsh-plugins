...
[![confirmed](https://img.shields.io/badge/confirmed-9064-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-12574-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_8462-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1547-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2673-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **9064 plugin repos indexed** (manifest-level classification, v2 engine), **12574 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260902T124501Z` (2026-09-02 20:45:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17578 · age 218m"]
    ...
    B1 -->|"plugins 9064"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12574"}
    D1 -->|"8462 / 2673"| E1["aggregate + README stats"]
    D1 -->|"1439 env retries"| C1
    E1 --> E2["cadence deliver<br/>delta this cycle —/100<br/>dual-repo bot PRs (idempotent)"]
    M["radar-probe every 15 min self-heal<br/>7 metric streams × 60s · done 52"]
    M -.-> A1
    M -.-> C1
```
<!-- AUTO:pipeline:END -->
...
- **🔌 Web UI 增强**（2336）— OK 1290 · incompatible 383 · pending 226 · untested 16 · watching 421 — [details](PLUGINS-ALL.md#-web-ui-增强2336)
- **💻 编码开发**（1805）— OK 890 · incompatible 340 · pending 176 · untested 19 · watching 380 — [details](PLUGINS-ALL.md#-编码开发1805)
- **🤖 Agent 能力**（1693）— OK 751 · incompatible 276 · pending 167 · untested 10 · watching 489 — [details](PLUGINS-ALL.md#-agent-能力1693)
- **📡 消息通讯**（571）— OK 238 · incompatible 121 · pending 50 · untested 3 · watching 159 — [details](PLUGINS-ALL.md#-消息通讯571)
- **🗂 文件数据**（543）— OK 255 · incompatible 96 · pending 48 · untested 9 · watching 135 — [details](PLUGINS-ALL.md#-文件数据543)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260902T124501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260902T124501Z（2026-09-02 20:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8462 可用 · 2673 不兼容 · 1439 待定（共 12574 个，k8s agent 口径）|

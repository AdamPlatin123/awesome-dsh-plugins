...
[![confirmed](https://img.shields.io/badge/confirmed-8840-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-12345-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_8298-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1523-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2632-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **8840 plugin repos indexed** (manifest-level classification, v2 engine), **12345 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260902T040001Z` (2026-09-02 12:00:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17519 · age 174m"]
    ...
    B1 -->|"plugins 8840"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12345"}
    D1 -->|"8298 / 2632"| E1["aggregate + README stats"]
    D1 -->|"1415 env retries"| C1
...
- **🔌 Web UI 增强**（2293）— OK 1262 · incompatible 378 · pending 225 · untested 16 · watching 412 — [details](PLUGINS-ALL.md#-web-ui-增强2293)
- **💻 编码开发**（1764）— OK 872 · incompatible 329 · pending 172 · untested 19 · watching 372 — [details](PLUGINS-ALL.md#-编码开发1764)
- **🤖 Agent 能力**（1709）— OK 769 · incompatible 292 · pending 163 · untested 10 · watching 475 — [details](PLUGINS-ALL.md#-agent-能力1709)
...
- **🛠 基建部署**（1062）— OK 418 · incompatible 125 · pending 133 · untested 3 · watching 383 — [details](PLUGINS-ALL.md#-基建部署1062)
- **📚 学习研究**（113）— OK 32 · incompatible 9 · pending 7 · untested 2 · watching 63 — [details](PLUGINS-ALL.md#-学习研究113)
- **❓ 其他**（4720）— OK 1705 · incompatible 296 · pending 235 · untested 21 · watching 2463 — [details](PLUGINS-ALL.md#-其他4720)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260902T040001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260902T040001Z（2026-09-02 12:00 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8298 可用 · 2632 不兼容 · 1415 待定（共 12345 个，k8s agent 口径）|

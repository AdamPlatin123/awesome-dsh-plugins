...
[![confirmed](https://img.shields.io/badge/confirmed-8214-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-11482-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_7673-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1332-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2583-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **8214 plugin repos indexed** (manifest-level classification, v2 engine), **11482 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260830T004501Z` (2026-08-30 08:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 16357 · age 216m"]
    ...
    B1 -->|"plugins 8214"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 11482"}
    D1 -->|"7673 / 2583"| E1["aggregate + README stats"]
    D1 -->|"1226 env retries"| C1
...
- **🔌 Web UI 增强**（2155）— OK 1189 · incompatible 382 · pending 193 · untested 16 · watching 375 — [details](PLUGINS-ALL.md#-web-ui-增强2155)
- **💻 编码开发**（1630）— OK 813 · incompatible 314 · pending 151 · untested 18 · watching 334 — [details](PLUGINS-ALL.md#-编码开发1630)
- **🤖 Agent 能力**（1590）— OK 713 · incompatible 291 · pending 142 · untested 10 · watching 434 — [details](PLUGINS-ALL.md#-agent-能力1590)
- **📡 消息通讯**（545）— OK 221 · incompatible 123 · pending 43 · untested 3 · watching 155 — [details](PLUGINS-ALL.md#-消息通讯545)
- **🗂 文件数据**（483）— OK 228 · incompatible 81 · pending 45 · untested 9 · watching 120 — [details](PLUGINS-ALL.md#-文件数据483)
- **🎮 娱乐生活**（316）— OK 148 · incompatible 28 · pending 28 · untested 0 · watching 112 — [details](PLUGINS-ALL.md#-娱乐生活316)
...
- **❓ 其他**（4166）— OK 1492 · incompatible 289 · pending 221 · untested 20 · watching 2144 — [details](PLUGINS-ALL.md#-其他4166)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260830T004501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260830T004501Z（2026-08-30 08:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 7673 可用 · 2583 不兼容 · 1226 待定（共 11482 个，k8s agent 口径）|

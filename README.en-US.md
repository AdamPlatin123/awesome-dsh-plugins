...
[![runtime OK](https://img.shields.io/badge/runtime_OK_9013-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1751-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2683-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **960 plugin repos indexed** (manifest-level classification, v2 engine), **13334 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260905T220001Z` (2026-09-06 06:00:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18470 · age 49m"]
    ...
    D1 -->|"9013 / 2683"| E1["aggregate + README stats"]
    D1 -->|"1638 env retries"| C1
...
- **🔌 Web UI 增强**（2779）— OK 1352 · incompatible 398 · pending 256 · untested 17 · watching 756 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2154）— OK 934 · incompatible 353 · pending 186 · untested 19 · watching 662 — [details](catalog/all/编码开发.md)
- **🤖 Agent 能力**（2245）— OK 818 · incompatible 269 · pending 165 · untested 11 · watching 982 — [details](catalog/all/Agent%20能力.md)
...
- **❓ 其他**（7191）— OK 1841 · incompatible 363 · pending 253 · untested 21 · watching 4713 — [details](catalog/all/其他.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260905T220001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260905T220001Z（2026-09-06 06:00 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 9013 可用 · 2683 不兼容 · 1638 待定（共 13334 个，k8s agent 口径）|

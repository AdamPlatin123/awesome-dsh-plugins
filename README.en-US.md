...
[![confirmed](https://img.shields.io/badge/confirmed-7800-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-11160-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_7442-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1784-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2040-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **7800 plugin repos indexed** (manifest-level classification, v2 engine), **11160 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260829T214501Z` (2026-08-30 05:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 16357 · age 36m"]
    ...
    B1 -->|"plugins 7800"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 11160"}
    D1 -->|"7442 / 2040"| E1["aggregate + README stats"]
...
- **🔌 Web UI 增强**（2107）— OK 1158 · incompatible 299 · pending 266 · untested 16 · watching 368 — [details](PLUGINS-ALL.md#-web-ui-增强2107)
- **💻 编码开发**（1584）— OK 788 · incompatible 262 · pending 192 · untested 18 · watching 324 — [details](PLUGINS-ALL.md#-编码开发1584)
- **🤖 Agent 能力**（1559）— OK 708 · incompatible 231 · pending 187 · untested 10 · watching 423 — [details](PLUGINS-ALL.md#-agent-能力1559)
- **📡 消息通讯**（535）— OK 224 · incompatible 100 · pending 56 · untested 3 · watching 152 — [details](PLUGINS-ALL.md#-消息通讯535)
- **🗂 文件数据**（474）— OK 225 · incompatible 65 · pending 56 · untested 9 · watching 119 — [details](PLUGINS-ALL.md#-文件数据474)
- **🎮 娱乐生活**（305）— OK 144 · incompatible 27 · pending 30 · untested 0 · watching 104 — [details](PLUGINS-ALL.md#-娱乐生活305)
...
- **❓ 其他**（4035）— OK 1439 · incompatible 247 · pending 247 · untested 20 · watching 2082 — [details](PLUGINS-ALL.md#-其他4035)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260829T214501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260829T214501Z（2026-08-30 05:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 7442 可用 · 2040 不兼容 · 1678 待定（共 11160 个，k8s agent 口径）|

...
[![confirmed](https://img.shields.io/badge/confirmed-4854-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-5797-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=en)](https://dshfind.com/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK-3830-brightgreen)](#2-understand-status-unified-4-tier-scale) [![incompatible](https://img.shields.io/badge/incompatible-1284-red)](#2-understand-status-unified-4-tier-scale) [![pending](https://img.shields.io/badge/pending-683-yellow)](#2-understand-status-unified-4-tier-scale) [![untested](https://img.shields.io/badge/untested-0-lightgrey)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **4854 plugin repos indexed** (manifest-level classification, v2 engine), **5797 runtime-tested on the k8s track**.

## How it works

> Data as of snapshot `20260828T093001Z` (2026-08-28 17:30:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 15618 · age 28m"]
    ...
    B1 -->|"plugins 4854"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 5797"}
    D1 -->|"3830 / 1284"| E1["aggregate + README stats"]
    D1 -->|"683 env retries"| C1
...
- **🔌 Web UI 增强**（1256）— OK 688 · incompatible 238 · pending 123 · untested 22 · watching 185 — [details](PLUGINS-ALL.md#-web-ui-增强1256)
- **💻 编码开发**（946）— OK 469 · incompatible 191 · pending 109 · untested 22 · watching 155 — [details](PLUGINS-ALL.md#-编码开发946)
- **🤖 Agent 能力**（808）— OK 356 · incompatible 154 · pending 98 · untested 12 · watching 188 — [details](PLUGINS-ALL.md#-agent-能力808)
- **📡 消息通讯**（315）— OK 129 · incompatible 74 · pending 32 · untested 3 · watching 77 — [details](PLUGINS-ALL.md#-消息通讯315)
- **🗂 文件数据**（277）— OK 121 · incompatible 55 · pending 36 · untested 9 · watching 56 — [details](PLUGINS-ALL.md#-文件数据277)
- **🎮 娱乐生活**（161）— OK 84 · incompatible 15 · pending 17 · untested 0 · watching 45 — [details](PLUGINS-ALL.md#-娱乐生活161)
- **🛠 基建部署**（646）— OK 258 · incompatible 93 · pending 93 · untested 4 · watching 198 — [details](PLUGINS-ALL.md#-基建部署646)
- **📚 学习研究**（60）— OK 18 · incompatible 5 · pending 3 · untested 2 · watching 32 — [details](PLUGINS-ALL.md#-学习研究60)
- **❓ 其他**（1945）— OK 614 · incompatible 219 · pending 136 · untested 21 · watching 955 — [details](PLUGINS-ALL.md#-其他1945)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260828T093001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260828T093001Z（2026-08-28 17:30 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 3830 可用 · 1284 不兼容 · 683 待定（共 5797 个，k8s agent 口径）|

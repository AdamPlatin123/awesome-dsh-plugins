...
[![confirmed](https://img.shields.io/badge/confirmed-8715-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-12215-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_8207-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1506-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2610-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **8715 plugin repos indexed** (manifest-level classification, v2 engine), **12215 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260902T020002Z` (2026-09-02 10:00:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17519 · age 53m"]
    ...
    B1 -->|"plugins 8715"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12215"}
    D1 -->|"8207 / 2610"| E1["aggregate + README stats"]
    D1 -->|"1398 env retries"| C1
...
- **🔌 Web UI 增强**（2274）— OK 1254 · incompatible 376 · pending 221 · untested 16 · watching 407 — [details](PLUGINS-ALL.md#-web-ui-增强2274)
- **💻 编码开发**（1744）— OK 859 · incompatible 329 · pending 169 · untested 19 · watching 368 — [details](PLUGINS-ALL.md#-编码开发1744)
- **🤖 Agent 能力**（1694）— OK 762 · incompatible 290 · pending 162 · untested 10 · watching 470 — [details](PLUGINS-ALL.md#-agent-能力1694)
...
- **🛠 基建部署**（1056）— OK 416 · incompatible 125 · pending 133 · untested 3 · watching 379 — [details](PLUGINS-ALL.md#-基建部署1056)
- **📚 学习研究**（113）— OK 32 · incompatible 9 · pending 7 · untested 2 · watching 63 — [details](PLUGINS-ALL.md#-学习研究113)
- **❓ 其他**（4660）— OK 1676 · incompatible 292 · pending 233 · untested 21 · watching 2438 — [details](PLUGINS-ALL.md#-其他4660)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260902T020002Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260902T020002Z（2026-09-02 10:00 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8207 可用 · 2610 不兼容 · 1398 待定（共 12215 个，k8s agent 口径）|

...
> Data as of snapshot `20260904T034501Z` (2026-09-04 11:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17811 · age 156m"]
    ...
    B1 -->|"plugins 553"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12960"}
    D1 -->|"8805 / 2555"| E1["aggregate + README stats"]
    D1 -->|"1600 env retries"| C1
...
- **🔌 Web UI 增强**（2706）— OK 1320 · incompatible 384 · pending 242 · untested 16 · watching 744 — [details](PLUGINS-ALL.md#-web-ui-增强2706)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260904T034501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260904T034501Z（2026-09-04 11:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8805 可用 · 2555 不兼容 · 1600 待定（共 12960 个，k8s agent 口径）|

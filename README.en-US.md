...
> Data as of snapshot `20260903T223001Z` (2026-09-04 06:30:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17943 · age 77m"]
    ...
    B1 -->|"plugins 466"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12960"}
    D1 -->|"8798 / 2646"| E1["aggregate + README stats"]
    D1 -->|"1516 env retries"| C1
...
- **🔌 Web UI 增强**（2716）— OK 1331 · incompatible 389 · pending 236 · untested 16 · watching 744 — [details](PLUGINS-ALL.md#-web-ui-增强2716)
- **💻 编码开发**（2096）— OK 918 · incompatible 344 · pending 171 · untested 19 · watching 644 — [details](PLUGINS-ALL.md#-编码开发2096)
- **🤖 Agent 能力**（2194）— OK 806 · incompatible 265 · pending 156 · untested 10 · watching 957 — [details](PLUGINS-ALL.md#-agent-能力2194)
- **📡 消息通讯**（703）— OK 249 · incompatible 122 · pending 50 · untested 3 · watching 279 — [details](PLUGINS-ALL.md#-消息通讯703)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260903T223001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260903T223001Z（2026-09-04 06:30 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8798 可用 · 2646 不兼容 · 1516 待定（共 12960 个，k8s agent 口径）|

...
> Data as of snapshot `20260902T041502Z` (2026-09-02 12:15:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17519 · age 189m"]
    ...
    B1 -->|"plugins 8845"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12352"}
    D1 -->|"8302 / 2634"| E1["aggregate + README stats"]
    D1 -->|"1416 env retries"| C1
...
- **🔌 Web UI 增强**（2296）— OK 1264 · incompatible 378 · pending 225 · untested 16 · watching 413 — [details](PLUGINS-ALL.md#-web-ui-增强2296)
...
- **❓ 其他**（4722）— OK 1705 · incompatible 296 · pending 235 · untested 21 · watching 2465 — [details](PLUGINS-ALL.md#-其他4722)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260902T041502Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260902T041502Z（2026-09-02 12:15 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8302 可用 · 2634 不兼容 · 1416 待定（共 12352 个，k8s agent 口径）|

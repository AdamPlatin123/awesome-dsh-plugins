...
[![confirmed](https://img.shields.io/badge/confirmed-5191-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-6229-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=en)](https://dshfind.com/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK-4149-brightgreen)](#2-understand-status-unified-4-tier-scale) [![incompatible](https://img.shields.io/badge/incompatible-1369-red)](#2-understand-status-unified-4-tier-scale) [![pending](https://img.shields.io/badge/pending-711-yellow)](#2-understand-status-unified-4-tier-scale) [![untested](https://img.shields.io/badge/untested-0-lightgrey)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **5191 plugin repos indexed** (manifest-level classification, v2 engine), **6229 runtime-tested on the k8s track**.

## How it works

> Data as of snapshot `20260828T103001Z` (2026-08-28 18:30:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 15618 · age 87m"]
    ...
    B1 -->|"plugins 5191"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 6229"}
    D1 -->|"4149 / 1369"| E1["aggregate + README stats"]
    D1 -->|"711 env retries"| C1
...
- **🛒 市场与管理**（165）— OK 67 · incompatible 16 · pending 12 · untested 8 · watching 62 — [details](PLUGINS-ALL.md#-市场与管理165)
- **🔌 Web UI 增强**（1318）— OK 741 · incompatible 230 · pending 129 · untested 22 · watching 196 — [details](PLUGINS-ALL.md#-web-ui-增强1318)
- **💻 编码开发**（990）— OK 515 · incompatible 183 · pending 114 · untested 21 · watching 157 — [details](PLUGINS-ALL.md#-编码开发990)
- **🤖 Agent 能力**（840）— OK 388 · incompatible 142 · pending 100 · untested 12 · watching 198 — [details](PLUGINS-ALL.md#-agent-能力840)
- **📡 消息通讯**（328）— OK 142 · incompatible 70 · pending 33 · untested 3 · watching 80 — [details](PLUGINS-ALL.md#-消息通讯328)
- **🗂 文件数据**（290）— OK 134 · incompatible 54 · pending 36 · untested 9 · watching 57 — [details](PLUGINS-ALL.md#-文件数据290)
- **🎮 娱乐生活**（176）— OK 92 · incompatible 13 · pending 20 · untested 0 · watching 51 — [details](PLUGINS-ALL.md#-娱乐生活176)
- **🛠 基建部署**（667）— OK 276 · incompatible 87 · pending 95 · untested 3 · watching 206 — [details](PLUGINS-ALL.md#-基建部署667)
- **📚 学习研究**（63）— OK 19 · incompatible 5 · pending 3 · untested 2 · watching 34 — [details](PLUGINS-ALL.md#-学习研究63)
- **❓ 其他**（2031）— OK 676 · incompatible 186 · pending 145 · untested 21 · watching 1003 — [details](PLUGINS-ALL.md#-其他2031)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260828T103001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260828T103001Z（2026-08-28 18:30 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 4149 可用 · 1369 不兼容 · 711 待定（共 6229 个，k8s agent 口径）|

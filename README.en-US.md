...
> Data as of snapshot `20260903T203001Z` (2026-09-04 04:30:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17870 · age 181m"]
    ...
    B1 -->|"plugins 428"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12960"}
    D1 -->|"8793 / 2697"| E1["aggregate + README stats"]
    D1 -->|"1470 env retries"| C1
...
- **💻 编码开发**（2099）— OK 920 · incompatible 348 · pending 168 · untested 19 · watching 644 — [details](PLUGINS-ALL.md#-编码开发2099)
- **🤖 Agent 能力**（2193）— OK 803 · incompatible 268 · pending 155 · untested 10 · watching 957 — [details](PLUGINS-ALL.md#-agent-能力2193)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260903T203001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.

...
[![confirmed](https://img.shields.io/badge/confirmed-966-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-13182-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_9004-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1754-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2533-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **966 plugin repos indexed** (manifest-level classification, v2 engine), **13182 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260905T180001Z` (2026-09-06 02:00:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18450 · age 25m"]
    ...
    B1 -->|"plugins 966"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13182"}
    D1 -->|"9004 / 2533"| E1["aggregate + README stats"]
    D1 -->|"1645 env retries"| C1
    E1 --> E2["cadence deliver<br/>delta this cycle —/100<br/>dual-repo bot PRs (idempotent)"]
    M["radar-probe every 15 min self-heal<br/>7 metric streams × 60s · done 31"]
    M -.-> A1
    M -.-> C1
```
<!-- AUTO:pipeline:END -->
...
- **🔌 Web UI 增强**（2752）— OK 1347 · incompatible 384 · pending 253 · untested 16 · watching 752 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2135）— OK 935 · incompatible 339 · pending 184 · untested 19 · watching 658 — [details](catalog/all/编码开发.md)
- **🤖 Agent 能力**（2227）— OK 819 · incompatible 255 · pending 166 · untested 10 · watching 977 — [details](catalog/all/Agent%20能力.md)
- **📡 消息通讯**（710）— OK 260 · incompatible 110 · pending 55 · untested 3 · watching 282 — [details](catalog/all/消息通讯.md)
- **🗂 文件数据**（672）— OK 272 · incompatible 90 · pending 57 · untested 9 · watching 244 — [details](catalog/all/文件数据.md)
- **🎮 娱乐生活**（450）— OK 175 · incompatible 31 · pending 31 · untested 0 · watching 213 — [details](catalog/all/娱乐生活.md)
...
- **❓ 其他**（7128）— OK 1848 · incompatible 324 · pending 256 · untested 21 · watching 4679 — [details](catalog/all/其他.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260905T180001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260905T180001Z（2026-09-06 02:00 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 9004 可用 · 2533 不兼容 · 1645 待定（共 13182 个，k8s agent 口径）|

...
> Data as of snapshot `20260901T001501Z` (2026-09-01 08:17:05 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17120 · age 182m"]
    ...
    B1 -->|"plugins 8691"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12076"}
    D1 -->|"8125 / 2621"| E1["aggregate + README stats"]
    D1 -->|"1330 env retries"| C1
...
- **🔌 Web UI 增强**（2242）— OK 1236 · incompatible 382 · pending 213 · untested 16 · watching 395 — [details](PLUGINS-ALL.md#-web-ui-增强2242)
- **💻 编码开发**（1717）— OK 854 · incompatible 321 · pending 162 · untested 19 · watching 361 — [details](PLUGINS-ALL.md#-编码开发1717)
- **🤖 Agent 能力**（1668）— OK 751 · incompatible 294 · pending 153 · untested 10 · watching 460 — [details](PLUGINS-ALL.md#-agent-能力1668)
...
- **❓ 其他**（4580）— OK 1665 · incompatible 290 · pending 231 · untested 21 · watching 2373 — [details](PLUGINS-ALL.md#-其他4580)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260901T001501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260901T001501Z（2026-09-01 08:17 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8125 可用 · 2621 不兼容 · 1330 待定（共 12076 个，k8s agent 口径）|

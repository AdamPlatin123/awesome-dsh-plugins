...
[![confirmed](https://img.shields.io/badge/confirmed-2996-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-2050-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=en)](https://dshfind.com/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK-1405-brightgreen)](#2-understand-status-unified-4-tier-scale) [![incompatible](https://img.shields.io/badge/incompatible-457-red)](#2-understand-status-unified-4-tier-scale) [![pending](https://img.shields.io/badge/pending-188-yellow)](#2-understand-status-unified-4-tier-scale) [![untested](https://img.shields.io/badge/untested-0-lightgrey)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **2996 plugin repos indexed** (manifest-level classification, v2 engine), **2050 runtime-tested on the k8s track**.

## How it works

> Data as of snapshot `20260827T204501Z` (2026-08-28 04:45:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 15443 · age 59m"]
    ...
    C1 --> D1{"verdict · total 2050"}
    D1 -->|"1405 / 457"| E1["aggregate + README stats"]
    D1 -->|"188 env retries"| C1
...
- **💻 编码开发**（448）— OK 234 · incompatible 84 · pending 57 · untested 31 · watching 42 — [details](PLUGINS-ALL.md#-编码开发448)
- **🤖 Agent 能力**（371）— OK 181 · incompatible 87 · pending 52 · untested 15 · watching 36 — [details](PLUGINS-ALL.md#-agent-能力371)
...
- **🛠 基建部署**（296）— OK 136 · incompatible 64 · pending 51 · untested 4 · watching 41 — [details](PLUGINS-ALL.md#-基建部署296)
- **📚 学习研究**（22）— OK 11 · incompatible 4 · pending 0 · untested 2 · watching 5 — [details](PLUGINS-ALL.md#-学习研究22)
- **❓ 其他**（802）— OK 373 · incompatible 155 · pending 76 · untested 30 · watching 168 — [details](PLUGINS-ALL.md#-其他802)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260827T204501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260827T204501Z（2026-08-28 04:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 1405 可用 · 457 不兼容 · 188 待定（共 2050 个，k8s agent 口径）|

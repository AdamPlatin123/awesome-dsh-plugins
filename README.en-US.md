...
> Data as of snapshot `20260903T201501Z` (2026-09-04 04:15:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17870 · age 166m"]
    ...
    B1 -->|"plugins 425"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12960"}
    D1 -->|"8794 / 2700"| E1["aggregate + README stats"]
    D1 -->|"1466 env retries"| C1
...
- **🔌 Web UI 增强**（2715）— OK 1325 · incompatible 399 · pending 231 · untested 16 · watching 744 — [details](PLUGINS-ALL.md#-web-ui-增强2715)
- **💻 编码开发**（2099）— OK 920 · incompatible 349 · pending 167 · untested 19 · watching 644 — [details](PLUGINS-ALL.md#-编码开发2099)
- **🤖 Agent 能力**（2193）— OK 804 · incompatible 268 · pending 154 · untested 10 · watching 957 — [details](PLUGINS-ALL.md#-agent-能力2193)
- **📡 消息通讯**（703）— OK 243 · incompatible 127 · pending 51 · untested 3 · watching 279 — [details](PLUGINS-ALL.md#-消息通讯703)
- **🗂 文件数据**（660）— OK 271 · incompatible 93 · pending 48 · untested 9 · watching 239 — [details](PLUGINS-ALL.md#-文件数据660)
- **🎮 娱乐生活**（449）— OK 176 · incompatible 32 · pending 30 · untested 0 · watching 211 — [details](PLUGINS-ALL.md#-娱乐生活449)
...
- **❓ 其他**（6998）— OK 1823 · incompatible 316 · pending 250 · untested 21 · watching 4588 — [details](PLUGINS-ALL.md#-其他6998)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260903T201501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260903T201501Z（2026-09-04 04:15 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8794 可用 · 2700 不兼容 · 1466 待定（共 12960 个，k8s agent 口径）|

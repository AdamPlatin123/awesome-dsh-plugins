...
> Data as of snapshot `20260903T074501Z` (2026-09-03 15:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17667 · age 139m"]
    ...
    B1 -->|"plugins 330"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12605"}
    D1 -->|"8523 / 2633"| E1["aggregate + README stats"]
...
- **❓ 其他**（6860）— OK 1762 · incompatible 310 · pending 240 · untested 21 · watching 4527 — [details](PLUGINS-ALL.md#-其他6860)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260903T074501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260903T074501Z（2026-09-03 15:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

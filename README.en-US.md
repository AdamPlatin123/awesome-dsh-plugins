...
> Data as of snapshot `20260901T224501Z` (2026-09-02 06:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17411 · age 90m"]
    ...
    B1 -->|"plugins 8774"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12146"}
    D1 -->|"8165 / 2598"| E1["aggregate + README stats"]
    D1 -->|"1383 env retries"| C1
...
- **🔌 Web UI 增强**（2256）— OK 1242 · incompatible 375 · pending 219 · untested 16 · watching 404 — [details](PLUGINS-ALL.md#-web-ui-增强2256)
- **💻 编码开发**（1729）— OK 852 · incompatible 323 · pending 170 · untested 19 · watching 365 — [details](PLUGINS-ALL.md#-编码开发1729)
- **🤖 Agent 能力**（1684）— OK 758 · incompatible 292 · pending 159 · untested 10 · watching 465 — [details](PLUGINS-ALL.md#-agent-能力1684)
- **📡 消息通讯**（561）— OK 232 · incompatible 119 · pending 50 · untested 3 · watching 157 — [details](PLUGINS-ALL.md#-消息通讯561)
...
- **🛠 基建部署**（1053）— OK 415 · incompatible 127 · pending 131 · untested 3 · watching 377 — [details](PLUGINS-ALL.md#-基建部署1053)
- **📚 学习研究**（113）— OK 31 · incompatible 9 · pending 8 · untested 2 · watching 63 — [details](PLUGINS-ALL.md#-学习研究113)
- **❓ 其他**（4645）— OK 1673 · incompatible 293 · pending 235 · untested 21 · watching 2423 — [details](PLUGINS-ALL.md#-其他4645)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260901T224501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260901T224501Z（2026-09-02 06:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8165 可用 · 2598 不兼容 · 1383 待定（共 12146 个，k8s agent 口径）|

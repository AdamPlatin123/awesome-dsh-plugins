...
**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **960 plugin repos indexed** (manifest-level classification, v2 engine), **13182 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260905T174501Z` (2026-09-06 01:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18450 · age 10m"]
    ...
    B1 -->|"plugins 960"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13182"}
    D1 -->|"8997 / 2494"| E1["aggregate + README stats"]
...
- **🔌 Web UI 增强**（2752）— OK 1346 · incompatible 383 · pending 255 · untested 16 · watching 752 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2135）— OK 933 · incompatible 338 · pending 187 · untested 19 · watching 658 — [details](catalog/all/编码开发.md)
- **🤖 Agent 能力**（2227）— OK 819 · incompatible 254 · pending 167 · untested 10 · watching 977 — [details](catalog/all/Agent%20能力.md)
- **📡 消息通讯**（710）— OK 260 · incompatible 109 · pending 56 · untested 3 · watching 282 — [details](catalog/all/消息通讯.md)
- **🗂 文件数据**（672）— OK 272 · incompatible 89 · pending 58 · untested 9 · watching 244 — [details](catalog/all/文件数据.md)
...
- **❓ 其他**（7128）— OK 1846 · incompatible 324 · pending 258 · untested 21 · watching 4679 — [details](catalog/all/其他.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260905T174501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260905T174501Z（2026-09-06 01:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8997 可用 · 2494 不兼容 · 1691 待定（共 13182 个，k8s agent 口径）|

...
> Data as of snapshot `20260905T173001Z` (2026-09-06 01:30:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18348 · age 475m"]
    ...
    B1 -->|"plugins 956"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13181"}
    D1 -->|"8997 / 2483"| E1["aggregate + README stats"]
    D1 -->|"1701 env retries"| C1
...
- **🛒 市场与管理**（306）— OK 99 · incompatible 20 · pending 18 · untested 8 · watching 161 — [details](catalog/all/市场与管理.md)
- **🔌 Web UI 增强**（2751）— OK 1346 · incompatible 380 · pending 257 · untested 16 · watching 752 — [details](catalog/all/Web%20UI%20增强.md)
- **💻 编码开发**（2135）— OK 933 · incompatible 339 · pending 187 · untested 19 · watching 657 — [details](catalog/all/编码开发.md)
- **🤖 Agent 能力**（2229）— OK 820 · incompatible 255 · pending 167 · untested 10 · watching 977 — [details](catalog/all/Agent%20能力.md)
- **📡 消息通讯**（710）— OK 260 · incompatible 109 · pending 55 · untested 3 · watching 283 — [details](catalog/all/消息通讯.md)
...
- **❓ 其他**（7126）— OK 1844 · incompatible 321 · pending 260 · untested 21 · watching 4680 — [details](catalog/all/其他.md)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260905T173001Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260905T173001Z（2026-09-06 01:30 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8997 可用 · 2483 不兼容 · 1701 待定（共 13181 个，k8s agent 口径）|

...
[![confirmed](https://img.shields.io/badge/confirmed-1258-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-2295-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/awesome-dsh-plugins?lang=en)](https://dshfind.com/plugins/AdamPlatin123/awesome-dsh-plugins?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)
...
**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **1258 plugin repos indexed** (manifest-level classification, v2 engine), **2295 runtime-tested on the k8s track**.

## How it works

> Data as of snapshot `20260827T074501Z` (2026-08-27 15:45:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 15158 · age 26m"]
...
- **💻 编码开发**（420）— OK 191 · incompatible 99 · pending 54 · untested 42 · watching 34 — [details](PLUGINS-ALL.md#-编码开发420)
- **🤖 Agent 能力**（353）— OK 147 · incompatible 108 · pending 47 · untested 22 · watching 29 — [details](PLUGINS-ALL.md#-agent-能力353)
...
- **❓ 其他**（740）— OK 325 · incompatible 192 · pending 61 · untested 51 · watching 111 — [details](PLUGINS-ALL.md#-其他740)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260827T074501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260827T074501Z（2026-08-27 15:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 | 1258 个仓库 |
| 运行级实测 | 1179 可用 · 874 不兼容 · 242 待定（共 2295 个，k8s agent 口径）|

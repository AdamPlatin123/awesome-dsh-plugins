...
> Data as of snapshot `20260901T214501Z` (2026-09-02 05:45:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 17411 · age 29m"]
    ...
    B1 -->|"plugins 8769"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12146"}
    D1 -->|"8160 / 2622"| E1["aggregate + README stats"]
    D1 -->|"1364 env retries"| C1
...
- **💻 编码开发**（1729）— OK 854 · incompatible 325 · pending 166 · untested 19 · watching 365 — [details](PLUGINS-ALL.md#-编码开发1729)
...
- **🗂 文件数据**（509）— OK 243 · incompatible 81 · pending 47 · untested 9 · watching 129 — [details](PLUGINS-ALL.md#-文件数据509)
...
- **❓ 其他**（4645）— OK 1674 · incompatible 294 · pending 233 · untested 21 · watching 2423 — [details](PLUGINS-ALL.md#-其他4645)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260901T214501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260901T214501Z（2026-09-02 05:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8160 可用 · 2622 不兼容 · 1364 待定（共 12146 个，k8s agent 口径）|

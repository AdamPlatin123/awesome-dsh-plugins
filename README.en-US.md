...
[![confirmed](https://img.shields.io/badge/confirmed-595-blue)](#featured) [![scan](https://img.shields.io/badge/scan-every_6h-green)](#ecosystem-snapshot) [![tested](https://img.shields.io/badge/tested-12960-orange)](#how-we-assess-compatibility) [![dshfind](https://dshfind.com/api/badge/AdamPlatin123/dsh-plugin-radar?lang=en)](https://dshfind.com/plugins/AdamPlatin123/dsh-plugin-radar?ref=badge) [![license](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

[![runtime OK](https://img.shields.io/badge/runtime_OK_8810-0.1.1--rc.2-brightgreen)](#2-understand-status-unified-4-tier-scale) [![to test](https://img.shields.io/badge/to_test_1745-0.1.1--rc.2-lightgrey)](#2-understand-status-unified-4-tier-scale) [![needs adapt](https://img.shields.io/badge/needs_adapt_2514-0.1.1--rc.2-yellow)](#2-understand-status-unified-4-tier-scale)

[English](README.en-US.md) | [简体中文](README.md)

---

**What is this?** DeepSeek Harness (DSH) is an open-source coding agent where everything is a plugin. This repo is a **radar** that automatically tracks its plugin ecosystem — **595 plugin repos indexed** (manifest-level classification, v2 engine), **12960 runtime-tested on the k8s track**.
...
> Data as of snapshot `20260904T081501Z` (2026-09-04 16:15:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18010 · age 165m"]
    ...
    B1 -->|"plugins 595"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 12960"}
    D1 -->|"8810 / 2514"| E1["aggregate + README stats"]
    D1 -->|"1636 env retries"| C1
    E1 --> E2["cadence deliver<br/>delta this cycle —/100<br/>dual-repo bot PRs (idempotent)"]
    M["radar-probe every 15 min self-heal<br/>7 metric streams × 60s · done 15"]
    M -.-> A1
    M -.-> C1
```
<!-- AUTO:pipeline:END -->
...
- **🔌 Web UI 增强**（2716）— OK 1329 · incompatible 379 · pending 248 · untested 16 · watching 744 — [details](PLUGINS-ALL.md#-web-ui-增强2716)
- **💻 编码开发**（2095）— OK 917 · incompatible 339 · pending 176 · untested 19 · watching 644 — [details](PLUGINS-ALL.md#-编码开发2095)
- **🤖 Agent 能力**（2194）— OK 804 · incompatible 262 · pending 161 · untested 10 · watching 957 — [details](PLUGINS-ALL.md#-agent-能力2194)
- **📡 消息通讯**（702）— OK 251 · incompatible 118 · pending 51 · untested 3 · watching 279 — [details](PLUGINS-ALL.md#-消息通讯702)
- **🗂 文件数据**（661）— OK 269 · incompatible 92 · pending 52 · untested 9 · watching 239 — [details](PLUGINS-ALL.md#-文件数据661)
- **🎮 娱乐生活**（449）— OK 176 · incompatible 29 · pending 33 · untested 0 · watching 211 — [details](PLUGINS-ALL.md#-娱乐生活449)
...
- **❓ 其他**（7001）— OK 1819 · incompatible 311 · pending 262 · untested 21 · watching 4588 — [details](PLUGINS-ALL.md#-其他7001)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260904T081501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260904T081501Z（2026-09-04 16:15 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 8810 可用 · 2514 不兼容 · 1636 待定（共 12960 个，k8s agent 口径）|

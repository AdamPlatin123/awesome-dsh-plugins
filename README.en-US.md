...
> Data as of snapshot `20260831T021501Z` (2026-08-31 10:15:02 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 16796 · age 71m"]
    ...
    B1 -->|"plugins 8386"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 1064)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 11846"}
    D1 -->|"7928 / 2665"| E1["aggregate + README stats"]
    D1 -->|"1253 env retries"| C1
    E1 --> E2["cadence deliver<br/>delta this cycle —/100<br/>dual-repo bot PRs (idempotent)"]
    M["radar-probe every 15 min self-heal<br/>7 metric streams × 60s · done 4"]
    M -.-> A1
    M -.-> C1
```
<!-- AUTO:pipeline:END -->
...
- **🔌 Web UI 增强**（2207）— OK 1213 · incompatible 390 · pending 198 · untested 16 · watching 390 — [details](PLUGINS-ALL.md#-web-ui-增强2207)
- **💻 编码开发**（1686）— OK 838 · incompatible 325 · pending 154 · untested 18 · watching 351 — [details](PLUGINS-ALL.md#-编码开发1686)
- **🤖 Agent 能力**（1639）— OK 734 · incompatible 303 · pending 145 · untested 10 · watching 447 — [details](PLUGINS-ALL.md#-agent-能力1639)
- **📡 消息通讯**（553）— OK 225 · incompatible 124 · pending 45 · untested 3 · watching 156 — [details](PLUGINS-ALL.md#-消息通讯553)
...
- **🛠 基建部署**（1041）— OK 410 · incompatible 127 · pending 126 · untested 3 · watching 375 — [details](PLUGINS-ALL.md#-基建部署1041)
- **📚 学习研究**（108）— OK 30 · incompatible 9 · pending 8 · untested 2 · watching 59 — [details](PLUGINS-ALL.md#-学习研究108)
- **❓ 其他**（4362）— OK 1571 · incompatible 293 · pending 223 · untested 20 · watching 2255 — [details](PLUGINS-ALL.md#-其他4362)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260831T021501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260831T021501Z（2026-08-31 10:15 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 7928 可用 · 2665 不兼容 · 1253 待定（共 11846 个，k8s agent 口径）|

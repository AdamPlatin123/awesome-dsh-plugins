...
> Data as of snapshot `20260905T054501Z` (2026-09-05 13:45:03 UTC+8 · classifier unified-v2-bridge)

<!-- AUTO:pipeline:START -->
```mermaid
flowchart TB
    subgraph Discovery["Discovery (every 6h · probe every 15 min)"]
        A1["GitHub Search<br/>topic ×2 + keyword ×3<br/>candidates 18297 · age 12m"]
    ...
    B1 -->|"plugins 844"| C1["k8s runtime test<br/>1 pod per plugin · concurrency 10<br/>dsh agent + Qwen (de-stream)"]
    B1 -->|"non-plugins (dropped 0)"| B3["dropped to save space"]
    C1 --> D1{"verdict · total 13181"}
    D1 -->|"9002 / 2468"| E1["aggregate + README stats"]
    D1 -->|"1711 env retries"| C1
...
- **🔌 Web UI 增强**（2752）— OK 1347 · incompatible 375 · pending 262 · untested 16 · watching 752 — [details](PLUGINS-ALL.md#-web-ui-增强2752)
- **💻 编码开发**（2134）— OK 929 · incompatible 338 · pending 191 · untested 19 · watching 657 — [details](PLUGINS-ALL.md#-编码开发2134)
- **🤖 Agent 能力**（2228）— OK 821 · incompatible 253 · pending 167 · untested 10 · watching 977 — [details](PLUGINS-ALL.md#-agent-能力2228)
- **📡 消息通讯**（710）— OK 259 · incompatible 109 · pending 56 · untested 3 · watching 283 — [details](PLUGINS-ALL.md#-消息通讯710)
- **🗂 文件数据**（671）— OK 273 · incompatible 89 · pending 57 · untested 9 · watching 243 — [details](PLUGINS-ALL.md#-文件数据671)
...
> [!NOTE]
> **Test version**: dsh (in-container agent) driven by Qwen3.6-35B (via the de-stream proxy) · k8s, 5 shards · each run is anchored by the snapshot `run_id` (currently `20260905T054501Z`). The DSH npm version is not recorded per snapshot — cross-check via run_id and the `reports/agent-test/` dates.
...
> 渲染于快照 20260905T054501Z（2026-09-05 13:45 UTC+8）· 数据源 data/snapshots/（渲染即对齐）

| 证据层 | 当前结果 |
|---|---:|
| 自动收录 全量索引 2943 · 收录（克隆验证）2996 · 当前版本（0.1.1-rc.2）已测 345 个仓库 |
| 运行级实测 | 9002 可用 · 2468 不兼容 · 1711 待定（共 13181 个，k8s agent 口径）|

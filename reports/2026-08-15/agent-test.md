# Agent 运行级测试（2026-08-15 · dsh + Qwen3.6-35B · k8s 5 分片）

- 方法：容器内 dsh（agent）经 de-stream 代理用 Qwen 读插件 README → 自主导入 → 验证；3 次重试，全败判不兼容。
- 总计 814：✅可用 628 / ❌不兼容 130 / ⏭️跳过 0 / 未知 55
...
| dsh-updater-ui | ✅ 可用 | (无) |
| dsh-upstream-watch | ✅ 可用 | (无) |
| dsh-usage-chart | ✅ 可用 | (无) |
| dsh-usage-cost | ✅ 可用 | (无) |
...
| dsh-tui-app | ❌ 不兼容 | FAIL: 依赖包（ink, @deepseek-ai/dsh-agent, @deepseek-ai/dsh-llm, @deepseek-ai/dsh-session 等）未安装，且环境无法写入依赖缓存，导致模块动态导入失败（ERR_MODULE_NOT_FOUND）。 |
...
| dsh-vision-toolkit | ❌ 不兼容 | FAIL: dsh plugin 安装依赖 pnpm，但当前环境未安装 pnpm 且无法通过 npm 安装 |
...
| dsh-web-search-exa | ❌ 不兼容 | FAIL: 缺少 peer 依赖（@deepseek-ai/dsh-settings），无法独立加载 |
...
| dsh-whale-diving | ❌ 不兼容 | FAIL: 该插件是 dsh monorepo 的 workspace 包，依赖 workspace:^ 协议，且 lib/ 构建产物缺失，无法独立安装和加载。 |
...
| Liltloom | ❌ 不兼容 | FAIL: 缺少 DSH 运行时 peer 依赖（@deepseek-ai/dsh-scope 等），无法在无 DSH 环境的 Node 中加载。 |
...
| zat-dsh-engine | ❌ 不兼容 | FAIL: peerDependencies (cordis, dsh-typert-protocol, dsh-subprocess, dsh-fs, dsh-llm) 均为 @deepseek-ai 内部私有包，独立环境无法安装，只能在完整 DSH 运行时中加载。 |
...
| dsh-gpu | ⚠️ 待定 | (无) |
| dsh-noema | ⚠️ 待定 | (无) |
| dsh-plugin-qqbot | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-plugin-ssh-remotes | ⚠️ 待定 | (无) |
| dsh-provider-billing | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-rich-file-reader | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-serenity-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-session-tree | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-show-image | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-silly-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-SkillsManagePlugins | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-skin20260814 | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-snapshot | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-Solarized | ⚠️ 待定 | (无) |
| dsh-spec-loop | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-super-injector | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-surface-contract-proof | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-task-planner | ⚠️ 待定 | (无) |
| dsh-testgen | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-text-diff | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-theme-palettes | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-theme-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-theme-taffy | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-think-any-lang | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-tmcra-memory | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-todo-freshness-guard | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-token-dashboard | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-tool-obsidian-vault | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-tool-somark | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| dsh-wanghong-handwritten-ppt | ⚠️ 待定 | (无) |
| dsh-wechat-bridge | ⚠️ 待定 | (无) |
| dsh-whale-report | ⚠️ 待定 | (无) |
| dsh-windows-readiness-proof | ⚠️ 待定 | (无) |
| dsh-workshop | ⚠️ 待定 | (无) |
| dsh-workspace-enhance | ⚠️ 待定 | (无) |
| dsh-workspace-env | ⚠️ 待定 | (无) |
| dsh-xiapan-media | ⚠️ 待定 | (无) |
| dsh-yolo-mode | ⚠️ 待定 | dsh: STREAM_CLOSED: SSE stream ended without [DONE] |
| dsh-youmind-plugin | ⚠️ 待定 | (无) |
| dsh-yuzuha-prompts-manager | ⚠️ 待定 | (无) |
| dsh-zotero | ⚠️ 待定 | (无) |
| knowlp-rag | ⚠️ 待定 | (无) |
| Liang-Saint-Slider | ⚠️ 待定 | (无) |
| mcp-server | ⚠️ 待定 | (无) |
| mstar-harness | ⚠️ 待定 | (无) |
| obsidian-dsh | ⚠️ 待定 | (无) |
| remielle-dsh-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| remio-harness-plugin | ⚠️ 待定 | (无) |
| sage-mem | ⚠️ 待定 | (无) |
| session-teleport | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| silk-background | ⚠️ 待定 | (无) |
| slcatwujian-dsh-vision-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| surfing-plugin | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |
| TokenLedger | ⚠️ 待定 | (无) |
| yet-another-subagent | ⚠️ 待定 | dsh: TRANSPORT: DeepSeek API request to http://127.0.0.1:18093/v1 failed |

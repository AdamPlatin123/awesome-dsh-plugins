...
- 总计 242：✅可用 121 / ❌不兼容 121 / ⏭️跳过 0 / 未知 0
...
| cross-harness-cite | ❌ 不兼容 | All peer deps exist in profiles. Let me set up the plugin in the workspace and try to build + load it. |
| deepseek-harness-desktop | ❌ 不兼容 | (无) |
| deepseek-manners | ❌ 不兼容 | The `headless` profile already has all the deps. Let me try loading via that: |
...
| dsh-agent-budget | ❌ 不兼容 | Now let me test loading the plugin from this writable location: |
| dsh-agent-teams | ❌ 不兼容 | Fix npm cache permission issue and try installing. |
| dsh-aigc-canvas | ❌ 不兼容 | The `@deepseek-ai/*` packages aren't available on the public npm registry — they're internal/deepseek packages. Let me install only producti |
...
| dsh-auto-chess | ❌ 不兼容 | The DSH plugin is available. Let me try the dsh plugin install command directly as README says, and also check if we can build the JS withou |
| dsh-automation | ❌ 不兼容 | FAIL: 缺少依赖包 @deepseek-ai/schemastery（未列入 package.json） |
| dsh-bash-encoding | ❌ 不兼容 | Good, the source is now writable. Let me build and test loading in the writable location. |
| DSH-better-sidebar | ❌ 不兼容 | 5. 验证加载 |
| dsh-browser | ❌ 不兼容 | (无) |
| dsh-browser-panel | ❌ 不兼容 | (无) |
...
| dsh-code | ❌ 不兼容 | The `prepareProfile` function tries to write to `cordis.yml`. Let me check if the profile's `cordis.yml` has a read-only issue or if there's |
...
| dsh-companion | ❌ 不兼容 | I've done the reads. Now let me test actual loading with a clean approach. |
...
| dsh-design | ❌ 不兼容 | Let me try a different approach to symlink using `danger-full-access`: |
...
| dsh-engram-relay | ❌ 不兼容 | (无) |
| dsh-evolve | ❌ 不兼容 | npm is failing due to log directory. Let me check the source and try a different approach. |
| dsh-gomoku | ❌ 不兼容 | Sandbox blocked writing to ~/.dsh. Let me use NODE_PATH instead. |
| dsh-grok-tui | ❌ 不兼容 | The missing package is `@deepseek-ai/dsh-user-interaction`. Let me check which `@deepseek-ai/*` packages are available globally: |
| dsh-inspect | ❌ 不兼容 | The replace didn't strip references properly. Let me write a clean one. |
| dsh-interconnect | ❌ 不兼容 | OK, I need to work within `/home/node/work` (workspace-write sandbox). Let me build and test the plugin there. |
| dsh-interpreters | ❌ 不兼容 | npm cache 目录权限有问题（root 所有），需要更宽的文件权限来修复： |
| dsh-involute | ❌ 不兼容 | (无) |
...
| dsh-plus | ❌ 不兼容 | The dsh-external GitHub repos are private and can't be cloned. Let me try to verify the module loads directly and run the install script to  |
...
| dsh-self-control-guard | ❌ 不兼容 | The DSH profile already has `@deepseek-ai/dsh-llm` and all the peer deps. The build succeeded. The only failure was that the plugin's peer d |
| dsh-session-cluster | ❌ 不兼容 | (无) |
...
| dsh-super-injector | ❌ 不兼容 | The sandbox blocks writes to `/home/node/.dsh/profiles/web`. Let me try an alternative path using the workspace. |
...
| dsh-tool-schema | ❌ 不兼容 | The symlink at `/clones/node_modules/@deepseek-ai/dsh-tools` blocks `cp`. Let me create a separate directory with the deps and point to it. |
...
| dsh-vision-toolkit | ❌ 不兼容 | npm install blocked. Let me take the direct approach — the lib/ is already built, so test loading the module directly with Node. |
...
| dsh-web-review | ❌ 不兼容 | (无) |
| dsh-web-ui | ❌ 不兼容 | Build succeeded. The `dsh-web-ui-all` is an aggregator package (no build output, it uses `workspace:*` deps). Let me verify the actual plugi |
...
| fabric | ❌ 不兼容 | `/clones` is read-only and sandbox blocks creating `~/.dsh/profiles/web`. Let me try the existing `headless` profile instead. |
| mstar-harness | ❌ 不兼容 | The `dsh plugin` command hardcodes `/home/node/.dsh` and ignores the `HOME` env var. Let me try directly requiring the dsh package entrypoin |
...
| oh-dsh-desktop | ❌ 不兼容 | The build fails because the `upstream/DSH-better-sidebar` submodule is missing. Let me try to initialize it and re-run the build — but first |
| oh-my-dsh | ❌ 不兼容 | FAIL: npm 缓存目录 `/home/node/.npm` 权限不足（root 所有），无法安装依赖或构建插件 |
| Qwen-MM-Plugins | ❌ 不兼容 | Good — the code imports but can't resolve `schemastery`. Let me install the one real dependency and retry. |
...
| sandbox-nono | ❌ 不兼容 | The peer deps `@deepseek-ai/dsh-sandbox` and `@deepseek-ai/dsh-invariants` are internal - not on npm. Let me create proper stubs and try com |
...
| zephyr | ❌ 不兼容 | Dependencies installed. Now let me verify the plugin load. |
